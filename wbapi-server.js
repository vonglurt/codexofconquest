    ...
    const HELP = {
      ...
      import: {
        ...
        body: [
          ...
          '',
          'This directive applies to every book imported into the game.',
          'Seeds complete = import trigger. Process one location at a time.',
          '',
          'PRE-IMPORT — READ FIRST (required before any API call)',
          '  1. Read the book source file:',
          '       1367-sources/{CODE}-{title}.md  — vignette seeds + UQF quest cycles',
          '  2. Read the index:',
          '       1367-sources/index.md           — canonical nodes, airport codes, terrain',
          '  Location information MUST adhere to the story. Cross-reference the vignette',
          '  text to verify each node label, terrain, and geographic position.',
          '',
          'NODE NAMING — AIRPORT CODES',
          '  Use the IATA airport code for the nearest major airport as the 3-letter code.',
          '  If the IATA code is already taken, use the nearest alternate airport code.',
          '  If no airport exists (medieval-only location), derive a 3-letter abbreviation',
          '  from the city name and document it in index.md.',
          '  Always verify uniqueness first:',
          `    curl ${b}/api/list/node   # check all existing codes`,
          '',
          'DATA INTEGRITY — AUDIT BEFORE AND AFTER',
          '  Run GET /api/audit before the first write (baseline) and after the last',
          '  write (post-write check). Fix all errors before POST /api/save.',
          '  Cell spacing: the 4x expansion left 3 empty cells between every adjacent',
          '  pair of original nodes — there is ample room; use GET /api/coords/near/{code}',
          '  to find unoccupied slots.',
          '',
          'IMPORT STEPS (one location at a time)',
          '  STEP 0 — Baseline audit',
          `    curl ${b}/api/audit | jq \'.errors\'`,
          '  STEP 1 — Verify code is available',
          `    curl ${b}/api/list/node | jq \'keys\'`,
          '  STEP 2 — Find available slots near starting node',
          `    curl '${b}/api/coords/near/NTN?radius=8'`,
          '    → available[]: empty {r,c} slots sorted by distance.',
          '    1–3 cells: same-town nodes | 4–8: route waypoints | 8+: distant cities',
          '  STEP 3 — Create node with coordinates',
          `    curl -XPOST ${b}/api/node -d \\`,
          '      \'{"code":"NTN","label":"Nottingham — Sheriff\'s Gate","name":"city","act":3,"r":48,"c":116}\'',
          '  STEP 4 — Verify node',
          `    curl ${b}/api/location/NTN`,
          '  STEP 5 — Create state flags',
          `    curl -XPOST ${b}/api/flags -d \'{"name":"stnAct1Done","defaultValue":false}\'`,
          '  STEP 6 — Create chained quests (one per act)',
          '    Act 1: activateNode=starting town, no activateCond',
          '    Act 2+: activateCond = previous act checkPassFlag',
          '    Act 5: questComplete: true',
          '  STEP 7 — Verify chain',
          `    curl ${b}/api/quest/{id}/chain`,
          ...
          '',
          'IMPORT QUEUE',
          '  See 1367-sources/plan.md §IMPORT-01 for the full per-book queue.',
          '  Each book: read story → audit → nodes → flags → quests → audit → save → commit.',
        ].join('\n'),
      },
