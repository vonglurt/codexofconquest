...
function cli(args) {
  return new Promise((resolve) => {
    execFile('node', [path.join(ROOT, 'src', 'api', 'wb.js'), ...args], { cwd: ROOT }, (err, stdout, stderr) => {
...
async function main() {
  ...
  const aclEx = JSON.parse(fs.readFileSync(path.join(ROOT, 'src', 'config', 'mesh-acl.json.example'), 'utf8'));
