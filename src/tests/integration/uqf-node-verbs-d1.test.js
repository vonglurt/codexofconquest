...
test.describe('§VM-01-G4c — the registry after the D1 migration', () => {
  test('the four D1 verbs are button verbs, and every entry belongs to a dispatched group', async ({ page }) => {
    ...
    // added the §CROWN-01 dispatch verbs; §VM-01-G-FU-e added the §LXX smelt verb; §DX-02fv added
    // `tl-vonn`, which carries a label but is a D2 exclusive choice, not a D1 button. This test
    // owns the G4c D1 set, uqf-node-verbs-d3.test.js owns the CDG tail,
    // uqf-node-verbs-crown.test.js the crown groups, uqf-node-lxx.test.js the lxx group, and
    // dx02fv-tl-vonn-choice.test.js the Vonn beat.
    const d1 = r.filter(v => v.hasLabel && (v.group || '').indexOf('cdg-') !== 0
                          && (v.group || '').indexOf('crown-') !== 0
                          && (v.group || '').indexOf('lxx-') !== 0
                          && v.group !== 'tl-vonn').map(v => v.id);
