...
const CFG  = path.join(ROOT, 'src', 'config');              // config/ — data + config files
...
// sentry's cell (play.html §MESH-01h). Single-writer holds: only THIS
...
function sessionPrune() {
  ...
  for (const [id, s] of SESSIONS) {
    ...
    if (now - s.lastSeen > SESSION_TTL) {
      ...
      const evt = presenceEvt(id, s.playerName, { r: s.r, c: s.c }, null);
      broadcastCell(s.r, s.c, 'player_left', evt, id);
      broadcastAll('player_moved', evt, id);
      emitMeshEvent('player_left', evt, s.r, s.c);
      emitMeshEvent('player_moved', evt, s.r, s.c);
...
// One envelope for every presence event — {pid, name, from, to} built in
// exactly one place so the surfaces can never drift apart (the FU 3
...
const SERVER_NAME = String(process.env.SERVER_NAME
  ...
  || require('os').hostname() || 'coc-server').slice(0, 60);
...
// ══ §MESH-01 mesh layer → mesh.js (§MESH-01-REVIEW extraction) ═══════════════
// ACL · rate limit · presence gossip · tracker/federation/bootstrap now live in
// mesh.js (the mover.js/rooms.js precedent). The factory receives this server's
// live surfaces; destructuring the same names back keeps every call site below
// unchanged. All deps are defined above this line — order matters.
const {
  MESH, TRACKER,
...
// client builds at load (play.html). Closes the latent bug where the
...
// (play.html): a sea-lane cell is 'ocean'; otherwise the majority terrain
...
// Mirrors the client _roomWorld() (play.html) field-for-field — same
...
// ═══════════════════════════════════════════════════════════════════════════
// Schema — canonical field reference derived from play.html
// ═══════════════════════════════════════════════════════════════════════════
const SCHEMAS = {
  _version: '1.0',
  _source: 'play.html',
  ...
  monster: {
    ...
    fields: {
      ...
      voidTainted:{type:'boolean',  required:false, editable:true,  note:'Read by the enemy-AI Void classifier; when absent the classifier falls back to a name match. Carried by 3 monsters (§DX-02iy).' },
    },
    related: {
  ...
  node: {
    _section: 'NODE_MAP',
    _description: 'World map nodes. Each node is a location the player can visit. Adjacency is coordinate proximity through CELL_GRID (§CELL-01), not the N/S/E/W fields below.',
    fields: {
      code:     { type:'string',  required:true,  editable:false, note:'Node code (BK, LHR, etc.). 2–3 characters. Use MOVE to rename.' },
      ...
      board:    { type:'boolean', required:false, editable:true,  note:"Forces or forbids a Warrant's Board here; absent, `_boardHost` falls back to `sleep`." },
      isFishingLake:{type:'boolean',required:false,editable:true, note:'Offers the fishing card at this node.' },
      finalBattle:{type:'object', required:false, editable:true,  note:'{minLevel, minShards} — the entry requirement for the final encounter.' },
      N:        { type:'string',  required:false, editable:true,  classified:'written-not-carried',
                  note:'Retired as a node field by §CELL-01 — 0 of 416 nodes carry it and nothing reads it; adjacency is CELL_GRID. Still WRITTEN by the graph endpoints (connect, spawn-junction, cluster-bridge), which is why it is classified rather than removed (§DX-02kv).' },
      S:        { type:'string',  required:false, editable:true,  classified:'written-not-carried', note:'See N.' },
      E:        { type:'string',  required:false, editable:true,  classified:'written-not-carried', note:'See N.' },
      W:        { type:'string',  required:false, editable:true,  classified:'written-not-carried', note:'See N.' },
    },
  ...
  quest: {
    ...
    fields: {
      ...
      vignetteText:{ type:'string',  required:false, editable:true,  note:'Board vignette prose; falls back to `hint` when absent. Carried by 94.' },
      vignetteTextAlt:{type:'string',required:false, editable:true,  note:'Alternate vignette shown when the quest has already been failed once. Carried by 1.' },
      rumor:       { type:'string',  required:false, editable:true,  note:"Authored Warrant's Board hook; absent, the board synthesizes one from the destination terrain. Carried by 25." },
      boardExempt: { type:'boolean', required:false, editable:true,  note:"True keeps a personal or arc-scripted quest off the Warrant's Board entirely (§VM-01-G3). Carried by 3." },
      retryable:   { type:'boolean', required:false, editable:true,  note:'A failed attempt may be retried after the retry gate elapses. Carried by 121.' },
      retryGateDays:{type:'number',  required:false, editable:true,  note:'Days after a failed attempt before a retryable quest is offered again; defaults to 1. Carried by 21.' },
      onActivate:  { type:'object',  required:false, editable:true,  note:'{msg, delayMs?} — a story message posted when the quest activates. Carried by 20.' },
  ...
  fish: {
    ...
    fields: {
      ...
      isNight:{ type:'boolean',required:false, editable:false, classified:'derived',
                note:'Not stored on any entry — derived from which pool the fish lives in (FISH_POOL vs NIGHT_FISH_POOL).' },
    },
    related: {
...
function serializeNpcDialoguesSection() {
  const entries = Object.entries(WBAPI.npcDialogues)
    ...
    .join('');
  return `\n// GENERATED by serializeNpcDialoguesSection() in src/js/wbapi-server.js — any write to a /api/npc/*/dialogue route re-emits this header, edits made here do not survive one.\n// Keyed by NPC key (brynn, yael…); the per-node resident voices are the singular NPC_DIALOGUE.\nconst NPC_DIALOGUES = { // → doc: docs/story/story-arc-npc-dialogues.md\n${entries}};\n`;
}
...
function serializeD100Table(entries) {
  ...
  return `\n// GENERATED by serializeD100Table() in src/js/wbapi-server.js — any write to /api/loot re-emits this header, edits made here do not survive one.\n// Unified d100 drop table — each entry: { weight, _type, _magic? }\n// _type: 'potion_minor'|'potion'|'potion_greater'|'potion_superior'|'scroll'|'flashbang'|'dagger'|'mainweapon'|'gold'\n// A 'dagger' or 'mainweapon' row REQUIRES _magic: _rollD100Loot() gates it on _magicTierAllowed(row._magic), and an absent _magic compares against NaN, so the row never drops and never errors.\nconst _D100_TABLE = [ // → doc: docs/mechanics/mechanics-combat.md §Loot Table\n${rows}\n]; // total weight = ${total}\n`;
}
...
async function route(req, res) {
  ...
  if (parts[0] === 'help' && method === 'GET') {
    ...
    const HELP = {
      ...
      overview: {
        ...
        body: [
          'OVERVIEW',
          '  play.html is the single source of truth for the entire game.',
      ...
      modes: {
        ...
        body: [
          ...
          '   POST / PUT / DELETE endpoints that mutate play.html.',
          ...
          '     → { "nonce": "ab12cd34ef56gh78", "type": "quest", "id": "quest_new_01",',
          '         "expiresAt": "2026-09-07T04:11:00.000Z" }',
          '',
          ...
          '',
          '   Writes auto-save. There is no save step.',
          `     curl -XPOST ${b}/api/save     # a dated SNAPSHOT beside the game file,`,
          '                                        # not a flush of pending work',
          '',
      ...
      nonce: {
        ...
        body: [
          ...
          '',
          '  Response: { "nonce": "ab12cd34ef56gh78", "type": "quest", "id": "quest_new_01",',
          '               "expiresAt": "2026-09-07T04:11:00.000Z" }',
          '  Nonces expire 5 minutes after issue. Read expiresAt — there is no',
          '  `expires` field, and .expires reads undefined into an empty header.',
          '',
          'USING THE NONCE',
      ...
      read: {
        ...
        body: [
          ...
          '',
          `GET ${b}/api/source`,
          '  Raw HTML source of play.html. Pipe to file to download.',
      ...
      write: {
        ...
        body: [
          ...
          '',
          'SYSTEM',
          `  POST ${b}/api/save      — serialise all in-memory edits back to play.html`,
          `  POST ${b}/api/reload    — re-parse play.html from disk (auto-reload already active via fs.watch — manual call is redundant)`,
      ...
      node: {
        ...
        body: [
          'NODE SCHEMA',
          '',
          '  code    string   2–3 char map code (e.g. BK, LHR, VBY)',
          '  label   string   display name shown on map',
          '  name    string?  terrain key from WORLD_DB (e.g. scholars_qtr)',
          ...
          `    -H \'Content-Type: application/json\' \\`,
          `    -d \'{"code":"SD","label":"Sunken Docks","act":1,"name":"docks",`,
      ...
      terrain: {
        ...
        body: [
          ...
          '',
          '  key       string   snake_case identifier (e.g. market_quarter)',
          ...
          `  curl -XPUT http://localhost:${PORT}/api/terrain/fog_docks \\`,
          `    -H \'Content-Type: application/json\' \\`,
          `    -d \'{"monsters":["dock_rat","drowned_sailor","goblin"]}\'`,
        ].join('\n'),
      },
      ...
      import: {
        ...
        body: [
          ...
          '      "activateNode":"SHW","checkStat":"wis","checkDC":12,',
      ...
      export: {
        ...
        body: [
          ...
          '                    A world exported this way can be walked and talked to:',
          '                    coordinates and NPCs travel with it.',
          '',
          'FORMATS',
      ...
      workflow: {
        ...
        body: [
          ...
          '  ./bin/api list quest --arc shk --q "format"   # find quest ID',
  ...
  if (parts[0] === 'tracker' && parts[1] === 'peers' && method === 'GET') {
    ...
    if ((url.searchParams.get('format') || '') === 'txt') {
      ...
      return res.end([
        `# coc mesh bootstrap — generated by tracker ${getServerId().slice(0, 8)} at ${new Date().toISOString()}`,
  ...
  // Serves this server's play.html verbatim with identity headers. The
  ...
  if (parts[0] === 'reload' && method === 'POST') {
    try {
      ...
      return json(res, 200, {
        ok: true,
        note: 'Auto-reload is already active — the server watches play.html and reloads automatically on any external edit. Manual POST /api/reload is redundant unless the watcher missed an event.',
      });
    } catch(e) {
    ...
    if (method === 'PUT' && !parts[1]) {
      ...
      try {
        ...
        const run = (script) => new Promise((resolve) => {
          execFile(process.execPath, [path.join(ROOT, 'src', 'scripts', script), ...(script === 'build-roads.js' ? ['--apply'] : [])],
  ...
  if (parts[0] === 'session') {
    ...
    if (sub === 'start') {
      ...
      const evt = presenceEvt(sessionId, playerName, null, { r: s.r, c: s.c });
      broadcastCell(s.r, s.c, 'player_arrived', { ...evt, node: hub }, sessionId);
      emitMeshEvent('player_arrived', { ...evt, node: hub }, s.r, s.c);
      const look = buildLook(s);
    ...
    if (sub === 'move') {
      ...
      const evt = presenceEvt(sessionId, s.playerName, { r: prevR, c: prevC }, { r: newR, c: newC });
      broadcastCell(newR, newC, 'player_arrived', { ...evt, node: newCode }, sessionId);
      broadcastAll('player_moved', evt, sessionId);
    ...
    if (sub === 'pos') {
      ...
      if (moved) {
        const evt = presenceEvt(sessionId, s.playerName, { r: prevR, c: prevC }, { r, c });
        broadcastCell(prevR, prevC, 'player_left', evt, sessionId);
        broadcastCell(r, c, 'player_arrived', { ...evt, node: s.nodeCode }, sessionId);
        broadcastAll('player_moved', evt, sessionId);
        emitMeshEvent('player_left', evt, prevR, prevC);
    ...
    if (sub === 'end') {
      ...
      const evt = presenceEvt(sessionId, s.playerName, { r: s.r, c: s.c }, null);
      broadcastCell(s.r, s.c, 'player_left', evt, sessionId);
      broadcastAll('player_moved', evt, sessionId);
      emitMeshEvent('player_left', evt, s.r, s.c);
      emitMeshEvent('player_moved', evt, s.r, s.c);
  ...
  if (type === 'export' && rawId && method === 'GET') {
    ...
    const exportMap = {
      ...
      all:             () => ({
        NODE_MAP: WBAPI.nodeMap, NODE_COORDS: WBAPI.nodeCoords,
        QUEST_DB: WBAPI.questDb,
...
server.listen(PORT, BIND_ADDR, () => {
  ...
  console.log(`    ${C.green}curl${C.reset} ${C.dim}${b}/api/node/BK${C.reset}           ${C.dim}# node by code${C.reset}`);
