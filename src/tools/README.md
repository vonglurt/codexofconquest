...
| File | What it does |
|------|--------------|
| `worldmap.js` | Terminal ASCII world-map views. Spawned by `./bin/api worldmap [--city/--region/--search/--route]` (path wired in `../api/wb.js`); also runnable directly: `node src/tools/worldmap.js --regions`. |
| `layout-solve.js` | Propagates node coordinates from geo anchors: `node src/tools/layout-solve.js --apply`. |
| `layout-spring.js` | Force-directed layout experiment (standalone). |
| `wbapi-cli.js` | Legacy WBAPI CLI (superseded by `../api/wb.js` / `./bin/api`). |
