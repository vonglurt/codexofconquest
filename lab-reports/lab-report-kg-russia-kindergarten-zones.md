...
|---|---|
| Census via the real parser | `js/wbapi-core.js` `W.load()` on parent, ship and HEAD — never a line regex |
...
| Acceptance test re-run | `tests/integration/kg-zones.test.js` at HEAD |

> The ship tree is **flat** — the `scripts/` and `js/` reorganisation is later — so an unprefixed path in
...
| off-road cell adjacent to a node inherits that terrain | `function _inferTerrain(r, c) {@28383` — orthogonal-neighbour majority | ✓ |
| Hunt Mode 80/20 at-or-below bias | `if (S_story.huntMode && _seededNext() < 0.8) {@38245` | ✓ |
| Hunt Mode roughly doubles the rate | `if (S_story.huntMode) baseRate = Math.min(0.8, baseRate * 2);@28440` | ✓ |
| battle XP = AC · maxHP | `const xpAward = Math.round((S.enemy.ac@25292` | ✓ |
| Level 6 costs 5,500 XP | `const XP_LEVELS = [@24418` — index 5 is 5,500 | ✓ **exact** |
...
`tests/integration/kg-zones.test.js` — **5/5 green**. `check:roads` — green (R1 244/244 · R2 single
...
The render path is `const _npcDial = NPC_DIALOGUE[node.code];@35818`, the d-pad gate is
`!(node.npc && NPC_DIALOGUE[node.code])@35924` — and none of the five NODE_MAP
...
Exploration XP (`const EXPLORE_XP = 10;@24435`) and effort XP (`const EFFORT_XP_PCT = 0.25;@24426`, added
four days later) both shorten it; neither existed on the day this was designed.

**Hunt Mode does what the report says.** `function _weightedMonsterPick(terrain) {@38237` filters to
