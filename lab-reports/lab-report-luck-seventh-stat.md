...
**Original:** roll2hit.com development session, 2026-05-25 · **Verified against HEAD:** 2026-08-12 (§DOC-02w)
**Subject:** `roll2hit-v3.html` — single-file browser D&D 5e RPG (17,631 lines at authoring; 38,712 at HEAD)
...
`function _calcLuck() {@23438` · `function _luckMod() { return Math.floor((_calcLuck() - 10) / 2); }@23444`
...
|---|---|---|
| d100 loot roll | `Math.max(0, _luckMod())); // Layer 48: Luck bonus@24546` | `min(99, u + max(0, L))`; now on the seeded stream (§VM-01-B) |
| Death saves | `let d20 = Math.ceil(Math.random() * 20) + _luckMod();@25896` | flat `+L`; **unseeded** — §DX-02m's named instance |
| Tournament tie-break | `outcome = _luckMod() > 0 ? 'win'@34130` | undocumented; added after this report; hosted at `SSJ:{r:4,c:192},@9430` |
| Character sheet | `✦ LUCK@37688` | `${_calcLuck()} [${m(_luckMod())}]` + the italic note, verbatim |

**Live but unreachable consumers (4)** — all behind **§FISH-01**

`Luck reduces Survival DC@30491` (and its title twin) · `bare hook uses LuckMod@30526` ·
`typeTotal  = tDie + bait.type + _luckMod()@30556` · `const lm = _luckMod();@23423` (§DROP-03
`luckScale`, added later).

`const hasFish = node.isFishingLake;@35366` is the only gate on the Fish button, and
...
a cell. `function storyFishing() {@30392` therefore never draws.
...
is a finite space: `const _CC_COST = [0,1,2,3,4,5,7,9];@38564` with `const CC_BUDGET = 27;@38565` and
...
untouched custom panel (`let _cc_scores = { str:10, dex:10, con:10, int:8, wis:8, cha:8 };@38567`) —
...
That `||` branch is unreachable: `abilityScores: { str:10, dex:8, con:8, int:8, wis:8, cha:8 },@23046`
is declared in `_S_DEFAULTS()`, and `const scores = startScores || { str:10, dex:8, con:8, int:8, wis:8, cha:8 };@23952`
...
- **§DX-02m (existing, +confirmation)** — `let d20 = Math.ceil(Math.random() * 20) + _luckMod();@25896`
...
| Anchor | Content |
|---|---|
| `function _calcLuck() {@23438` | geometric-mean computation, byte-identical since `3f11e5b` |
| `function _luckMod() { return Math.floor((_calcLuck() - 10) / 2); }@23444` | standard modifier |
| `const s = S_story.abilityScores || { str:16, dex:12, con:14, int:10, wis:12, cha:8 };@23439` | the dead fallback (§V-D) |
| `abilityScores: { str:10, dex:8, con:8, int:8, wis:8, cha:8 },@23046` | the real default |
| `const scores = startScores || { str:10, dex:8, con:8, int:8, wis:8, cha:8 };@23952` | `storyNewGame` |
| `const _CC_COST = [0,1,2,3,4,5,7,9];@38564` · `const CC_BUDGET = 27;@38565` | the space enumerated in §V-B |
| `let _cc_scores = { str:10, dex:10, con:10, int:8, wis:8, cha:8 };@38567` | untouched-panel default → Mod −1 |
| `Math.max(0, _luckMod())); // Layer 48: Luck bonus@24546` | loot roll |
| `const _D100_TABLE = [@24516` · `const gp = Math.floor(_seededNext() * 200) + 50;@24556` | what luck's 1 pp actually moves |
| `let d20 = Math.ceil(Math.random() * 20) + _luckMod();@25896` | death saves |
| `outcome = _luckMod() > 0 ? 'win'@34130` · `SSJ:{r:4,c:192},@9430` | tournament tie-break (undocumented) |
| `Luck reduces Survival DC@30491` · `bare hook uses LuckMod@30526` · `typeTotal  = tDie + bait.type + _luckMod()@30556` | stranded fishing trio |
| `const lm = _luckMod();@23423` | §DROP-03 `luckScale`, also stranded |
| `function storyFishing() {@30392` · `const hasFish = node.isFishingLake;@35366` | the single entry point |
| `BOO:{r:2,c:194},@9422` · `LYR:{r:2,c:194},@9423` · `const CELL_GRID = (() => {@9852` | §FISH-01 |
| `✦ LUCK@37688` | character sheet |
| `_rarityFromRoll = (r) => r <= 5@30429` | rarity thresholds — home docs exact |
