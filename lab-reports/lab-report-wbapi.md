...
`js/wbapi-core.js` parser and counting keys — the same parser the `:1367` server uses — never by
...
| File | Role | At HEAD |
|---|---|---|
| `wbapi-core.js` | parse + CRUD + save/export/sync | ✅ `js/wbapi-core.js` |
| `wbapi-server.js` | HTTP layer, port 1367 | ✅ `js/wbapi-server.js` |
| `worldbuilder.html` | browser UI — Map, Bestiary, NPCs, Quests, Dice Lab, ⚙ API tab | ✅ |
| `api.sh` / `api/wb.js` | primary CLI — queued HTTP, auto-nonce, retry, `--ai` | ✅ *(retrofit — see §II)* |
...
All three parsers and all four helpers verify at both trees (`js/wbapi-core.js:function extractObj@146`,
`js/wbapi-core.js:function removeFns@175`, `js/wbapi-core.js:function parseSimple@280`,
`js/wbapi-core.js:function parseSanitized@293`).
...
`new Function()` scope `P` is undefined. `js/wbapi-core.js:const Pp = new Proxy(P@290` returns
...
| Export | *The original file is **never modified**. Every `save()` produces a new timestamped copy.* | **inverted** — writes persist per-write via `saveGameFile()`; `js/wbapi-core.js:save() requires a destination path@1662` refuses the argless call | 🕳 **RETIRED** — §VI-4 |
| Export | `wbapi-cli.js export ./world` / `sync ./world` | `js/wbapi-core.js:exportWorld(dir) {@1694` / `js/wbapi-core.js:syncWorld(dir) {@1743` | ✅ mechanism survives its CLI |
...
see. `js/wbapi-core.js:chain(id) {@1089` reads `_questFlags` / `_flagToQuests` and nothing else.
...
`js/wbapi-core.js:this._questsByNode[q[field]].push(id);@775` sits inside
...
key-first is right. The rest is not: `js/wbapi-core.js:_findKey(col, idOrTitle) {@833` makes a single
...
`js/wbapi-server.js:claude-haiku-4-5-20251001@10607` and `api/wb.js`; port 1367 is right; and the
...
with `js/wbapi-server.js:SEED FALLBACK — set ANTHROPIC_API_KEY@10623` when no key is set. The
...
| Claim | Measured |
|---|---|
| *"NPC system prompt cached 5 min — repeated calls cost ~10 % of first call"* | The code does set `js/wbapi-server.js:cache_control: { type: 'ephemeral' }@10659`. **Claude Haiku 4.5's minimum cacheable prefix is 4096 tokens; this system block measures 451–546.** Below the minimum the API caches nothing and reports nothing — no error, `cache_creation_input_tokens: 0`. All 20 calls in `milepoints/npc-speak.log` show cache_read **and** cache_write at 0. |
