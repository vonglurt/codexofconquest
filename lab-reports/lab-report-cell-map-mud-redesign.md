...
Re-measured against live `roll2hit-v3.html` (38,707 lines, 416 nodes) and `js/wbapi-server.js` two months on:
...
### Server — `js/wbapi-server.js` (was `wbapi-server.js`; moved by `cc35c08` §CLEANUP-02, 2026-07-09)

| § | Claim | Anchor | Verdict |
|---|---|---|---|
| CELL-06 | 5 graph algorithms → cell-grid BFS | `js/wbapi-server.js:function buildCellGrid@948`, `js/wbapi-server.js:const MOVES4@944`, `js/wbapi-server.js:const DIR_NAMES@945` | ⚠️ **1 of 5** — see Δ1 |
| CELL-07 | MUD session layer, 7 endpoints, SSE, 30-min TTL | `js/wbapi-server.js:const SESSIONS@71` | ✅ shipped, now **9** endpoints (Δ3) |
| CELL-08 | 5 read-only cell/grid endpoints | `js/wbapi-server.js:→ node at grid cell@11532` (help table `@11532`–`@11536`) | ✅ all 5 live |
| CELL-08 | `POST`/`PUT /api/node` reject direction fields | `js/wbapi-server.js:_badNodeFields@10145`, `js/wbapi-server.js:_badPutFields@11043` | ✅ **live and load-bearing — empirically proven below** |
...
|---|---|---|---|
| **Δ1** | "All server-side graph algorithms (highway, reweave, fill-gap, fix-diagonal, reachability) replaced with cell-grid BFS." | **One** was: `GET /api/graph/reachability` (js/wbapi-server.js:parts[1] === 'reachability' && method === 'GET'@5501), and it went further than promised — it floods the **90×360 terrain field**, not the cell grid. `fill-gap` · `rip-and-connect` · `reweave-all` were **retired to 410** by §WALK-3. `cluster-bridge`, `junction-audit` and `smart-connect` still walk `node.N/S/E/W`. `highway` and `fix-diagonal` do not exist at HEAD under those names. | **PARTIAL — the report's headline defect** |
| **Δ2** | (unstated) `POST /api/graph/cluster-bridge --execute` bridges isolated clusters. | It **cannot**. Its two writes are `PUT /api/node/:code {N:…}` (`js/wbapi-server.js:const putA = await httpReq@7036`) — rejected 400 by §CELL-08's own guard. See Finding 2. | **INERT — contained by this report** |
| **Δ3** | Seven session endpoints: `start`, `move`, `look`, `who`, `say`, `end`, `events`. | **Nine.** `POST /api/session/pos` (js/wbapi-server.js:POST /api/session/pos@8918) and `GET /api/session/chat` (js/wbapi-server.js:§MESH-02(h) chat history@8718) (§MESH-02h chat history) were added later. | GROWTH — report accurate when written |
...
| **Δ7** | "Files Changed" lists 13 paths at repo root. | 4 moved (`wbapi-server.js` → `js/`, `wbapi-help.md`/`API-README.md` → `docs/api/`, `spec-corridors.md` → `docs/spec/`, `docs-node-network.md` → `docs/notes/`). Content claims hold. | STALE (path only) |
...
|---|---|---|
| `GET /api/graph/reachability` (js/wbapi-server.js:parts[1] === 'reachability' && method === 'GET'@5501) | **416 / 416 reachable · 1 component · 0 clusters** ✅ | the **terrain field** — an 8-way-clamped land flood over 90×360 with E↔W wrap |
| `GET /api/grid/reachability` (js/wbapi-server.js:if (sub === 'reachability')@8623) | **2 / 416 reachable · 414 unreachable** | `CELL_GRID` **named-node** adjacency — a node counts only if another *named node* touches it orthogonally |
| `POST /api/graph/cluster-bridge` (js/wbapi-server.js:parts[1] === 'cluster-bridge'@6919) | **1 / 416 reachable · 415 isolated clusters** | `node.N/S/E/W` — **deleted by §CELL-01** |
| `GET /api/graph/junction-audit` (js/wbapi-server.js:parts[1] === 'junction-audit'@7060) | `namedReachable: 2`, `namedUnreachable: 413` | same deleted edge set |
...
**Corollary defect:** the 68-line **auto-junction rule** (`js/wbapi-server.js:const autoJunctionEnabled@11054`) — which mints new `J\d+`
...
`POST /api/graph/nuke-junctions` (js/wbapi-server.js:parts[1] === 'nuke-junctions'@6643) selects by *code pattern* — `js/wbapi-server.js:const jCodeRe@6656` (`/^J\d+$/`) — and guards
...
The counterpoint is instructive. `POST /api/admin/delete-junctions` (js/wbapi-server.js:parts[1] === 'delete-junctions'@8337) gates on `if (!node.junction) continue`
...
*Verified §DOC-02ay, 2026-08-13, against `roll2hit-v3.html` @ `985505c` + live `js/wbapi-server.js` (port 1367).*
