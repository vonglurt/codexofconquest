...
`symbol@line` anchors validated through `node scripts/resolve-anchors.js`, not `grep`.
...
path: `7d3615a` later moved the file into `lab-reports/`, so the `lab-reports/` path returns nothing at
...
`node scripts/resolve-anchors.js`, not `grep -cF` — the §DOC-02ai lesson); `check:legacycodes` /
