...
| Five tier values `trivial\|easy\|medium\|hard\|deadly` | **exact** — now a pinned contract (§DX-02g) | `function _notorietyWeights@38207` |
| `_weightedMonsterPick` | **name survives; signature and body both replaced** — see §III | `function _weightedMonsterPick@38232` |
| `S_story.quests` as a key→status map | **exact** | — |
| Battle pipeline `loadWorldMonster` → `pendingBattle` → prebattle overlay → `storyApplyOutcome` | **live**, though entered from `_startStoryBattle` | `function _startStoryBattle@38254` |
...
encounter rate to a 0.8 cap (`if (S_story.huntMode) baseRate@28435`) and biases 80/20 toward
monsters at or below the player's level via `function _monsterLevel@38226`. It has no dialog, no
...
1. **§AUDIT-03t — 36 nodes carry `act:NaN`, and two render `undefined` to the player.** Traced
   ...
   readers fail in three different ways — `S_story.actNumber = node.act || 1@34568` silently
   coerces to Act 1 (NaN is falsy); `ACT_NAMES[node.act]@34597` yields `undefined`, so the act
...
[1] §TIMELESS-01 removal spec — `lab-reports/lab-report-timeless-movement-hunt-removal.md`,
...
[2] §WALK / §NAV-01 — navigation-core redesign (geo grid, `MOVER:CORE`, `ROAD_RUNS`); supersedes
    Layer 9 entirely. `plan-archive.md`, `lab-reports/lab-report-nav01-navigable-world.md`.
