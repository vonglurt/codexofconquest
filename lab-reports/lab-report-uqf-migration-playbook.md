...
| Wave 1l | Codex Inquisitor gauntlet (`quest_inquisitor_handshake`/`questions`/`final`) at NUE — `checkAbility`/`checkLabel` quests (the ~30 that worked pre-§SKILLFIX-01) → pure parity. **Fully decomposed** (no onPass `_legacy_fn`): `checkPassFlag`→`mission_bit{flag}` (no label), `xpAward`→`reward{xp}` (50/75/200), final's item-push → `reward{items:[Archive Key]}`. Only `questions` keeps `onFail:[_legacy_fn]` (hp−10 psychic). Chained flag gates; `retryGateDays` 0/0/1 top-level. The §D01-02 NUE handshake button dispatches transparently. **Zero engine changes.** | _this commit_ |
