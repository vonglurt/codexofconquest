...
| **1** | skill-check arcs **with** `onPass` closures (now migrating whole arcs incl. their `side` acts) + Wave-1 singletons | **78** onPass-closures + singleton cluster (done: wane 6, whisper 5, glut 5, ceremonia_yael 5, 1367 skill 4, d02xx 40, inn 3, spark 5, spark2 5, inquisitor 3, sea 3, sb 4, hunt+hunt2 8, bilge 4, alch 7, scar 4, §SIREN-01 5 = **~105 quests**) | full bit-chain transform (the wis/wane pattern). **d02xx family** (9 arcs × 5 = 40 quests) **✅ 40/40 COMPLETE**. Remaining Wave-1: ~9 singletons (`stoning_lystra`/`basket_damascus`, iodine chain `iodine_01`/`shore_02`/`forge_01`/`sunken_01`, Highland `df_02`/`sk_02`, folk `lxvii67`/`guide_04`). |
