...
| **1** | skill-check arcs **with** `onPass` closures (now migrating whole arcs incl. their `side` acts) | **78** onPass-closures (done: wane 6, whisper 5, glut 5, ceremonia_yael 5, 1367 skill 4, d02xx 40, inn 3, spark 5 = **~62 quests**) | full bit-chain transform (the wis/wane pattern). **d02xx family** (9 arcs × 5 = 40 quests) **✅ 40/40 COMPLETE**. Remaining Wave-1 arcs: `spark2`(5), `inquisitor`(3), `sea`, `sb`, `hunt`/`hunt2`, `bilge`, `alch`, `scar` + ~10 singletons. |
