...
roll2hit.com is one static HTML file. A reader who opens it finds `const S29_AUROS_THEORY@27050` — a
...
| 5 | `surveyDeliveredToAuros` → `undercitySurveyDelivered` | correction survives as engine comment `not surveyDeliveredToAuros@28094`; spec name has 1 occurrence, that comment | ✅ exact |
...
| 11 | FC06 `magicBonus = max(0, base − floor(random()×4))` | pool pre-filtered `magicBonus === 0`; `deg = Math.min(0, d6 - 5)@24592` → **−4…0** | ❌ different mechanism (Finding 5) |
| 12 | FC07 `FISHING_GUIDE_TEXT` + `quest_fishing_guide` | both live | ✅ |
| 13 | V-B: zone gating UI *"deferred"*, *"all three zones always accessible"* | shipped `59cc13b`, **10 h 47 m later** — `Unlocks after landing a Large fish@30474` | ❌ inverted (Finding 6) |
| 14 | `baitFishingActive` *"wired"* | **1 occurrence** — `tackleboxZoneUnlocks: {shore:true@23107` — declared, never read | ❌ → §DX-02n |
...
| 17 | Annotation target `froberger-journal-all-entries.txt` | file lives at `sources/…`; path unresolvable | ❌ path stale |
...
| 51 | Weimar Scholar Gate | 6 marker comments + `// Layer 51: Weimar Scholar Gate@23127` **inside `_S_DEFAULTS()`** |
...
| 59 | Pressure Cascade | `// ── Layer 59: Pressure Cascade ──@26933` — **and SP4's own Phase 2 annotated its const** |

Layer 59 is the sharpest instance: Phase 2 wrote `const NPC_VOID_PRESSURE_LINES@26934` pointing at
...
| Genuinely absent | 9 |
| Target file path wrong (`froberger-journal-all-entries.txt` → `sources/`) | 1 |
...
| | Report | HEAD (`_rollMonsterWeaponDrop(monsterDmgDie)@24581`) |
|---|--------|------|
| Pool | all weapons; degrade after selection | pre-filtered `w.magicBonus === 0` — a magic weapon is **never selected** |
| Formula | `max(0, base − floor(random()×4))` | `deg = Math.min(0, d6 - 5)@24592` |
...
| Rationale | protect the vendor economy from full-quality drops | `capped at base tier@24586` — *"fishing is the only source of +bonus weapons"* |
...
(`Unlocks after landing a Large fish@30474`), with auto-unlock driven by the catch log. §DOC-02j's
...
its own declaration at `tackleboxZoneUnlocks: {shore:true@23107`. Declared, never written, never read.
→ **§DX-02n**.
