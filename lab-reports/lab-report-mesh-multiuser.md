...
**Predecessors:** `lab-report-walk5-mud-harness.md` (§WALK-5 instanced encounters + `tests/mud-harness.mjs`) · `lab-report-cell-map-mud-redesign.md` §CELL-07 (SESSIONS store + SSE broadcast) · `lab-report-mesh-sync-architecture.md` (the seven-layer transport this report sits on top of)
...
**File relocations since ship** (both correct for their day, annotated not rewritten): `wbapi-server.js` → `js/wbapi-server.js` and `duel.js` → `js/duel.js`, moved by §CLEANUP-02 on 2026-07-14; the mesh kernel was extracted to `js/mesh.js` by `4367a38` nine hours after this report's last edit.
...
**Identity and compatibility** — `js/wbapi-server.js:const sha16 = (s) => crypto.createHash('sha256')@673` · `js/wbapi-server.js:const MANIFEST_PARTS = ['NODE_MAP', 'NODE_COORDS'@674` · `serverId` = 16 random bytes hex in `.wbapi-server-id` (live file: `6f139689a4f8ae48c07d88e6af4b2c2d`).

**Mesh constants** — `js/mesh.js:const MESH_GOSSIP_MS@116` (2 s) · `js/mesh.js:const MESH_ORIGIN_TTL = 90_000;@117` · `js/mesh.js:const MESH_FANOUT_MAX_AGE = 10_000;@118` · `js/mesh.js:if (MESH.log.length > 500)@148` (the bounded presence ring).

**Timers** — `js/wbapi-server.js:const SESSION_TTL = parseInt@76` (30 min) · `js/wbapi-server.js:const TRADE_TTL = parseInt@264` (60 s) · `js/wbapi-server.js:const DUEL_TTL = parseInt@591` (30 s per phase).
...
**Economy** — `js/wbapi-server.js:function playerKeyRegister(playerKey) {@290` · durable chain at `ledger/<serverId>.jsonl` · collision map persisted to `ledger/players.json`.

**Guards** — `js/wbapi-server.js:if (parts[0] === 'world' && parts[1] === 'download'@2921` · `js/wbapi-server.js:reason: 'cross-origin'@9688` (the v1 duel scope, enforced with a 400 rather than a comment).
...
| **d3** | `GET /api/world/download` + BIG WARNING modal + `scripts/world-diff.js` + magnet v2 | `8543e63` 07-02 10:38 | 1 h 32 m |
...
| `check:duelparity` | ✅ green — DUEL:CORE identical in `js/duel.js` and the game file (7,987 bytes) |
...
7. **`check:duelparity` is the one parity gate nothing runs.** §6.3 promised it would work *"exactly like `check:parity`/`check:roomsparity`"*. Those two are inside the 16-gate `check:walk` chain; `check:duelparity` is in neither `check:walk` nor CI, and `js/duel.js` is not in the CI trigger paths. It passes when invoked by hand — verified above — and nothing invokes it. Filed **§DX-02cr**.
...
- **§DX-02cr** 🟢 — `check:duelparity` runs in neither `check:walk` nor CI, and `js/duel.js` is not a CI trigger path. Two lines.
