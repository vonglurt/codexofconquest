...
| **i** ◐ | No-dupe ledger: mint-id on drops, per-player hash chain, trade handshake, fork-choice void. **Data shapes: §6.1–6.2.** **Slice 1 (single-server mint+trade) ✅ 2026-07-06:** persisted chains (`ledger/<origin>.jsonl`, fsync), `ledger/mint|owner|chain|status|ingest` + `trade/propose|accept|cancel`, pure resolver incl. lowest-hash dupe-void. **Remaining: slice 2** — the parallel durable gossip channel + anti-entropy (cross-mesh replication; `ingest` is already its receive half) + client S_story `mintId` stamping/trade UI + a durable player identity (see plan.md §MESH-01 ladder (i) note). | ✅ slice 1: mud-harness [I] 26 checks (dupe-void determinism incl. second-server verdict, honest-trade, provenance-reject, restart durability) · slice 2: cross-mesh dupe-void over real gossip |
