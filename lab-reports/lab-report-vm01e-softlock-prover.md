...
analysis, ported that one, and built `scripts/check-questgraph.js` — a reachability prover that executes the
...
**Scope:** (1) audit + port the surviving `_legacy_fn` bits; (2) build `scripts/check-questgraph.js` — the
...
- Extracted every `{ kind:'_legacy_fn', fn:… }` bit from the `QUEST_DB` region (`const QUEST_DB = {@10615` to
  `WORLDBUILDER:QUEST_DB:END@21951`; lines 10492–21692 on the ship-day build) with a string- and
...
- Cross-checked the vocabulary against the live kernel (`js/quest.js`, the `QUEST:CORE` fence). Twelve kinds,
...
`quest_1367_d_hansa: {@13960`, `quest_1367_f_plague: {@13964`, `quest_scar_03: {@14014` — are counted once.
...
| **FAVOR** | 2 | No | `_setNpcFavor(key, level)@23462` / `_checkDearFriendUpgrade(key)@23489` — the `favor` opcode already wraps `E.setFavor`. |
...
**Finding 0 — the blocker is one bit, and it is also a bug (P0).** `quest_1367_f_plague: {@13964`, `onFail`:
...
(*"Roll CON DC 13 or gain plague_exposed"*) and the `_S_DEFAULTS` declaration at `plague_exposed: false,@23290`
(*"Set on failed CON DC 13 save"*). The code rolls a modifier-free coin instead. `skill_check` already nests
through `js/quest.js:resolveSkillCheck(bit, ctx)@322` into `execBits`, synchronously, drawing the injected
seeded stream at `js/quest.js:const d20  = Math.ceil(E.rng() * 20)@314`. A one-line change with an outsized
...
|---|---:|---|---|
| 2 | 31 | Counter that mints a one-shot reward at a cap | `counter` gains `onReach:[bits]`, or `function _innKindness(n)@23529` and `function _addCroneMark()@23539` stay whitelisted host effects — their write-set is small and fixed |
...
**`scripts/check-questgraph.js` — the dynamic soft-lock prover.** Loads the live `QUEST_DB` headlessly
...
| `tests/integration/uqf-softlock.test.js` | 3/3 | 3/3 (also 3/3 at HEAD) | ✅ |
...
- **§DX-02dz** — `scripts/check-quest-parity.js:quest parity: QUEST:CORE identical@25` prints `a.length`
...
flags only) · §DX-02dx (D6, the dead require) · §DX-02dw (`const d20  = Math.ceil(E.rng() * 20)@22248` is the
...
`const QUEST_DB = {@10615` · `WORLDBUILDER:QUEST_DB:END@21951` · `quest_1367_f_plague: {@13964` ·
`quest_1367_d_hansa: {@13960` · `quest_scar_03: {@14014` · `plague_exposed: false,@23290` ·
`_legacy_fn(bit, ctx)@22327` · `_legacy_fn:  { required:@21999` ·
`const d20  = Math.ceil(E.rng() * 20)@22248` · `function _innKindness(n)@23529` ·
`function _addCroneMark()@23539` · `function _setNpcFavor(key, level)@23462` ·
`function _checkDearFriendUpgrade(key)@23489` · `const WM_ARCHIVE_DOCS = [@27787` ·
...
plague 13831 · handler js/quest.js:349 (HTML 22025) · contract js/quest.js:59 (HTML 21735) · helpers 23181 /
