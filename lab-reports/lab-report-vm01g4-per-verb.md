...
`NODE_PANELS` (`const NODE_PANELS@31285`, rendered by `_renderNodePanels(node, st)@31536`) and
`NODE_HOOKS` (`const NODE_HOOKS@33882`, dispatched in place by `_runNodeHook(id, node, ctx)@33940`).
...
the player's own prose — `S_story.entry42Text@34327` and `S_story._voidTollSecret@34439`. Both are
...
permanently dead (`S_story.actNumber = node.act || 1@34244`, assigned every render). Swept for the
same shape across G4's region: **one act comparison remains** —
`node.code === 'NUE' && (S_story.actNumber || 1) >= 3@34367` (Sweelinck's "map before the city"
...
`storyRender(node, prefix)@34238` · `_mkSection(id, icon, label)@34991` ·
`const NODE_PANELS@31285` · `_renderNodePanels(node, st)@31536` · `const NODE_HOOKS@33882` ·
`_runNodeHook(id, node, ctx)@33940` · `let _uqfPending@6824` · `function _uqfPump(gen, answer)@6827` ·
...
**Added by G4b (§12¾):** `const NODE_VERBS@33978` · `function _renderNodeVerbs(node, st, group, container)@34184` ·
