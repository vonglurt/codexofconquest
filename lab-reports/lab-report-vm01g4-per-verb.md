...
`NODE_PANELS` (`const NODE_PANELS@31285`, rendered by `_renderNodePanels(node, st)@31462`) and
`NODE_HOOKS` (`const NODE_HOOKS@33017`, dispatched in place by `_runNodeHook(id, node, ctx)@33059`).
...
the player's own prose — `S_story.entry42Text@33446` and `S_story._voidTollSecret@33558`. Both are
...
permanently dead (`S_story.actNumber = node.act || 1@33363`, assigned every render). Swept for the
same shape across G4's region: **one act comparison remains** —
`node.code === 'NUE' && (S_story.actNumber || 1) >= 3@33486` (Sweelinck's "map before the city"
...
`storyRender(node, prefix)@33357` · `_mkSection(id, icon, label)@34110` ·
`const NODE_PANELS@31285` · `_renderNodePanels(node, st)@31462` · `const NODE_HOOKS@33017` ·
`_runNodeHook(id, node, ctx)@33059` · `let _uqfPending@6824` · `function _uqfPump(gen, answer)@6827` ·
...
**Added by G4b (§12¾):** `const NODE_VERBS@33097` · `function _renderNodeVerbs(node, st, group, container)@33303` ·
