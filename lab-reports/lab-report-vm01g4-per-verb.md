...
`NODE_PANELS` (`const NODE_PANELS@31285`, rendered by `_renderNodePanels(node, st)@31492`) and
`NODE_HOOKS` (`const NODE_HOOKS@33171`, dispatched in place by `_runNodeHook(id, node, ctx)@33217`).
...
the player's own prose — `S_story.entry42Text@33604` and `S_story._voidTollSecret@33716`. Both are
...
permanently dead (`S_story.actNumber = node.act || 1@33521`, assigned every render). Swept for the
same shape across G4's region: **one act comparison remains** —
`node.code === 'NUE' && (S_story.actNumber || 1) >= 3@33644` (Sweelinck's "map before the city"
...
`storyRender(node, prefix)@33515` · `_mkSection(id, icon, label)@34268` ·
`const NODE_PANELS@31285` · `_renderNodePanels(node, st)@31492` · `const NODE_HOOKS@33171` ·
`_runNodeHook(id, node, ctx)@33217` · `let _uqfPending@6824` · `function _uqfPump(gen, answer)@6827` ·
...
**Added by G4b (§12¾):** `const NODE_VERBS@33255` · `function _renderNodeVerbs(node, st, group, container)@33461` ·
