...
`NODE_PANELS` (`const NODE_PANELS@31285`, rendered by `_renderNodePanels(node, st)@31515`) and
`NODE_HOOKS` (`const NODE_HOOKS@33455`, dispatched in place by `_runNodeHook(id, node, ctx)@33507`).
...
the player's own prose — `S_story.entry42Text@33894` and `S_story._voidTollSecret@34006`. Both are
...
permanently dead (`S_story.actNumber = node.act || 1@33811`, assigned every render). Swept for the
same shape across G4's region: **one act comparison remains** —
`node.code === 'NUE' && (S_story.actNumber || 1) >= 3@33934` (Sweelinck's "map before the city"
...
`storyRender(node, prefix)@33805` · `_mkSection(id, icon, label)@34558` ·
`const NODE_PANELS@31285` · `_renderNodePanels(node, st)@31515` · `const NODE_HOOKS@33455` ·
`_runNodeHook(id, node, ctx)@33507` · `let _uqfPending@6824` · `function _uqfPump(gen, answer)@6827` ·
...
**Added by G4b (§12¾):** `const NODE_VERBS@33545` · `function _renderNodeVerbs(node, st, group, container)@33751` ·
