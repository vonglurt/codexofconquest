...
> `wbapi-server.js`, not `js/wbapi-server.js`. A wrong path yields a 0-byte file and a census of
...
`ACT→BGD` are three of the eight literal entries in `PRIORITY_HIGHWAYS` (`api/wb.js:1070–1082` at
...
`POST /api/graph/reweave-all` returns **410** (`js/wbapi-server.js:reweave-all is deprecated@6912`);
...
word in a tombstone comment (`js/wbapi-server.js:Historically this reused buildHighway@6993`), and that
...
(`js/wbapi-server.js:const cbAdj = new Map()@6930`) builds its adjacency from `node[d]` for
...
It is published as a green CLI verb under *"§22 NETWORK HEALTH & REPAIR"* (`api/wb.js:3153`,
...
cell-touching BFS (`js/wbapi-server.js:function degree(code)@5385`); it also disagrees with itself
