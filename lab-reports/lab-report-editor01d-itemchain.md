...
> are `` `symbol@line` `` anchors (§DX-01e). This file is HISTORY in `scripts/legacy-codes.js` —
...
| 9 | `scripts/check-itemchain.js` + `npm run check:itemchain` + CI `paths:` | **SHIPPED; 29/29 green today**, ten weeks on | `.github/workflows/walk-invariants.yml:32,51,80` |
| 9 | `tests/integration/worldbuilder-itemchain.test.js` | **NOT SHIPPED.** `git log -S` finds exactly one commit naming it — this report. Coverage landed in the *existing* `worldbuilder-quest-editor` / `worldbuilder-crud-arrays` specs, plus a later `worldbuilder-chain-editor` | instrument 4 |
...
`lab-reports/lab-report-editor01d-fu-chain-ui.md:5` — all traceable to this report. Three
independent sources say **61**: the FU(b) design lock, `scripts/check-ladder-migration.js:5`, and
...
> `ANCHOR_RE` (`scripts/resolve-anchors.js`) is ``/`([^`\n]{1,80}?)@(\d{2,6})`/`` — the line number
> needs **at least two digits**. A first draft here pointed at
> `scripts/check-ladder-migration.js` line **5**; it rendered as a plain code span, resolved
