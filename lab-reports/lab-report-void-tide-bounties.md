...
`const VOID_TIDE_EVENTS@22368` is a table of seven day-keyed dread beats (days 3, 7, 14, 21, 28, 35,
42) fired on sleep by `function storyCheckVoidTide()@36358`, reached from
`function storyConfirmSleep()@36244`. Beat **#21** narrates a Warrant bounty the engine never
modelled:
...
|---|---|---|---|
| `VOID_TIDE_FEATURED` | const map, tideDay → `{quest, standing, rumor}` | `const VOID_TIDE_FEATURED@37104` | the featured roster, per-quest standing bonus, dread hook |
| `VOID_FEATURED_IDS` | derived `Set` | `const VOID_FEATURED_IDS@37109` | ids excluded from the normal rotation pool |
| `quest_void_tide_21/35/42` | 3 UQF `combat` quests | `quest_void_tide_21@21308` · `quest_void_tide_35@21311` · `quest_void_tide_42@21314` | the completable featured bounties |
| `_bountyPostable(q,node)` | pure predicate | `function _bountyPostable(q, node)@37149` | shared legality gate; deliberately excludes the FU7 reward ceiling |
| `_voidFeatured(node)` | pure selector | `function _voidFeatured(node)@37163` | the active featured bounty, or `null` |
| `_voidBonus(id)` | pure lookup | `function _voidBonus(id)@37111` | escalated standing for a Void bounty, else 1 |
...
`dayMax` is **exclusive** — `if (g.dayMax != null && d >= g.dayMax) return false;@22097` in
`function _matchActivationLeaf(g, st)@22048` — so the migration is behaviour-preserving to the day.
...
`function storyCheckQuests(node)@30166`, and `_bountyPostable`. What falls out for free:
...
two honest ways: the quest's own `reward` XP bit, read by the existing `function _boardReward@37053`
path (never fake gold, per FU1), and the escalated standing applied at
`function _creditWarrant(id)@37117`. Standing is host bookkeeping, not an economy change.
...
*Unknown* (`rewardCap: 250@37075`, slate 4) and the day-42 pin still renders **⭐320 xp** as a fifth
...
- `function _boardBounties(node, limit)@37183`: (a) the loop's legality checks were factored into
  `_bountyPostable`; (b) one skip added — `if (VOID_FEATURED_IDS.has(q.id)) continue;@37191`; (c)
  after distance labelling, `const vf = _voidFeatured(node);@37217` and `shown.unshift(vf);@37221`.
  The pin is an **extra** row, unshifted past the slate slice, always first. Pure and read-only.
- Render, at `lbl: b.void ?@35707` inside `function storyRender(node, prefix)@34567`: the card
...
| Data-driven, no per-node `if` — the pin rides the node-flag path via `function _boardHost(node)@37038` | ✅ held |
...
`tests/integration/warrants-board.test.js`, and every one of the six assertions is present:
...
`scripts/check-questgraph.js`, the soft-lock prover, was born `354b20a` on 2026-07-22 — **one day
...
`scripts/check-questgraph.js:284` names the leaf explicitly — *"dayMin·dayMax — monotone-satisfiable
...
site — `S_story.day = Math.min(49, S_story.day + 1)@36270`, inside `storyConfirmSleep` — a retry
...
`active`, and `onFail` is empty. The only movement is §XP-01 effort XP — `const EFFORT_XP_PCT@24426`
= 0.25 of the pass reward, paid **once per quest** (80 of the 320; a second failure pays nothing).
The measured delta was 90, of which the remaining 10 is `const EXPLORE_XP@24435` for first arrival at
...
- **The engine comment this increment shipped carries a line number that was wrong the day it was
  written.** `beats at 21821@37098` points at `VOID_TIDE_EVENTS`, which sits at **21822** in the very
  build that comment shipped in, and at **22368** today. Nothing can catch it: the anchor gate walks
  `*.md` only (`scripts/resolve-anchors.js:65`). Filed as **§DX-02ef**.
...
since the initial commit — `dayEl.className@36105` — so FU8's second and third windows open on
...
cd shipworld && npx playwright test tests/integration/warrants-board.test.js        # 21/21
npx playwright test tests/integration/quest-runtime-uqf.test.js -g "type:combat"    # RED at ship
