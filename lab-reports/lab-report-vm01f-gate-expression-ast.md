...
| ~~`_activateIndex` (Map), host, lazy~~ | — | **NOT SHIPPED.** Built and reverted this increment (§4.5). The name never existed in code; the follow-on shipped it as `function _questsByNode(nodeCode)@37015`. |
...
**Verified byte-identical to `function _compileGate(node, mode)@22156` at the ship commit and at HEAD** — 31 days and one migration front later, not a character has moved.
...
Both lines verified byte-exact against `completion:{ any:[ { flagsAny:['archiveLetterObtained']@11071` (ship and HEAD) and against the parent build's pre-F line. The fallback path is `q.waypointNode || (q.completion && q.completion.atNode)@28802`, unchanged.
...
Memoised lazily, keyed by the immutable gate object. This keeps the compiler **inside the parity-fenced kernel** and matches the file's own lazy-built-once idiom (`function _questNodes()@36995`, `let _gateFlagCache = null@26132`). An eager "walk `QUEST_DB` at load" pass would have to live in the host and either call into the kernel or duplicate compile logic. Runtime effect is identical. **Rejected axes:** interpret-per-render; eager host pass.
...
| Symbol | `js/quest.js` | `roll2hit-v3.html` (parent) | verdict |
|---|---|---|---|
...
At HEAD the same symbols live at `function _matchActivationLeaf(g, st)@22048`, `function _matchCompletionLeaf(g, st)@22107`, `function _gatePred(node, mode)@22167`, `const _gateCache = new WeakMap()@22166`, `function _gateFlagSet()@26133`.
...
- **Parity fence.** Kernel edits are inside `QUEST:CORE`; `js/quest.js` is edited then re-inlined byte-for-byte. ✅ **`check:questparity` identical at ship** — and only `js/quest.js` is touched among the four kernels, so `MOVER`/`ROOMS`/`DUEL:CORE` are untouched by construction. All four fences verified green on the rebuilt ship-day tree.
...
- **Known latent (documented, not triggered).** `function _gateFlagSet()@26133` structurally walks `q.gate.flags/flagsAny/notFlags` and would not see flags nested under an AST *activation* gate. F introduces none, so it is not triggered; when activation gates adopt the AST, that walker must recurse.
...
**Invariants:** all four parity fences green with 0 sentinels changed · `check:rng` green · `node --check js/quest.js` clean · only the two pre-existing `check:walk` baselines red.
...
**They are not an OR shape — they are a comparison shape, and `{all}`/`{any}`/`{not}` cannot make one.** The combinators *compose* predicates; they cannot *create* one. So the term-per-quest pressure CONTRIBUTING diagnosed did not stop; it moved one axis over, from OR-position to comparison-position, where the grammar still has nothing to say. The evidence that this is systemic rather than a single lapse is `is not expressible in canActivate@13973`, still in the file: *"lxvii67's `faith_folk>=1` gate is not expressible in `canActivate`'s term set ⇒ `gate:{_legacyFn:true}`"* — and 14 quests still ride that escape hatch, unchanged for a month. `dayMin`/`dayMax` is a `counterMin` for one hard-coded counter. **This measurement is appended to §DX-02dy**, whose proposed fix is the general leaf; two special-cased comparison terms shipping within a day of the grammar landing is the strongest available argument for it.
...
| **§DX-02ed** | 🟢 NEW | `function _gateFlagSet()@26133` does not recurse `{all}`/`{any}`/`{not}`; latent since 2026-07-22, named here as a follow-on and never filed (§8) |
