...
1. **The three parity-fenced kernels** — `MOVER:CORE:START@9914` · `ROOMS:CORE:START@9985` · `DUEL:CORE:START@10238`. Pure, world-injected, byte-identical to `js/{mover,rooms,duel}.js`, asserted by `scripts/check-*-parity.js`. They take a `world` and return plain data; they never read `S_story` and never touch the DOM.
...
**Every pointer from a doc into the code is written as one token: `` `symbol@1234` ``.** The **symbol** is the pointer — any literal substring of the line you mean (`` `const WORLD_DB@6279` ``, `` `execBits(pass ?@22259` ``, or file-qualified `` `js/wbapi-server.js:function seededNext@1147` ``). The **number is a cached hint**, refreshed mechanically.
