...
| `scripts/legacy-codes.js` | **`check:walk` gate #16 (§AUDIT-03m)** — the prose half of §AUDIT-03l: 26×16-era node codes still in running sentences. Fails on a bare legacy code in a **SWEEP** doc, and on a doc carrying legacy codes classified in neither SWEEP, PENDING nor HISTORY (explicit classification, #13/#14 style). **HISTORY docs are annotated, never rewritten** — the tool refuses `--annotate` on one. `npm run legacy` reports · `--annotate <file>` sweeps one. Sparse lines get `` `LHR` (historical `CI`) ``; a run of 3+ codes gets live codes plus one trailing key | ✅ Added 2026-08-04 |
