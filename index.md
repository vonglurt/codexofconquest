...
| **§WALK-5 (lab report + Inc 1–2)** | 2026-06-26 | MUD multi-client harness — **DESIGN LOCKED**; **Inc 1+2 done**. Lab report locks instanced per-session encounters, per-session seeded RNG, K-client no-bleed proof; reconciles the parent §7 sketch with current code (no `huntMode`; 409 block path; encounter at `s.encounter` not `s.state`); ferry deferred → §WALK-5-FU; flat tier weights (no server notoriety). **Inc 1** wired `terrainAt`/`encounterRate`/`getSeaLanes()` into `getMoverWorld()` + `scripts/check-terrain-parity.js` in `check:walk` (server↔client parity, 10440/10440 band cells). **Inc 2** added per-session RNG (`s.seed`/`s.rngState`) + the instanced encounter roll on `session/move` (`s.encounter`, surfaced on move response + `who`; `pickMonster` flat base-tier weights); verified live determinism + per-seed divergence (32/40 fire, 31 distinct traces). Next: Inc 3 `tests/mud-harness.*`. Lab report: `lab-reports/lab-report-walk5-mud-harness.md`. |
