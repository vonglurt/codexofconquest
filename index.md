...
| **§NAV-01** | Navigable World: MUD-coherent map + fungal road net — **IN PROGRESS: Inc a–f ✅** (pos-origin BFS, `ROAD_RUNS` net 400 cells / 88 junctions, `describeCell` room layer, road-weighted auto-travel, wayfinding UI, §NAV-01f MUD server room parity — `room` on all session look surfaces, byte-equal to client `describeCell`, mud-harness [M]) + map suite + GLOBE panel. Remaining: Inc g/h worldbuilder drag-&-lock + road-net editor. Spec: `plan.md §NAV-01`. |
