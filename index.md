...
| **§MESH-01** | Multiuser MUD — **core ✅ SHIPPED** (client presence, gossip/PEX mesh + `worldHash` scoping, tracker + federation, world download/diff, Mesh tab; archived 2026-07-03) · **gameplay ladder (f) buffs ✅ · (g) hireling ✅ · (h) sentry bots ✅ · (i) no-dupe ledger ✅ slices 1+2a+2b 2026-07-06** (durable hash chains `ledger/<origin>.jsonl`, mint + two-phase trade + pure lowest-hash dupe-void resolver; durable playerKey identity + cross-mesh gossip replication; client mint-stamping + ⇄ trade UI — mud-harness [I]/[I2] + `mesh-ledger-client.test.js` 7). Remaining: (i) cross-ORIGIN co-signed trades · (j) consensual duels (`DUEL:CORE` shapes ready — lab report §6.3). See plan.md §MESH-01 ladder. |
