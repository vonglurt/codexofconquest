...
| **§EDITOR-01-D core** | 2026-06-27 | Declarative `itemChain` quest field (Inc 1–4): `_applyItemChain` runtime (grant/take/grantBit/takeBit) hooked into both completion paths; Quest Creator `#ed-itemChain` textarea + shared `parseItemChainText`/`itemChainToText` codec; CRUD form `arr:'itemchain'`; `npm run check:itemchain` (19 checks). FU: visual chain UI + 58-branch reward-ladder migration |
