...
| **§WALK-5 (lab report + Inc 1–3)** | 2026-06-26 | MUD multi-client harness — **DESIGN LOCKED**; **Inc 1+2+3 done**. Lab report locks instanced per-session encounters, per-session seeded RNG, K-client no-bleed proof; reconciles the parent §7 sketch with current code (no `huntMode`; 409 block path; encounter at `s.encounter` not `s.state`); ferry deferred → §WALK-5-FU; flat tier weights (no server notoriety). **Inc 1** wired `terrainAt`/`encounterRate`/`getSeaLanes()` into `getMoverWorld()` + `scripts/check-terrain-parity.js` in `check:walk` (server↔client parity, 10440/10440 band cells). **Inc 2** added per-session RNG + the instanced encounter roll on `session/move` (`s.encounter`, surfaced on move response + `who`; flat base-tier weights); verified live determinism + per-seed divergence. **Inc 3** built `tests/mud-harness.mjs` (`npm run test:mud`) — HTTP+SSE K-client driver, 18/18 (co-presence + instancing/no-bleed + who/look); caught + fixed a `session/say` double-send bug. Next: Inc 4 (TTL-prune assert + CI wiring + doc-sync). Lab report: `lab-reports/lab-report-walk5-mud-harness.md`. |
