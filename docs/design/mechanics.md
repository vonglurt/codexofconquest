...
| `EFFORT_MISS_PCT` | `0.02` | each **missed attack** grants `round(AC·maxHp·EFFORT_MISS_PCT)`, banked silently (no mid-fight level-up modal). Paid on **both** counted attack surfaces since §DX-02df — `_overlayPlayerAttack` and the off-hand bonus action `_overlayOffhandAttack`, which tallied `attacksAttempted` and paid nothing. The mutual-flee free swing is neither tallied nor paid, and is the open call §XP-FLEE |
