...
Two outcomes: combat (fight `void_shaman` AC15/HP65) or persuasion (show *The Constructor's Log* — `readableKey:'constructors_log'` — from §XVII; Warden reads Entry 2 + Entry 7 and understands). Either way: `wardensLegacyKnown` set, Hollow Hands resolved, `warden_token` relic obtained.
