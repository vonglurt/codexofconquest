...
> The two derived cells, *HTML line count* and *Lab reports on disk*, are asserted by `check:badge` (gate #37): the increment that moves one restates it and names itself in the Status cell. Confirm FC item status by hand.
...
| `src/scripts/check-badge.js` | **`check:walk` gate #37 (§DX-02hz)** — the Doc Health Badge's two derived cells are held to the tree: *HTML line count* to `wc -l play.html` and *Lab reports on disk* to `ls docs/lab-reports/*.md \| wc -l`. A missing row or a non-numeric value is a finding, not a skip. The line count had fallen 47 behind across fifteen increments before the gate existed, and the Status cell's attribution chain is only worth keeping if each increment that moves the count restates it. Asserts only, never rewrites (§DX-02fx). `node src/scripts/check-badge.js --selftest` | ✅ Added 2026-09-25 |
