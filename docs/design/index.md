...
| Metric | Value | Status |
|--------|-------|--------|
| HTML line count | 38,985 | ✅ 2026-09-06 (`wc -l play.html`) — **+7 from §DX-02dl** (the `>=` milestone's four-line note, the value-true margin clause, less the line the Ceremonia direct write no longer needs). Prior: **+9 from §DX-02dg/dh** (`_dayAlarm` and its two threshold constants with the comment that says why the counter alarms at all, plus `courierMapSeen` moved from the seed literal into `_S_DEFAULTS()` with the §STATE-INIT note; the two ladder expressions it replaced were one-liners). Prior: **+8 from §DX-02iu** (the `itemsAll` term in `_matchActivationLeaf`, spliced into the QUEST:CORE fence from `src/js/quest.js`). Prior: **±0 from §DX-02dy** (eight gates migrated in place through `./bin/api`: 15 lines changed, none added or removed). Prior: **+9 from §DX-02eg** (the hoisted `const rewardXp`, the `_premium` field, the premium `seat()` call and three comment lines). Prior: **+48 from §DX-02ei** (`_roadGridWalk` extracted from `_roadGridPathCore` so the search has one relaxation and two callers, `_roadGridNearest` and its comment, `BOARD_LOCAL_PLACES` with the note on why the reserved seat does not scale with the slate, and the reserve-then-fill block in `_boardBounties`). Prior: **+13 from §DX-02eq** (`_isExhausted`, the Recompose decision inside `_applyPitPerks`, the per-encounter latch, the rest reset and the pre-battle line, less the four retired writes). Prior: **+13 from §DX-02ex** (the `couperiDebtReleased` default, the debt line's two branches, and `_splitCouperiDebtMeaning` with its two call sites). Prior: **+2 from §DX-02ew** (the ledger-ceiling invariant comment). Prior: **+30 from §DX-02et** (the world-note writer, the save migration and the panel section). Prior: **−13 from §DX-02fv** (the TL Vonn beat became a `NODE_VERBS` entry). Prior: **+14 from §DX-02aq** (the knowledge row builder and the Field Notes section). Prior: **+33 from §DX-02bt** (two quest cards rebuilt on DOM construction, two map comments). Prior: **±0 from §DX-02do**, **+2 from §DX-02ae** (the `onDeliver` hook and its call at the farewell delivery site). Prior: **±0 from §DX-02ac**, **−1 from §DX-02ai** (the hull button's inline XP grant). Prior: **−1 from §CHRON-01** (two NG+ preserve lines in, one tally and two print rows out). Prior: **+1 from §DX-02cl** (`_questNodeSet = null` in `_questsByNodeRevalidate`). Prior: **−1 from §DX-02dp** (two sibling-sweep loops and their comments replaced by `_clearStoryTransient()` and one markup comment). Prior: **+5 from §DX-02dw** (the `effects.rng` requirement and its four-line comment, re-inlined from `quest.js`). Prior: **+14 from §DX-02ea** (`_questsByNodeRevalidate` extracted from `_questsByNode`, `_questsByNodeUnchecked` added, one call at the top of `storyRender`, six comment lines). Prior: **+1 from §DX-02ej** (`LHR` gained `board:true` on its own line, as `put node` serialises it). Prior: **+9 from §AUDIT-03bn** (+2: `fiend_beast` gained a `voidTainted` field on its own line, as `put monster` serialises it; +6: `_isVoidEnemy` reads the field, then the fish rule, before the regex; +1: the §PLAY-01-B header comment names the field). **⚠️ the file measured 38,783 before this row touched it, against a badge of 38,767 — the +16 is §AUDIT-03bo's `watcher_gvw` dialogue entry, which shipped without moving the badge.** Prior: **−6 from §DROP-01-FU** (−8: the 20-entry `LOOT_TABLE` and the blank line it left; +2: two lines added to the header `serializeD100Table()` emits above `_D100_TABLE`). Prior: **+1 from §DOCPTR-01** (the Corelli `→ doc:` comment carried its section name and a sentence of prose on one line, which is unparseable; the prose moved to its own line). **⚠️ the file measured 38,772 before this row touched it, against a badge of 38,703 — 69 more unattributed, on top of the 9 §DX-02hz already owns.** Prior: **+1 from §DX-02en** (`standingSpoken`, the line all four endings append; the two threshold edits and the label brackets are in place). **⚠️ +9 unattributed between 2026-08-24 and this recount** — the badge read 38,693 and the file measured 38,702 before this row touched it, so at least one increment since §DX-02gk shipped without updating it; §2.3a says the badge moves in the same commit that invalidates it. Filed as §DX-02hz. Prior: — −7 from §AUDIT-03bh-FU (the STN Harbor Board's inventory push replaced by one `tlManifestFound` write); +2 from §AUDIT-03bk (`departedNodes` in `_S_DEFAULTS` + the `storyMove` departure record, one `innDeparted` decl out); ±0 from §AUDIT-03bj (53 gate/bit edits, all in place through `./bin/api`); ±0 from §AUDIT-03bh (four `completion` objects rewritten in place); ±0 from §DX-02gl (one act-table entry out, one comment line in); +12 from §DX-02gk: the ending scorer reads `DEAR_FRIEND_BITS` instead of respelling its six acts (`MISSION_ACT_BITS` + `_missionBits` cost more lines than the six predicates saved). Prior: −9 §DX-02gb, +1 §DX-02aj, +1 §DX-02gd, −19 §DX-02cm |
...
| `_addVoidPressure` | The Void meter's intended writer: clamps at 10, latches `voidCrackFired` (≥3), `voidFracturesFired` (≥6) and `voidImminentWarned` (**≥9 since §DX-02dl** — it was `=== 9`, which a `+3` writer steps straight over), then refreshes the HUD. Its warning reports the **live** pressure, because the band is 9 *or* 10. **Direct writes to `S_story.voidPressure` are down to 3** — the clamp itself plus two `_legacy_fn` bit bodies in `QUEST_DB` that the API cannot write (**§DX-02iv**, the row §DX-02dl is blocked behind). `grep -c 'S_story.voidPressure *=' play.html` is the census; the target is **1**. **No defeat check lives here** — whether reaching 10 ends the run at the write is §DX-02dk |
...
| `BOUNTY_TYPES` / `_boardHost` | Warrant's Board (§BOARD-01). `_boardHost(node)` is `board === true`, else `sleep === true` unless `board === false`; `LHR` is the one `board:true` author (§DX-02ej, via `./bin/api put node LHR board=true`). `BOUNTY_TYPES` allowlists `side · skill_check · hybrid · combat · hunt · delivery · escort · dialogue`; `craft` was an item type and is out; `hunt` is declared with 0 quests (§DX-02if). Eligible = UQF ∧ allowlisted = **2,776 of 2,823** at 2026-09-03, pinned by `warrants-board.test.js` |
| `voidTainted` / `_VOID_ENEMY_RE` | Press-vs-flee at ≤30 % HP (§PLAY-01-B). `_isVoidEnemy` reads the monster's `voidTainted` boolean first (3 bearers: `void_wolf`, `void_rat_swarm`, `fiend_beast:false`), then treats `fish_*`/`night_*` keys as beasts, then tests the name against `_VOID_ENEMY_RE`; 52 of 399 tagged at 2026-09-03 (§AUDIT-03bn), pinned by `enemy-ai.test.js`. The terrain half `_VOID_TERRAIN_RE` matches no live terrain (§DX-02di, open) |
