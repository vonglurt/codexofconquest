...
| `src/scripts/check-comment-codes.js` | **`check:walk` gate #39 (§AUDIT-03ba)** — a comment in `play.html`'s script that names a place by an uppercase code must name a live `NODE_MAP` key. Every other node-code scanner strips comments on purpose (§AUDIT-03f), and documentation authors read the world from exactly those comments: two lab reports published `CO` as a live location because the comment above a `TLS` guard said so. A token counts only in a place context (`at XX`, `XX node`, `Node XX`, `(XX —`, `(XX)`, `visit XX`, `XX and YY nodes`), so `IN PLACE` and `CO-001` are not hits. An unresolved token must sit in `NOT_A_NODE` (a word, with its reason) or `PENDING` (a dead code with its exact count and owning row). Both tables are ratchets that fail when stale. Asserts only (§DX-02fx). `node src/scripts/check-comment-codes.js --selftest` | ✅ Added 2026-09-25 |
...
| `docs/story/story-arc-investigation.md` | §XVI Weimar Scholar Gate + §XVII Void Archaeology + §XXI Void Shaman chain | `NUE` (historical `SQ`) · `GVA` (historical `MT`) · CI |
| `docs/story/story-arc-coastal.md` | §XIX Tilbury Harbor + §XX Visby Underground | `LCY` · `STN` · `TRD` |  *(historical: `DK`=`LCY` · `SF`=`STN` · `GC`=`TRD`)*
| `docs/story/story-arc-ngplus.md` | §XV NG+ Remembrance · Entry 42 · quest_ng_01/02/03 | LHR · NUE · TLS (historical CI · `NUE` (historical `SQ`) · `TLS` (historical `CO`)) |
| `docs/story/story-arc-npc-dialogues.md` | Birka Six NPC_DIALOGUES full transcript (120 quotes) · arc summary | `LHR` · `TLL` · `MHQ` · `LLA` · `HKG` |  *(historical: `CI`=`LHR` · `IN`=`TLL` · `TV`=`MHQ` · `BA`=`LLA` · `CY`=`HKG`)*
...
| **§1367** | Historical year 1367 AD integration — `GAME_YEAR=1367`, plague mechanic, Hanseatic faction score, faith triple-track (orthodox/reform/folk), 4 new Baltic nodes (written `LB`/`DZ`/`RG`/`BG` then; the live Baltic nodes are `LBC` Lübeck, `GDN` Danzig and `RIX` Riga), 6 arc seeds, historical NPCs. Full spec in `docs/notes/Year1367AD.md`. |
...
| **§NAV-01** | Navigable World: MUD-coherent map + fungal road net — **✅ COMPLETE 2026-07-03 (Inc a–h)**: pos-origin BFS (geo bounds + wrap), `ROAD_RUNS` net (410 cells / 89 junctions since §DX-01a's 2026-07-28 reweave, encounter rate 0, `check:roads` R1–R4), `describeCell` room layer (ROOMS:CORE `src/js/rooms.js`, deterministic prose + signposts), road-weighted auto-travel (`_roadGridPath` + `_travelTick`, 4 interrupt classes), wayfinding UI (exits signage, waypoint ★, `(n steps, NE)`), map suite + GLOBE panel, Inc f MUD server room parity (`room` on all four look surfaces, byte-equal to client, mud-harness [M]), Inc g worldbuilder drag-&-lock cities (`PUT /api/coords`, 🔒 → `src/config/roads-pins.json`), Inc h road-net editor (chain-link overlay, pin drag, ✚/┬/🔗/🗑 palette, ♻ Reweave Net = `PUT /api/roads` with auto-rollback). Gates at close: walk suite 89/89 · check:walk 6/6 · mud-harness 119. Lab report: `docs/lab-reports/lab-report-nav01-navigable-world.md` · archive: plan-archive.md §2026-07-03. |
...
| **§MESH-02** | Map-Tab Connection Center — multiplayer menus as Map-sheet sub-tabs (🗺 Local · 🌍 World · 🛰 Full · 🌐 Multiplayer · 🔭 Discover · 🛡 Lists). **(a) ACL/blocklist endpoints ✅ · (b) sub-tab shell ✅ · (c) Connect pane ✅ · (d) Discover pane (local scan D6, find, list sources, D4 whitelist gating) ✅ · (e) Lists pane (client black/whitelist, server ACL editor, D2/D3 peer-blocklist preview→explicit merge) ✅ · (f) committed tests ✅ 2026-07-07 (mud-harness [R] + hermetic `mesh-connections-ui.test.js` 8) · (h) 💬 multi-user chat history ✅ · (i) runtime `POST /api/mesh/connect` ✅ · (j) 👣 footprints ✅ · (g) docs + CLI parity ✅ `676dba7` — complete (a)–(j).** CLI: `./bin/api mesh acl\|blocklist\|connect`. Design: `docs/lab-reports/lab-report-mesh02-connections-ui.md`; player view: `mechanics.md §Multiplayer` "Connection center"; operator view: `docs/notes/docs-node-network.md §12` + `docs/api/wbapi-help.md §Mesh API`. |
...
| `docs/lab-reports/lab-report-epic-battlegrounds.md` | 39 | §0 20 EB dead-end nodes, `EB_NPC_DIALOGUE`, payment negotiation, return beats — **§DOC-02l verified 2026-08-12**: 20/20 statlines byte-exact, historical node codes `EF`…`EG` → `PRN`…`TBS`; see §EPIC-01 (40 quests orphaned by the rename) |
...
| `docs/lab-reports/lab-report-dungeon-ten-themes.md` | 80 | §DUNGEON-01 — 10 dungeon themes, P1–P3+ tiers, Node `LIM` (historical `MM`), Tribble counter, Madness Table, voidFluxActive, Prior Carrier, Codex Core ending compat |
...
| `docs/lab-reports/lab-report-world-mesh-construction.md` | World mesh construction — 240/597 → 435/597 nodes reachable, all 29 major GEO-referenced cities connected, using `./bin/api` alone |
...
| **Corinth / Korath (`ZTH` (historical `KR`))** | `story.md Layer 104a` | `docs/lab-reports/lab-report-saul-paul-travel-reference.md` |
...
|------|---------------|-----|-----|-----------------|----------|
| **Birka** | — (no hub node) | I + VIII | `TLL` 🛏 | `LHR` · `BMA` · `CDG` · `MHQ` · `LLA` · `KRN` · `HKG` | Commander Bruhns |
...
| `BOSS_COMMANDER_AUROS` | AC22/HP300/ATK+12/3d8+6; final boss at `TLS` (historical `CO`) node; requires Lv20 + 7 shards |
| `VENDOR_NODES` | Set of node codes with vendor access (5 nodes: `LLA`/`LGW`/`STN`/`PDL`/`VBY`; until §DX-02ll the fifth was the historical `BK`, which is also the live Birka Shore, so the Visby vendor's shop opened there) |
...
| `S_story.frobergerLetterFound` | boolean | Froberger's sealed letter at `TLS` (historical `CO`) found |
...
| `S_story.vshamanFound` | boolean | Warden located at `GVA` (historical `MT`) |
...
| `S_story.patrolBA` | boolean | Player visited `LHR` (historical `CI`) while `quest_city_watch_patrol` active |
| `S_story.patrolIN` | boolean | Player visited `TLL` (historical `IN`) after `LHR` (historical `CI`) while `quest_city_watch_patrol` active |
| `S_story.patrolRouteComplete` | boolean | Full patrol route `LHR` → `TLL` → `MHQ` completed (the flags keep the historical codes `BA` and `IN`, and `TA` was never a node); triggers quest completion |
...
**Status (2026-06-05):** source books `MQ`/`SQ`/`EPIC` done (book codes, not the historical node codes they share letters with). §IMPORT-102 RIX complete (449 nodes, 1695 quests). See `docs/api/api-data-audit.md §Per-Book Queue`.
