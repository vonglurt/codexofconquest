...
| Metric | Value | Status |
|--------|-------|--------|
| HTML line count | 38,981 | ✅ 2026-09-06 (`wc -l play.html`) — **−4 from §DX-02dl increment 2** (two `bits` arrays re-emitted as one line each by the API's serializer, three lines to one, twice). Prior: **+7 from §DX-02dl increment 1** (the `>=` milestone's note and the value-true margin clause, less the line the Ceremonia direct write no longer needs) |
