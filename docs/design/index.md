...
| **Warrant's Board (bounty slate)** | `docs/lab-reports/lab-report-warrants-board.md` | `src/tests/integration/warrants-board.test.js` · hosts: `_boardHost` reads `NODE_MAP[code].board` (1 author, `LHR`, §DX-02ej) else `sleep:true` (38 nodes) · eligible 2,776 of 2,823 UQF (`BOUNTY_TYPES`) |
...
| `WARRANT_TIERS` / `_warrantTier` | Warrant standing ladder (§BOARD-01-FU7): five rungs at min 0 · 3 · 7 · 12 · 20 with slates **4 · 5 · 6 · 7 · 8** (§DX-02eh made every rung widen the board) and reward caps 250 · 350 · 500 · ∞ · ∞; a const table, never serialized; `warrants-board.test.js` pins the slates strictly increasing |
...
| `S_story.worldLog` | array | Layer 44 §III's record of what the world did while you were elsewhere — `{text, day}`, newest first, written by `` `function _pushWorldNote@27705` `` and rendered by `storyRenderInventory` under `.journal-entry.world`. **Deliberately passive:** nothing speaks a note through `storyMsg`, per the design's *"the player may miss these entirely if they don't check"* |
