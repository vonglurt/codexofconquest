...
| `VENDOR_NODES` | Set of node codes with vendor access (5 nodes, as the engine spells them: `LLA`/`LGW`/`STN`/`PDL`/`BK`; `BK` is both a live node, Birka Shore, and the historical code for `VBY`, and which one the set means is §DX-02ll) |
