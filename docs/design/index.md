...
│   │                       a ratchet that only goes down (§DX-02dy: 14 → 6, survivors named)
...
| `LEGACY_GATE_CEILING` / `gate:{_legacyFn:true}` | **6** — quests whose activation condition is a closure the gate grammar cannot express, so `check-questgraph.js`'s `gateSat` reads them as unconditionally satisfiable and the prover is blind to the condition. Sound (it over-approximates with the monotone bound, so no *unreachable* verdict is wrong) but invisible until §DX-02dy gave it a reported census and a **down-only ratchet**. **14 → 6:** six closures were `() => true`, two were counter comparisons `countMin` already expressed. The six survivors are all inventory tests the activation leaf has no term for — §DX-02iu. Lower the ceiling with the row that lowers the count |
