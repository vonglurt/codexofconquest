...
grep -n "yourIdentifier" index.html
```
...
New content is **data added to the big collections** in `index.html`: `NODE_MAP` (places), `QUEST_DB` (missions, UQF-1.0), `MONSTER_POOL` (encounters), `WORLD_DB` (terrains), `NPC_DIALOGUES` (talk). The pattern is always: *search it doesn't exist → author via API (or hand-edit the data block for nested shapes) → wire it into the world → sync the doc → verify.*
...
> - `worldbuilder-crud-arrays.test.js` — **4/6** (CRUD array-field + itemChain widgets; never loads `index.html`) — §DX-02d
>
...
./api.sh list quest --q "TERM";  ./api.sh get node LHR;  grep -n "SYMBOL" index.html
