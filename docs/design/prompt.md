...
- **A doc anchor names a symbol, not a line (§DX-01e).** Write it as one token — `` `execBits(bits, ctx)@22223` `` — where the **symbol is the pointer** (any literal substring of the line you mean, optionally file-qualified as `` `src/js/wbapi-server.js:function seededNext@1147` ``) and the number is a cached hint. `npm run anchors` audits every doc, `npm run anchors:fix` refreshes drifted numbers mechanically, and `node src/scripts/resolve-anchors.js <symbol>` answers *where does this live now*. `check:anchors` (gate #15) **fails on a dead symbol** — a real stale pointer — and only warns on a drifted number. Bare `` `21722` ``-style anchors survive in `plan-archive.md` and the lab reports: those are **history — annotate, don't rewrite**, and never trust their numbers.
