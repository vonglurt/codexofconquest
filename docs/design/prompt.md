...
# prompt.md — Operating Directive for Building CodexOfConquest.com
...
```bash
./bin/api ping                       # server alive + up to date?
./bin/api list quest --q "keyword"   # does this quest exist?
./bin/api audit                      # what's actually there vs. what the plan assumes
grep -n "yourIdentifier" play.html
```
Confirm current state from the live file, never from memory or a "DONE" claim.

**3. Lab-report gate (conditional).** If the item is a major collection, a multi-system redesign, a new arc of 3+ nodes, or a pre-implementation design review, write `docs/lab-reports/lab-report-<title>.md` **locking the data shapes before any HTML edit** (Lab Report Policy, CONTRIBUTING.md). A single quest/monster/value edit does *not* need one.
...
**Golden rule:** all world-building goes through `./bin/api`. It handles nonces, retries, error formatting, and pipe-safe JSON. Never use raw `curl` for authoring; if `./bin/api` lacks a command you need, add the endpoint + a named `./bin/api` wrapper. Full reference: **`docs/api/API-README.md`** + `docs/api/wbapi-help.md`; day-to-day: `./bin/api help`.

```bash
# Server lifecycle
./bin/wbapi start | status | restart | stop   # :1367 (one-shot — the server never self-restarts)
./bin/api ping                                 # health + confirm it's up

# The common cycle — SEARCH → INSPECT → EDIT (never guess an ID)
./bin/api list quest --q "keyword"        # search
./bin/api get quest quest_courier_release # inspect full fields
./bin/api location LHR                     # composite: node + quests + NPCs + monsters
./bin/api put quest quest_x desc="..."     # edit
...
./bin/api save                          # deliberate dated backup beside the game file
./bin/api snapshots [--sweep] [--force] # list them / delete the patch-archived ones

# Network health — run at the start and end of every session that touches nodes
./bin/api reachability     # target 100% reachable from LHR
...
New content is **data added to the big collections** in `play.html`: `NODE_MAP` (places), `QUEST_DB` (missions, UQF-1.0), `MONSTER_POOL` (encounters), `WORLD_DB` (terrains), `NPC_DIALOGUES` (talk). The pattern is always: *search it doesn't exist → author via API (or hand-edit the data block for nested shapes) → wire it into the world → sync the doc → verify.*
...
- **`activateNode`/`waypointNode` must be a live `NODE_MAP` key.** Look it up in **`docs/maps/node-index.md`** (`npm run nodes` — generated, gate-fenced) or `./bin/api get node <CODE>`; **never** from a hand-maintained table. `maps.md`'s historical legend names 81 codes that do not exist, and `710bb75` copied three of them onto eight quests (§AUDIT-03c/03l). `./bin/api audit` catches a dead `activateNode`; `src/tests/integration/audit03c-node-refs.test.js` now catches both fields.
- Validate with `./bin/api advise <id>` (errors/warnings, NPC-key resolution), then sync the row in `quest.md` (location-organized register), then add an integration test under `src/tests/integration/`.

### Adding a node (place)

```bash
./bin/api post node code=NEW name=<terrainKey> label="Display Name" act=1
```
...
- A node on land contiguous with the main landmass is **automatically walk-routable** — the mover walks cell-by-cell. `./bin/api reachability` (BFS from LHR) is the authority, not the abandoned legacy edge graph. You do *not* need `highway`/`connect` to make it reachable.
...
5. **Parity fences.** `MOVER:CORE`, `ROOMS:CORE`, `DUEL:CORE`, `QUEST:CORE` are pure, world-injected, and byte-identical to their `src/js/*.js` twins, asserted by `src/scripts/check-*-parity.js`. **Never edit an inlined copy** — edit `src/js/<mod>.js`, re-inline, and re-run the checker.
...
7. **API-first.** Author through `./bin/api`; add the endpoint before the edit if it's missing.
...
**Known pre-existing baseline reds: `check:walk` is fully green (16/16; 11/11 since 2026-07-28 §DX-01a, gate #12 added 2026-07-29 §AUDIT-03l, gate #13 added 2026-07-31 §AUDIT-03j, gate #14 added 2026-07-31 §AUDIT-03n, gate #15 added 2026-07-31 §DX-01e, gate #16 added 2026-08-04 §AUDIT-03m) — any red there is a real regression you introduced. The Playwright suite carries FOUR, in ONE file** — a full run is **935 passed / 4 failed** (940 tests), last re-proved **2026-08-06** (§VM-01-G-FU-f2 — the TLS `finalBattle` node field; the run added the 5-test `vm01f2-finalbattle-field.test.js` — **3 of 5 fail on HEAD: exactly the source/data/helper migration tests — and both behaviour pins pass BOTH ways**, because the framing is unchanged by construction; `multiplayer-presence:171` flaked and retried green. The prior **931** was §SPARK-01-FU + §LXX-01-FU's count — the four double-pay fixes; **no new tests**: the four double-pay pins in `uqf-node-harbor.test.js`/`uqf-node-lxx.test.js` flipped in place to the single-pay contract, and `quest-runtime-uqf.test.js`'s fuzzy-OR `items` specimen repointed from quest_forge_02 — flag-gated since the fix — to `mq_6`, *pin the property, never the incident*; no flake that run. The prior **931** was §VM-01-G-FU-f's count, which added the 10-test `uqf-node-ssj.test.js` — **3 of 10 fail on HEAD: exactly the registry/source tests — and all 7 behaviour tests pass BOTH ways**, because the SSJ tournament moved verbatim as the family's third zero-delta no-op: the 16-combo golden is 16/16 byte-identical incl. 7 click combos and all 16 §7½ PNGs are byte-identical; no flake that run; the same slice grew the `uqf-node-hooks` pin 31→32. The prior **920** was §VM-01-G-FU-e's count, which added the 15-test `uqf-node-lxx.test.js` — **5 of 15 fail on HEAD: the 4 registry/source tests + the recovered-narrative test — and the 10 greens are exactly the verbatim/unchanged-behaviour ones**, including the three §LXX-01-FU double-pay pins, which pass BOTH ways because the §LXX stack moved verbatim: the 25-combo golden is 20/25 byte-identical with 1 id-only delta and the 4 others the same recovered smelt narrative; `multiplayer-presence:171` flaked in the full run and retried green; the same slice grew the `uqf-node-hooks` pin 27→31 and taught `uqf-node-verbs-d1`'s label-verb pin to exclude the `lxx-` group. The prior **906** was §VM-01-G-FU-d's count, which added the 17-test `uqf-node-harbor.test.js` — **6 of 17 fail on HEAD: the 3 registry/source tests, the 2 LCY writ-beat-fix tests and the MME cost-refusal-channel test — and the 11 greens are exactly the verbatim/unchanged-behaviour ones**, because the harbor stack moved verbatim except its two deliberate fixes: the 49-combo golden is 46/49 byte-identical with the 3 deltas exactly those fixes; the `multiplayer-presence:171` flake went red in the full run and passed 7/7 alone; the same slice grew the `uqf-node-hooks` pin 21→27 and converted `uqf-node-alch`'s own `slice(-6)` tail pin to the contiguous run — the G-FU-c lesson biting the file that wrote it. The prior **889** was §VM-01-G-FU-c's count, which added the 16-test `uqf-node-alch.test.js` — **3 of 16 fail on HEAD, exactly the registry/source tests, and all 13 behaviour tests pass BOTH ways**, because the whole §ALCHEMY/§WISDOM stack moved verbatim: the 37-combo golden is 37/37 byte-identical, the family's second zero-delta slice; the `multiplayer-presence:171` flake passed that run; the same slice grew the `uqf-node-hooks` pin 15→21 and adjusted `uqf-node-hunt`'s tail pin to the contiguous run. The prior **873** was §VM-01-G-FU-b's count, which added the 11-test `uqf-node-hunt.test.js` — **3 of 11 fail on HEAD, exactly the registry/source tests, and all 8 behaviour tests pass BOTH ways**, because the whole §HUNT stack moved verbatim: the 22-combo golden is 22/22 byte-identical, so a behaviour test that only passed after the change would disprove the no-op; the run reported 872/5 and the fifth was the known `multiplayer-presence:171` flake, 7/7 alone; the same slice grew the `uqf-node-hooks` pin 11→15. The prior **862** was §VM-01-G-FU-a's count, which added the 11-test `uqf-node-verbs-crown.test.js` — **6 of 11 fail on HEAD, and the 5 that pass are exactly the verbatim-move / unchanged-behaviour ones**; the same slice grew three registry pins in place — `uqf-node-hooks` 9→11 crown hooks, `uqf-npc-row-hooks`' npc-row block is contiguous but no longer the registry tail, `uqf-node-verbs-d1`'s label-verb pin excludes the `crown-` groups its own file owns. The prior **851** was §VM-01-G4d's count, which added the 12-test `uqf-node-verbs-d3.test.js` — 6 red on HEAD, the 6 passers exactly the unchanged-behaviour ones. The prior 839 was §VM-01-G4c's count, which added the 13-test `uqf-node-verbs-d1.test.js`; that run reported **838/5** because the known `multiplayer-presence:171` flake did not retry green, and the file passes **12/12** on its own. **7 of the 13 fail on HEAD and the 6 that pass are exactly the ones asserting *unchanged* behaviour.** The run before it also carried a red that was **mine and was correct** — `uqf-quest-activation.test.js:120` pinned that clicking Ori paid *nothing at click time*, which was a timing artifact of the inline handler, not the property the test exists for; the property is *"+300gp exactly once, from the quest chain"*, and it is now asserted against a further render instead. **Pin the property, never the incident.** The prior 826 was §VM-01-G4b's count, which added the 10-test `uqf-node-verbs.test.js` — **all 10 fail on HEAD**, because the migrated surface's DOM identity changed with it, so the before/after equivalence is carried by a separate golden-DOM diff of all four DUS states rather than by the test file. The prior 816 was §VM-01-G4a's count, which added the 12-test `uqf-verb-driver.test.js`; the run that added them reported 815/5, and **the fifth red was mine and was the assertion working as designed** — `quest-runtime-uqf.test.js:28` pins the whole `HANDLERS` key list precisely so a new opcode cannot enter the table unrecorded, so adding `cost` to the pin is part of the fix, not a concession to it. The prior 804 was §VM-01-G2b's count, which added the 8-test `uqf-npc-row-hooks.test.js`; that run reported 804/5 because the known `multiplayer-presence.test.js:171` flake did not retry green, and the file passes 7/7 on its own. The prior 796 was §AUDIT-03q's count, which added 5 tests to `audit03m-legacy-codes.test.js`; that run reported 795/5 because the known `multiplayer-presence.test.js:171` flake did not retry green, and the file passes 7/7 on its own — the earlier 791 was §AUDIT-03m-FU's count and 771 §DX-02l-FU's). **`multiplayer-presence.test.js:171` is a known flake — re-run the file before calling it a fifth red.** This list is the whole red set; **anything beyond these four is yours**:
> - `worldbuilder-crud-arrays.test.js` — **4/6** (CRUD array-field + itemChain widgets; never loads `play.html`) — §DX-02d
>
...
- **A doc anchor names a symbol, not a line (§DX-01e).** Write it as one token — `` `execBits(bits, ctx)@22251` `` — where the **symbol is the pointer** (any literal substring of the line you mean, optionally file-qualified as `` `src/js/wbapi-server.js:function seededNext@1150` ``) and the number is a cached hint. `npm run anchors` audits every doc, `npm run anchors:fix` refreshes drifted numbers mechanically, and `node src/scripts/resolve-anchors.js <symbol>` answers *where does this live now*. `check:anchors` (gate #15) **fails on a dead symbol** — a real stale pointer — and only warns on a drifted number. Bare `` `21722` ``-style anchors survive in `plan-archive.md` and the lab reports: those are **history — annotate, don't rewrite**, and never trust their numbers.
...
```bash
# Am I set up?
./bin/api ping && ./bin/wbapi status

# What exists already? (grep before building)
./bin/api list quest --q "TERM";  ./bin/api get node LHR;  grep -n "SYMBOL" play.html
npm run nodes && open docs/maps/node-index.md   # the LIVE node-code reference (never read a code off a doc table)
node src/scripts/resolve-anchors.js SYMBOL   # where does this live NOW? (doc anchors are `symbol@line`, §DX-01e)
npm run stats                            # live node/monster/quest/terrain/NPC/line/byte totals (never trust a hardcoded count)

# Author
./bin/api post node code=NEW name=<terrainKey> label="…" act=N
./bin/api post quest id=… title="…" type=side activateNode=…    # flat quests only
#   nested UQF → hand-author a QUEST_DB block (restart server first), then ./bin/api advise <id>

# Wire + validate the world
./bin/api reachability     # 100%?
./bin/api broken           # 0?
./bin/api advise <questId> # errors=[] warnings=[]?
./bin/api snapshots        # anything dated left beside the game file? (gitignored — nothing else reports it)

# Verify + close
git rev-parse --abbrev-ref HEAD          # on the track's feat/<slug> branch, NOT main? (§8)
npm run check:walk;  node --check src/js/wbapi-server.js
