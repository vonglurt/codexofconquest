...
*Entry 41 (read aloud at `TLS` (historical `CO`)):* The entry that must always be read. The blank line after it is what Froberger left for the person who came after him. In NG+, the player writes Entry 42. The blank line was always the invitation.

---

#### The Cup — Brynn Clerambault (`TLL` (historical `IN`))
...
#### The Corner — Yael Scheidemann (`LHR` (historical `CI`))
...
#### The Manifests — Commander Seraphine Bruhns (`HKG` (historical `CY`))
...
#### The Blank Page — Void Archaeology (`TLS` (historical `CO`))
...
> **⚠️ Note:** This is a conceptual terrain topology map — not the canonical node grid, and **not a live reference**. The 26×16 grid it was drawn against was retired by §WALK (the world is a 90×360 geo grid now), and `maps.md`'s legend tables were quarantined as historical by §AUDIT-03l. **The authoritative node reference is `docs/maps/node-index.md`** — generated by `npm run nodes` and gate-fenced by `check:nodeindex`; never read a node code off a hand-maintained table. This diagram also predates junctions, EB nodes, and the defi_land cluster (`ZRH` (historical `DF`)/`DUS` (historical `HM`)/`MUC` (historical `GL`)).
...
- **Lore tie:** Void pressure from `ZRH` (historical `DF`) (Unbanked Quarter) leaking into Cat Quarter
...
**Item Found:** Fishing Rod — required to trigger `storyFishing()` at `BOO` (historical `YL`).  
**Sleep:** Free at this node.

**[✅ Layer 83 — §XLV] Yugurt Cabin Expanded — Tournament & Six Fishermen**  
Five additional NPCs will be present at `SSJ` (historical `YC`) alongside The Fisherman (who is the Master):
...
- Activates: `SSJ` (historical `YC`) visit with Fishing Rod + at least 1 prior catch  
...
This injection is a one-time surface per run (not NG+-preserved). It fires only at `TLS` (historical `CO`) after the Void is sealed. No journal entry. No reward. The console is the last dungeon.

*Implementation note:* Add `questMinusOne: false` to `_S_DEFAULTS()`. Gate in `storyRender()` at `TLS` (historical `CO`). See `plan-archive.md §XIV-G` for the 7-step implementation sequence.
...
- **Entry 42 modal at `LHR` (historical `CI`)** — textarea + "Write it." / "Leave it blank." buttons. Gated: `ngPlusRun ≥ 1`, `priorQuestMinusOne`, `entry42Written === false`, 3+ Dear Friends. Writes to `S_story.entry42Text`, sets `entry42Written = true`.
...
- **Quest chain (activated at `LHR` (historical `CI`) on NG+ first visit):**
  - `quest_ng_01` — "The Remembered Path": revisit 3 Dear Friends (tracked via `ngMemoryDelivered`). Reward: 500gp.
  - `quest_ng_02` — "The Open Page": write Entry 42 at `LHR` (historical `CI`). Only activated if `priorQuestMinusOne`. Reward: (the act itself).
  - `quest_ng_03` — "The Letter": find Froberger's sealed letter at `TLS` (historical `CO`). Reward: 300gp + `frobergerLetterFound = true`.
...
- **Five investigation sites** — `[INVESTIGATE]` button at `LHR` (historical `CI`)/`BMA`/`ZRH`/WM/`GVA` when prerequisites met. Each reveals one paragraph of the Antecedent Containment Protocol pattern. Flags: `vaCI`, `vaSL`, `vaDF`, `vaWM`, `vaMT`.  *(historical: `SL`=`BMA` · `DF`=`ZRH` · `MT`=`GVA`)*
...
`INN_DREAMS` const keyed by `TLL` (historical `IN`) and `NUE` (`STN` (historical `SF`)/`PDL` (historical `IS`) have `sleep:false` in WORLD_DB). Fires via `setTimeout` after `storyConfirmSleep()`, prefixed `💤 ‹dream›`. Cycle: `gameDay % 3`. Conditionals checked first; first matching flag wins.
...
### Q-NEXUS-01 — `DUS` (historical `HM`) | "Blueprints" ✅
...
### Q-NEXUS-02 — `DUS` (historical `HM`) | "Creative Literacy" ✅
...
| `storyPreFinalBattle()` | 10981 | Pre-battle screen for Auros boss fight at `TLS` (historical `CO`) | `BOSS_COMMANDER_AUROS`, `defeatedBattles` | `_preBattNode` for Auros |
| `storyMsg(txt)` | 11280 | Appends a message line to the node log | txt string | DOM story-log element |
| `_yaelEscortAction()` | 8057 | One-time Yael escort narration at `LHR` (historical `CI`); sets yaelEscortUsed | `yaelEscortUsed`, `npcFavorability.yael` | `yaelEscortUsed = true` |
...
## ✅ Implemented — The Froberger Memorial: A Living Stone at `LHR` (historical `CI`) (plan-archive.md §XXVIII, Layer 63)
...
Six story nodes (`KVA` (historical `PL`)/`ATH`/EF2/`ZTH`/`MLA`/`FCO`) completing the Mediterranean commission arc begun in §LIX–§LXIV. Five quests, one storyRender event (Malta snake), six NPC quoteFn entries. All nodes Act IV.  *(historical: `AE`=`ATH` · `KR`=`ZTH` · `ML`=`MLA` · `ST`=`FCO`)*
