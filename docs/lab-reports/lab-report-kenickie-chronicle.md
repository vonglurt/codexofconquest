...
- **Sheet-swapper.** `_cqNpcs.push('kenickie')@35338`.
- **Reachability.** `CDG:{ num:77@8811` is declared before `LIM`, `FRK` and `FRS`, its three cell-mates at `21,182`, so it is `list[0]` and can become `currentCode`. **The market is reachable** — not a §AUDIT-03x casualty.
...
- **Storage.** `careerStats: { kills:0@23179` and its `runStats` twin on the next line, both **inline ten-field literals** in `_S_DEFAULTS()`.
- **Factory.** `const _STAT_ZERO = () =>@23974` — an arrow const, used by the lazy-init guard and the respawn reset **but not by `_S_DEFAULTS()`**.
- **Writer.** `function _statTally(key, n) {@23975` increments both ledgers. **Fourteen call sites, and every one of them goes through it** — no direct mutation of either object exists. That contract, which is the report's real architectural claim, holds exactly.
...
- **Game-over surface.** `function _populateGameoverChronicle() {@23913` reads `const rs = S_story.runStats || {};@23916`, builds **nine** rows, and gates the div both ways — `el.classList.add('has-data')@23934` on data, `remove` on none, against `#gameover-chronicle.has-data@1720` (`display:block`, default `none`). The div exists at `id="gameover-chronicle"@5027` and `function storyGameOver() {@23937` populates it before revealing the modal, which is reached from `hp === 0`. The surface really renders.
- **Character sheet.** Ten rows, **two columns** — `This Life` (`runStats`) and `All Lives` (`careerStats`), the career column blanked by `const isFirstRun@37924`.
- **Respawn.** `const _survivingCareerStats@23985` copies the career ledger out, reloads the checkpoint, restores it, and zeroes `runStats`. Exactly as specified.
...
| 3 | Unlock on `quest_cat_05 === 'complete'` | `if (qs['quest_cat_05'] === 'complete')@33220` | ✅ |
...
| 12 | `kenickieMarketUsed` set on first purchase | `S_story.kenickieMarketUsed = true;@33271` | ✅ write |
...
| 15 | Kenickie's card *replaces* the CQ NPC display | **Appends** — `_cqNpcs.push('kenickie')@35338`, third card beside Jimmy and Sandy | ± |
| 16 | Render block inside `storyRender` (lines 14878–14933) | Extracted verbatim to `NODE_HOOKS` by §VM-01-G4d | ± moved |
| 17 | *"`quest_cat_05` — defeating the Cat-King"* | `quest_cat_05` is the **Don Fluffissimo** fight; the Cat-King is `title:'Tommy: The Cat-King Cometh'@13785` (`quest_cat_06`). Both titles identical at `4090c82` | ⚠️ |
| 18 | Kenickie is *"a single-state encounter"* with no favorability | `impartial` / `friendly` / `dearFriend` present **at birth**, and `{ kind:'favor', npc:'kenickie', set:3 }@34663` now drives him to Dear Friend | ⚠️ |
...
| 21 | `function _STAT_ZERO() { return {…} }` | `const _STAT_ZERO = () =>@23974`, arrow | ± |
| 22 | Ten stat fields, as named | 10/10 live under their specified names | ✅ |
| 23 | Written via **`_trackStat(field, amount)`** | **0 occurrences, 0 commits ever.** The writer is `_statTally(key, n)@23975` | ✗ NOT SHIPPED (name) |
| 24 | No direct mutation outside the writer | Holds — 14 call sites, all `_statTally` | ✅ |
| 25 | Game-over Chronicle has **ten** rows | **Nine.** `deaths` and `attacksHit` are not rows; a derived `['Hit rate',@23922` is | ± |
| 26 | The ten row labels as tabulated | **3 of 10** match (`Damage dealt`, `Damage received`, `Days adventuring`) | ± |
| 27 | `has-data` gates visibility | Both directions, plus CSS at `#gameover-chronicle.has-data@1720` | ✅ |
| 28 | On respawn: career kept, run zeroed | Exact | ✅ |
| 29 | **On NG+: `careerStats` preserved** | `function storyNewGamePlus() {@24059` preserved six named fields and `careerStats` was not one; `Object.assign(S_story, _S_DEFAULTS())` zeroed it, **false at `cab8865` too**. **§CHRON-01 ✅ 2026-09-03:** `savedCareerStats` is saved before the reset and restored after, and the sheet's `All Lives` column now shows on any NG+ run | ✅ since §CHRON-01 |
| 30 | Character sheet shows `careerStats` (not `runStats`) | Shows **both**, `runStats` in the first column, since `cab8865` | ⚠️ |
| 31 | `daysAdventuring` ≠ `gameDay`, not aliased | Correct — `S_story.gameDay = (S_story.gameDay || 0) + 1;@36452` is a separate counter with its own readers | ✅ |
| 32 | `sleeps` *"not equivalent to `daysAdventuring`"* | `_statTally('sleeps', 1);@36478` and `_statTally('daysAdventuring', 1);` (retired by §CHRON-01 (b), 2026-09-03) are adjacent, are each field's **only** writer, and are unconditional — the two fields are **permanently equal**, and the game-over screen prints both | ⚠️ → **§CHRON-01 (b)** |
...
| R2 | Inline comment at the `kenickieMarketUsed` declaration naming its purpose | **NOT SHIPPED** — `kenickieMarketUsed: false,@23153` is bare. Superseded: the flag has no purpose to document (delta 13) |
...
| R4 | Compute and display a hit-rate percentage | **ALREADY SHIPPED WHEN WRITTEN** — `['Hit rate',@23922` is in `aef1650`, the same commit that created `attacksAttempted` and `attacksHit`. The recommendation asks for a feature that could not have existed one commit earlier and did exist in the commit that gave it inputs |
...
The consequence compounds rather than merely losing data. `ngPlusRun` **is** preserved, so the game knows it is on run four while the ledger says nothing has ever happened. And because the character sheet blanks the career column when `const isFirstRun@37924`, a zeroed career ledger **also hides itself** — an NG+3 player reads a character sheet byte-identical to a player who has never died. There is no error, no empty column, no zero: the surface silently agrees with the wipe, which is why 79 days of play have not surfaced it.
...
| `S_story.kenickieMarketUsed = true;@33271` | The flag's only writer |
| `_cqNpcs.push('kenickie')@35338` | Sheet-swapper card |
| `{ kind:'favor', npc:'kenickie', set:3 }@34663` | §GR La Riva → Dear Friend |
| `const POTION_TIERS = {@24370` | Standard prices the 10% is measured against |
| `careerStats: { kills:0@23179` | Inline literal in `_S_DEFAULTS()` |
| `const _STAT_ZERO = () =>@23974` | Zero factory (arrow const) |
| `function _statTally(key, n) {@23975` | The real dual-ledger writer |
| `function _populateGameoverChronicle() {@23913` | Nine-row run summary |
| `['Hit rate',@23922` | The derived row |
| `#gameover-chronicle.has-data@1720` | Visibility gate |
| `const _survivingCareerStats@23985` | Respawn preserve — correct |
| `function storyNewGamePlus() {@24059` | NG+ — **does not preserve `careerStats`** |
| `const isFirstRun@37924` | Blanks the career column, hiding the wipe |
| `_statTally('sleeps', 1);@36478` · `_statTally('daysAdventuring', 1);` (retired by §CHRON-01 (b), 2026-09-03) | The permanently-equal pair |
