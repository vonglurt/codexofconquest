...
- **Unlock.** `if (qs['quest_cat_05'] === 'complete')@32988`, inside `function _nodeHookCdgKenickieMarket@32985`, registered as `{ id:'cdg-kenickie-market', nodes:['CDG'], fn:_nodeHookCdgKenickieMarket }@34179`.
...
- **Shop.** `shopDiv.id = 'kenickie-shop-div';@33009`, mounted by `cqDiv.insertAdjacentElement('afterend', shopDiv);@33054` — an in-place expansion, not a modal, exactly as specified.
...
- **Discount, still exact.** `const POTION_TIERS = {@24308` prices the same two potions at 50 and 150. 45 and 135 are 10% under both, unchanged after 79 days.
- **Bait stacking.** Matches on `name` **and** `type === 'bait'`, increments `count`, no duplicate rows.
- **Sheet-swapper.** `_cqNpcs.push('kenickie')@35113`.
...
| 3 | Unlock on `quest_cat_05 === 'complete'` | `if (qs['quest_cat_05'] === 'complete')@32988` | ✅ |
...
| 12 | `kenickieMarketUsed` set on first purchase | `S_story.kenickieMarketUsed = true;@33039` | ✅ write |
...
| 15 | Kenickie's card *replaces* the CQ NPC display | **Appends** — `_cqNpcs.push('kenickie')@35113`, third card beside Jimmy and Sandy | ± |
...
| 18 | Kenickie is *"a single-state encounter"* with no favorability | `impartial` / `friendly` / `dearFriend` present **at birth**, and `{ kind:'favor', npc:'kenickie', set:3 }@34397` now drives him to Dear Friend | ⚠️ |
...
| 21 | `function _STAT_ZERO() { return {…} }` | `const _STAT_ZERO = () =>@23916`, arrow | ± |
| 22 | Ten stat fields, as named | 10/10 live under their specified names | ✅ |
| 23 | Written via **`_trackStat(field, amount)`** | **0 occurrences, 0 commits ever.** The writer is `_statTally(key, n)@23917` | ✗ NOT SHIPPED (name) |
| 24 | No direct mutation outside the writer | Holds — 14 call sites, all `_statTally` | ✅ |
| 25 | Game-over Chronicle has **ten** rows | **Nine.** `deaths` and `attacksHit` are not rows; a derived `['Hit rate',@23863` is | ± |
...
| R4 | Compute and display a hit-rate percentage | **ALREADY SHIPPED WHEN WRITTEN** — `['Hit rate',@23863` is in `aef1650`, the same commit that created `attacksAttempted` and `attacksHit`. The recommendation asks for a feature that could not have existed one commit earlier and did exist in the commit that gave it inputs |
...
| `function _nodeHookCdgKenickieMarket@32985` | Market block (ex-`storyRender`, §VM-01-G4d) |
| `shopDiv.id = 'kenickie-shop-div';@33009` | Shop container |
| `S_story.kenickieMarketUsed = true;@33039` | The flag's only writer |
| `_cqNpcs.push('kenickie')@35113` | Sheet-swapper card |
| `{ kind:'favor', npc:'kenickie', set:3 }@34397` | §GR La Riva → Dear Friend |
| `const POTION_TIERS = {@24308` | Standard prices the 10% is measured against |
| `careerStats: { kills:0@23147` | Inline literal in `_S_DEFAULTS()` |
| `const _STAT_ZERO = () =>@23916` | Zero factory (arrow const) |
| `function _statTally(key, n) {@23917` | The real dual-ledger writer |
| `function _populateGameoverChronicle() {@23854` | Nine-row run summary |
| `['Hit rate',@23863` | The derived row |
