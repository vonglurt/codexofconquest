...
**Parent:** `docs/lab-reports/lab-report-play-review.md` §PLAY-01-A · **Track:** BACKLOG.md §PLAY-01 (archived → `plan-archive.md`)
...
| Day-49 defeat flavour at line 23096 | byte-exact; `The court had granted forty-nine days@23889` at HEAD | ✅ exact |
| `#s-day` shows `N/49` | `dayEl.textContent = S_story.day + @36086` renders `N/49` | ✅ exact |
| `#s-shards` shows `N/7` | `.textContent = S_story.shards + @36088` | ✅ exact |
| `#s-level` shows level and XP but never the Lv 20 target | `const lvEl = document.getElementById@36118` — no target | ✅ exact |
...
`You are Level 20.@31387` — the §XIV *Open Door* easter egg, which fires only **after** you already
...
| Goal constants | `const SHARD_GOAL = 7, LEVEL_GOAL = 20, DAY_DEADLINE = 49;@36157` | ✅ live, values as specified |
| Chip renderer | `function _renderObjectiveChip() {@36158` | ✅ live |
| Seven-symbol loop | `const got = Math.max(0, Math.min(SHARD_GOAL@36161` | ✅ live, clamped 0–7 |
| Level leg and gold-at-20 state | `lvEl.innerHTML = @36168` · `#objective-chip .obj-leg.hit b@1753` | ✅ live |
| Day leg | `(day >= DAY_DEADLINE - 3 ? @36175` | ⚠ superseded — see §7.1 |
| Render call site | `_renderObjectiveChip();   // §PLAY-01-A@36132` | ✅ live, at the tail of `storyUpdateStatus` |
...
| Frame trigger, fresh-game only | `_showCourierMap();   // §PLAY-01-A@23989` in `function storyNewGame(startScores) {@23952` | ✅ live, absent from `storyNewGamePlus` |
| Frame guard | `function _showCourierMap() {@23994` · `if (S_story.courierMapSeen) return;@23995` | ✅ live |
...
| 2. `src/tests/integration/courier-map.smoke.test.js` | ✅ **1/1**, 17 assertions, 2.5 s |
...
cap (`(day >= DAY_DEADLINE - 3 ? @36175`). The sidebar still carries the original doom ladder
(`(S_story.day >= 42 ? @36087`). Proved in the browser, one page, one tick apart:
...
- **(b) The level pill warns you for being level 8 of 20.** `(lv >= 8 ? @36123` turns `#s-level`
...
`Object.assign(S_story, _S_DEFAULTS());@23955`, which cannot clear a key its source object does not
...
**The reachable path is the defeat screen.** `, storyNewGame);@38311` wires the *"↩ New Game"*
...
4. **The chip degrades safely outside the game.** `function _renderObjectiveChip() {@36158` returns
...
> nothing."*
> — `The court had granted forty-nine days@23889`, the only place the game ever explained the
