...
**4.6 — The parity guard ships first.** `src/scripts/check-ladder-migration.js`, manifest-driven: one entry
...
| 4 | `FISHING_GUIDE_TEXT` at line **24017** | 23997 at `27956e4` (**−20**) | ⚠ hint drifted; the *argument* holds — `const FISHING_GUIDE_TEXT@26793` is still declared after `const QUEST_DB = {@10630`, so the TDZ objection is live today |
| 5 | 22 branches migrated (9 b2a + 13 b2b) | `check:laddermigration`: **22 quests, 148/148 checks** | ✅ **EXACT** |
| 6 | b1 allow-list: 13 optional fields | `for (const f of ['desc', 'readText'@26315` carries all 13 **plus `heal`** | ✅ 13/13 shipped; ➕ grown to 14 |
| 7 | `silent:true` added | `if (!s.silent) msgs.push@26322`; all 22 migrated grants carry it | ✅ SHIPPED |
| 8 | Four sites in lockstep | Runtime ✓ · `edit.html:const GRANT_RICH = [@8607` ✓ · widget `edit.html:{f:'silent',chk:true}@8598` ✓ · `src/scripts/check-itemchain.js:grant passes heal@83` ✓ — and all four carry `heal` | ✅ held through a later widening by a different track |
| 9 | Never emit `once:false` | 0 occurrences file-wide | ✅ HELD |
| 10 | 3 named load-bearing names survive | `const KEY_EVENTS = [@26340` holds 7 items; all three named ones grant through `itemChain` | ✅ VERIFIED |
| 11 | Guard wired into CI beside `check:itemchain` | `walk-invariants.yml` runs both on the same push paths | ✅ EXACT |
| 12 | "Both completion paths unchanged" | The second call site (legacy `_rollCeremonia`) was **retired** by §ARCH-01 W7d `f8691c1`; one call site remains — `msgs.push(..._applyItemChain(q))@30355` | ⚠ STALE by design, not a defect |
| 13 | The ladder shrinks but survives | **Deleted.** `a79c76a` (§ARCH-01 W7c, 2026-07-03) folded all 61 ids into per-quest `onComplete` bit chains — `W7c folded the per-id hardcoded effects block@30350` | ⚠ **SUPERSEDED — 0 branches since 2026-07-03** |
...
- **`fishing_guide`** — the one "pure push", but its `readText` is `FISHING_GUIDE_TEXT`, a const
  ...
  `readText:FISHING_GUIDE_TEXT@13847`.
...
- **`wm_01`** — `Scholar Kings' Seal` removal is conditional (`!archiveLetterObtained`) *and*
  ...
  `seal spend stays _legacy_fn@11087`.
...
- **§DX-02ci** 🟢 — the parity guard's check (c) is **vacuous**. `LADDER` has been empty since
  ...
  `src/scripts/check-ladder-migration.js:reward ladder stays deleted@208` — shows the right instinct and
...
- **§DX-02cj** 🟢 — the four-place lockstep is enforced by a **comment**. Nothing in `src/scripts/` or
  ...
  `for (const f of ['desc', 'readText'@26315`, so the two can diverge silently. It
