...
| Sea-lane parse | `` `src/js/wbapi-server.js:function getSeaLanes()@1021` `` | ✅ from `_rawSrc`, cached by source ref |
| Rate-table parse | `` `src/js/wbapi-server.js:function getEncounterRateTable()@1077` `` | ✅ **stronger than spec** (§5) |
| Server terrain inference | `` `src/js/wbapi-server.js:function terrainAt(r, c)@1094` `` | ✅ mirrors `` `function _inferTerrain(r, c)@28535` `` |
| World inputs wired | `` `src/js/wbapi-server.js:function getMoverWorld()@1116` `` | ✅ `terrainAt` + `encounterRate`; **no `ferryEdges`** |
| Per-session PRNG | `` `src/js/wbapi-server.js:function seededNext(s)@1150` `` | ✅ mulberry32 over `s.rngState` |
| Flat tier weights | `` `src/js/wbapi-server.js:const BASE_TIER_WEIGHTS@1161` `` | ✅ `{trivial:40, easy:35, medium:20, hard:4, deadly:1}` — byte-exact to Inc 2 |
| Monster draw | `` `src/js/wbapi-server.js:function pickMonster(terrain, s)@1165` `` | ✅ returns `{key, name, tier}` |
| The instanced roll | `` `src/js/wbapi-server.js:s.encounter = (seededNext(s) < mres.encounter.baseRate)@8902` `` | ✅ §4.4's pseudo-code, near-verbatim |
...
**single-player client** onto the same algorithm: `` `function _seededNext()@6436` `` is byte-identical to
...
| Named cells are not encounter-eligible | **Correct** — `` `function _enterEmptyCell(r, c)@28575` `` is still the only client site rolling against the terrain rate |
...
- **§AUDIT-03bg** 🟡 — **§4.3 logged one SP/MP encounter divergence; there are now four**, and none of the
  ...
  `` `function _enterEmptyCell(r, c)@28575` `` applies, in order: `_partyEncounterRate` (§MESH-01f — **×0.5
  with a co-present ally**, 0 with a sentry), then §KG-01's `` `if (S_story.huntMode) baseRate = Math.min(0.8, baseRate * 2);@28595` ``,
