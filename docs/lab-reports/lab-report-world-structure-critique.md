...
| Map bounds lat −8→68, lon −25→72 | **EXACT** — `minLon: -25` / `maxLon: 72`, `src/tools/worldmap.js` lines 226–227 |
...
| 9 | "Add to `PRIORITY_HIGHWAYS` in `src/api/wb.js`" | Deleted 26 h earlier | **RETIRED**, action shipped anyway |
...
`minLon: -25` (`src/tools/worldmap.js` line 226) — the only such entry of 155, at the reference build and at HEAD.
Two consequences, both silent:

- The region-membership test (`g.lon >= b.minLon`, `src/tools/worldmap.js` line 263) is a bare comparison with no
...
said to add an entry to `PRIORITY_HIGHWAYS` in `src/api/wb.js`. That constant was real on 2026-06-09 and was
