...
**System:** `` `function storyRender(node, prefix) {@34816` `` and the registries `` `const NODE_PANELS = [@31532` ``, `` `const NODE_HOOKS = [@34416` ``, `` `const NODE_VERBS = [@34518` ``
...
now authors `` `finalBattle:{minLevel:20,minShards:7},@8740` `` read by the single helper
`` `function _finalBattleReady(code) {@28139` `` at three call sites. The other six sites survive
...
`` `            S_story.brynnKeeperStoryTold = true;@33044` `` and a second inside
`ACT8_FAREWELL_BEATS.brynn.text()` at
`` `        S_story.brynnKeeperStoryTold = true;@33044` ``. The second is dead by F1, so the
