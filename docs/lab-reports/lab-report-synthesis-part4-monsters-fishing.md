...
- **Fishing is the only positive-magic vector in the economy.** Monster kills were deliberately
  ...
  `// FC06: monster drops capped at base tier; fishing is the only source of +bonus weapons@24646`.
...
|---|---|---|---|
| `FISH_POOL` | `const FISH_POOL = [@26637` | **20** ranks, `{rank,key,name,desc}` | SHIPPED |
| `NIGHT_FISH_POOL` | `const NIGHT_FISH_POOL = [@26659` | **5**, ranks 6/8/10/12/14 | SHIPPED §XLVIII |
| `LAKE_MAGIC_DB` | `const LAKE_MAGIC_DB = {@26669` | **8**, all `sell:0`, minRank 11–20 | SHIPPED §DROP-03 |
| `BAIT_TABLES` | `const BAIT_TABLES = {@26766` | **3 × 6 = 18** | SHIPPED |
| `FISHING_GUIDE_TEXT` | `const FISHING_GUIDE_TEXT =@26793` | readable, 1 reader | SHIPPED (see F6) |
| `FISH_SIZE_TIERS` | `const FISH_SIZE_TIERS = [@26819` | **5** bands, 1–4/5–9/10–14/15–19/20 | SHIPPED |
| `FISHING_RODS` | `const FISHING_RODS = [@26834` | **4** tiers, 20/75/200/600gp | SHIPPED |
| `NPC_TOUR_OPPONENTS` | `const NPC_TOUR_OPPONENTS = [@26842` | **6**, Pip −1/50gp → The Fisherman +8/1500gp | SHIPPED §XLV |
| Four-phase cast | `function storyFishing() {@30548` | DEX → Catch → Type → select | SHIPPED |
| Luck | `function _calcLuck() {@23470` | 6th-root, `product <= 0` guard | SHIPPED |
| Drop nerf | `function _rollMonsterWeaponDrop@24641` | `const deg = Math.min(0, d6 - 5);@24652` | SHIPPED (see F2) |
| Lake gate | `const hasFish = node.isFishingLake;@35574` | **1** flagged node (`BOO:{ num:75@8795`) | SHIPPED, unreachable (F1) |
| Zone alias map | `const _zoneMap = { bank:'shore'@30629` | reconciles 2 vocabularies | SHIPPED |
...
| 6 | Phase 1 "d20 + DEX mod **vs DC 12**" | `const castMod = dexTot@30690` is a **three-band** test: `<12 → −2`, `≥17 → +2`, else 0 | **INCOMPLETE** — the perfect-cast threshold (17) is unstated; there is no pass/fail |
...
| 16 | `pb.fish` prevents node lock | `!pb.corridor && !pb.fish@36791` | **EXACT** (the `!pb.stalk` term was deleted with §TIMELESS-01) |
...
`LYR:{ num:41@8736` (Arctic Wastes, act 7) and `BOO:{ num:75@8795` (Yugurt Lake, act 3) both
occupy cell `2,194` — and `LYR` is declared **59 lines earlier**. Arriving at that cell renders the
arctic node, `const hasFish = node.isFishingLake;@35574` is never true, and `storyFishing` has
...
Luck integration points the report calls "load-bearing." The cabin survives (`SSJ:{ num:76@8799`
...
it has shipped **harder** — `const deg = Math.min(0, d6 - 5);@24652` widens the floor to −4, adds a
...
`const s = S_story.abilityScores || { str:16, dex:12, con:14, int:10, wis:12, cha:8 };` (collapsed to `const s = S_story.abilityScores;@23471` by §DX-02ac, 2026-09-03),
...
`const _zoneMap = { bank:'shore', reeds:'reeds', shallows:'deep' };@30629` holds it together. The
