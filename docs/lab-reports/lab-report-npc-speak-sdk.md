...
**Cause:** Claude Haiku 4.5's minimum cacheable prefix is **4096 tokens**. The system block measures **451–546 tokens** — about one-eighth of the threshold. Below the minimum, `cache_control` is silently a no-op: no error, no warning, `cache_creation_input_tokens: 0`. The endpoint at *`cache_control: { type: 'ephemeral' }`@10671* is correct code that cannot fire on the model directly above it at `src/js/wbapi-server.js:claude-haiku-4-5-20251001@10619`.


> **✅ SHIPPED 2026-09-23 (§DX-02ak) `05bb436`.** `cache_control` is deleted, with a three-line comment recording the floor so it is not re-added. **Re-derived over the whole corpus rather than a sample:** the system block is built per NPC, and across all **204** `BIRKA_NPC` profiles the largest is **~694 tokens** (2,499 chars) and the median **~116** — **0 of 204** come within 5× of the 4096-token floor. Confirmed against the current API reference: Haiku 4.5's minimum is 4096, and below it the request caches nothing and reports nothing. **Padding to reach the floor is a loss, now with the real multipliers:** cache writes cost **1.25×** and reads **0.1×** at the default 5-minute TTL, so a padded prefix would cost 5,120 token-equivalents to write against 500 uncached — and with this endpoint's traffic (20 calls over two days) the 5-minute TTL guarantees almost every call is a write, never a read.
