...
| `_weightedMonsterPick(terrain)` — tier-weighted pool draw | **live**, signature intact; body now notoriety-scaled + seeded | `function _weightedMonsterPick@38219` |
| `_setActivePath(fromCode, toCode, dir)` | **live** — the last surviving line of the runtime layer, see §IV-B | `function _setActivePath@38182` |
| `S_story.lastExitCode` / `lastExitDir`; the `mc-exit-active` gold exit arrow | **live**, still wired exactly as §IX describes | `mc-exit-active@36701` |
| Battle pipeline `loadWorldMonster` → `pendingBattle` → `#story-prebatt-overlay` → `storyApplyOutcome` → `btn-outcome-win` | **live** | `function _startStoryBattle@38241` |
...
| 10 | **Warp mode** — "instant teleport to destination", encounter 0% | **RETIRED, then forbidden** | Hard invariant #3 is now **"No jump travel, ever"**, stated in source: `no jump travel. checkpointNode@26049`. Warp is not merely removed — it is a **banned design**. *(It also never was a choice: see delta 11.)* |
...
Its only consumer is the map overlay's gold exit arrow (`mc-exit-active@36701`) — the last item in
...
1. **§AUDIT-03x — 172 of 416 nodes can never be arrived at, blocking 774 quests.** Traced from this
   ...
   **172 are non-primary** (worst cell `32,203` holds **17**). `S_story.currentCode` is assigned at
   exactly two sites — `S_story.currentCode = destCode@28374`, where `destCode = res.destCodes[0]`,
   and the respawn line `checkpointNode || 'LHR'@26010` — so **a non-primary code can never become
   `currentCode`**, and its `text`/`npc`/`battle`/`loot`/`sleep` never render. Because
   `_uqfActivateAtNode(node)@30138` keys on `node.code`, **1,260 quests carry an `activateNode` on a
