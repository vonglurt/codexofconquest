...
> — `charcreate-sub@4960`, the character-creation screen, which states the entire contract in
> nineteen words.
...
|---|---|---|---|
| 1 | `S_story.tattoos[]` | Survives unconditionally — never touched by the death routine | `const _survivingTattoos@23984` |
| 2 | `careerStats` / `runStats` | `careerStats` snapshot-restored; `runStats` reset *on one path only* (§V-A) | `const _STAT_ZERO = () => ({ kills:0@23974` |
| 3a | `inventory` where `type ∈ {shard, key}` | **Survives** — filtered out before the strip | `const critTypes = new Set(['shard', 'key']);@26098` |
| 3b | all other `inventory` + all gold | Moved to the corpse quest; replaced by a fresh starter dagger | `S_story.inventory = [...critItems, Object.assign({}, STARTER_DAGGER)];@26125` |
...
`function storyRespawnFromCheckpoint()@23982` is **byte-identical** to the original specification
...
`const _S_DEFAULTS = () => ({@23092`. Hard Mode's purpose is ledger legibility: with a flat floor,
...
`_STAT_ZERO()` ships with all 10 specified fields, byte-exact, and `function _statTally(key, n)@23975`
...
| Stat | Site | Anchor |
|---|---|---|
| `battlesAttempted` | all battle types | `_statTally('battlesAttempted', 1);@24708` |
...
| `deaths` | death fall | `_statTally('deaths', 1);@26128` |
| `exitsTaken` | grid move | `_statTally('exitsTaken', 1);@28517` |
...
hook was **re-homed** into the §WALK/§NAV-01 grid mover at `function cellMove(dir)@28495`. Behaviour
...
`S_story.hour = ((S_story.hour || 0) + 6) % 24;@36479`. Verified. The clock is decoupled from the
...
|---|---|---|
| Game-over modal, "This Run" | `runStats`, 9 rows + hit rate | `function _populateGameoverChronicle()@23913` |
| Character sheet, Progression | level-keyed tattoos | `const tattooByLvl = {};@37824` |
| Character sheet, Deaths | death tattoos | `const deathTattoos = (S_story.tattoos || []).filter@37900` |
| Character sheet, Chronicle | both ledgers, two columns | `const isFirstRun@37924` |
| **Inventory sheet, Character Tattoos** | **all tattoos, newest first** | `makeSection('⚔ Character Tattoos');@31458` |
...
`S_story.runStats    = _STAT_ZERO();@23977` and `S_story.hour        = 0;@23994` occur at
**exactly one site each**,
both inside `storyRespawnFromCheckpoint()`. That function has two callers:

1. `btn-gameover-respawn@38523` — the game-over modal button.
2. `_onPitChampionLoss()@28069` — losing the optional Birka pit championship.

Combat death does not reach either. The death-save mini-game resolves to `_storyDeathSaveCrawl()`
(3 successes → survive at 1 HP) or `function _storyDeathSaveFall()@26083` (3 failures). The fall
...
- `name:"Innmother's Key"@23557` — granted when Innmother kindness reaches 5. Carries no `type`, so
  ...
  is a prop, and so is `S_story.innmotherKeyGiven = true;@23558`, a third marker for the same fact
  with no reader either. → **§DX-02n +2**.
- `name:"Glut's Gift", icon:'🍯'@31691` — a **real gated item**, tested by
  `some(i => i.name === "Glut's Gift")@34711` and consumed by `quest_glut_06`. Also untyped, so a
...
`const _STAT_ZERO = () => ({ kills:0@23974` is the specified factory, but
`careerStats: { kills:0@23179` and `runStats:` on the line below hand-write the same ten fields
...
`S_story = _S_DEFAULTS();@23435`, long after the factory is defined. This is the §STATE-INIT drift
...
| 3 | Negative tattoos (scars) | **PARTIALLY, ELSEWHERE.** `Thorn (Permanent)@37873` (§FUTURE-01) is a permanent character-sheet caption — the shape, arriving outside this system. |
| 4 | Cross-run ancestral record in NG+ | **NOT SHIPPED.** `storyNewGamePlus()@24059` calls `_S_DEFAULTS()`, discarding the ledger. The "you fell here before" warning does not exist. |
