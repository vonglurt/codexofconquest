...
|---|---|---|
| `execBits` becomes `function*`, `yield* r` when a handler is itself a generator | yes | ✅ `*execBits(bits, ctx) {@22251` |
| `choice` becomes `*choice`, applying **only** the picked option's bits, **after** the pick | yes | ✅ `*choice(bit, ctx) {@22347` |
| `ask` envelope `{ ask:'choice', prompt, options:string[] }`, never persisted | yes | ✅ still the only `ask` kind in the file |
| Module-level slot holding `{ gen, ask }` | yes, as `_uqfPending` | ✅ `let _uqfPending = null;@6835` |
| `pump(gen, answer)` | shipped **renamed** `_uqfPump` | ✅ `function _uqfPump(gen, answer) {@6838` |
| `runToCompletion(gen)` | shipped **renamed** `_uqfRunToCompletion` | ✅ `function _uqfRunToCompletion(gen) {@6851` |
...
| `BIT_CONTRACTS` untouched | yes | ✅ `const BIT_CONTRACTS = {@21993` |
...
- **`QuestRuntime` is no longer an object literal.** The report anchors *"`QuestRuntime` object 21713."*
  §VM-01-D refactored it into a factory — `const QuestRuntime = createQuestRuntime({@22369` — so the
  runtime could be `require`d by a headless server. **RETIRED, not wrong.**
- **A second, pure `runToCompletion` exists inside the kernel.** `function _questRunToCompletion(gen) {@22059`
...
> `// §VM-01-G4a: the sweep below takes any pending choice panel@34869` — *"Inc A's single module slot
...
`dus-kern-sable-first@34523` renders **3 option buttons** and parks the generator in `_uqfPending`.
...
|---|---|
| **Choice UI content** — a live on-screen `choice` render | ✅ **shipped**, §VM-01-G4a/G4b — but **NOT** under the name this report predicted. `renderChoiceBlock` appears exactly once in the file, in a comment recording that it **never existed as code**. The capability shipped as `function _uqfRenderAsk(gen, ask, mount, step) {@6896` + `function _uqfRunVerb(verb, mount) {@6925`. |
| **Quest-acceptance rewrite** | ⚠️ **not shipped.** The seam exists; nothing has used it for acceptance. |
| **Un-smuggling `skill_check`** | ⚠️ **not shipped, 30 days open.** `resolveSkillCheck(bit, ctx) {@22284` still branches inside the leaf, now wrapped in the pure twin and explicitly scope-fenced: a `choice` in `onPass`/`onFail` throws by design. |
...
- **§DX-02ds** 🟢 — the driver's own safety comment cites *"no autosave (storyAutoSave, 23237) ever
  ...
  is at `function storyAutoSave() {@23842` today. **Wrong when written, by exactly 76 — the
...
- **§DX-02dt** 🟢 — the game's only two authored `choice` bits (`dus-kern-sable-first@34523` and its
