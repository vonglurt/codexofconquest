...
3. Re-derive every quantity with the real parser (`src/js/wbapi-core.js`), never a line regex — a
...
|---|---|---|
| Win condition: 7 Shards + Level 20 + beat the Commander | `function _finalBattleReady(code) {@27999`, and TLS carries `minLevel:20, minShards:7` | ✅ exact |
| Final boss AC 22 / HP 300 | `const BOSS_COMMANDER_AUROS = {@26247` — `ac: 22, hp: 300` | ✅ exact |
| Void Tide on days 3/7/14/21/28/35/42 | `const VOID_TIDE_EVENTS = {@22369` — exactly those 7 keys | ✅ exact |
| Sleeping is the only day advance | `S_story.day = Math.min(49, S_story.day + 1);@36252` | ✅ exact |
| voidPressure 10 = defeat | `S_story.voidPressure = Math.min(10, prev + n);@26979` | ✅ exact |
| XP = AC × maxHP | `function _storyBattleVictory() {@25281` — the code comment reads *"XP = AC × maxHP"* | ✅ verbatim |
| Heal and gold each = 0.1 × AC × maxHP | same function — *"HPGive = goldDrop = 0.1 × AC × HPLoss"* | ✅ verbatim |
| 20 Epic Battleground bosses | `const EPIC_BOSS_POOL = {@26258` — 20 entries | ✅ exact |
| `WEAPON_ITEMS` is 70 | `const _BASE_WEAPONS = [@24472` (14) × `const WEAPON_ITEMS = [0, 1, 2, 3, 4]@24495` | ✅ exact |
| `_magicTierAllowed` = level ≥ magic × 5 | `function _magicTierAllowed(magic) {@24510` | ✅ exact |
| ~2,848 quests | 2,850 at the parent build, 2,853 at HEAD | ✅ within its own hedge |
| 6 curated Birka NPCs | `const birkaNpcs = {@35121` — Yael, Brynn, Quill, Pachelbel, Weckmann, Bruhns | ✅ exact |
...
| **B** | The Conqueror's Hand | ✅ | `0883fa9` spec → `8eb909e` | Void enrage live at `S.opp.enraged@24640`; the mundane-flee half has never once returned true (§DOC-02cf) |
...
- `function _rollMonsterWeaponDrop(monsterDmgDie) {@24582` caps its pool at `magicBonus === 0` and
  degrades it to −4…0. That part the review describes correctly.
- The second vector, `function _rollD100Loot() {@24534`, *does* branch on `dagger` and
  `mainweapon` with a magic tier — but `const _D100_TABLE = [@24517` holds **seven rows totalling
...
- `const DAGGER_ITEMS = [@24459` — the four magic daggers, priced to 8,000 gp — has exactly one
...
pedantry: it is why `const birkaNpcs = {@35121` was broken. The literal keyed Yael to `CI`, so her
...
of §AUDIT-03ab**: `mechanics.md` is listed in `HISTORY_FILES` in `src/scripts/legacy-codes.js`, whose
...
(`const BOSS_COMMANDER_AUROS = {@26247`); *"Commander Auros"* is the identifier and survives in
...
- **The enemy-AI diagnosis is fair and complete.** `function _storyEnemyTurn() {@25192` at the
