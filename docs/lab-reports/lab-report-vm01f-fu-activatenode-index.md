...
|---|---|---|---|
| 1 | **The test harness** | `src/tests/integration/quest-runtime-uqf.test.js` injects `QUEST_DB.__uqf_gate` after the first render, calls `storyCheckQuests` twice (gate closed, then open), then deletes it | ✅ live — this is the exact test the built-once cache broke |
...
of three runs of 1,000 iterations each. `QUEST_DB` census taken twice, once by `src/js/wbapi-core.js` over the source
...
`src/tests/integration/uqf-activatenode-index.test.js`, 4 cases — the plan and the shipped file agree item for item:
...
matches the §7 plan item for item · `src/js/quest.js` untouched.
