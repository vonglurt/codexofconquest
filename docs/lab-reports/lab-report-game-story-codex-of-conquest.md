...
1. **It gives optional content stakes without gating it.** A player who completes zero Epic
   ...
   (`function storyCheckVictory(node) {@28208` returns unless `defeatedBattles['TLS']`). What
...
4. **It makes the ending a record, not a verdict.** `vic-returns').textContent@28224` prints
...
be an ambush. `id:'story-sweelinck-variant'@31375` renders a bracketed Sweelinck line at the
...
|---|---|---|
| Curse Score, derived on demand | `function _curseScore() {@28192` | **Byte-exact to the spec formula** |
...
| `FROBERGER_JOURNAL` | `const FROBERGER_JOURNAL = [@27185` | Exact name; 4/4 field names; **41** entries |
| `SWEELINCK_DIALOGUE_VARIANTS` | `const SWEELINCK_DIALOGUE_VARIANTS = [@27230` | Exact name; 4 specified brackets + 1 |
| Ending variants in the victory sequence | `function storyCheckVictory(node) {@28208` | Live; **four** branches, not two |
| Collectible pickup on node arrival | `function storyCheckJournal(node) {@30210`, called at `storyCheckJournal(node);@36029` | Live; toast **byte-exact** |
| Epic Battleground register | `const EB_NPC_DIALOGUE = {@26300` · `const _EB_CODES = [@28031` | 20 entries, `category` field as classified |
| Covenant Standing ladder | `const COVENANT_STANDING_LABELS = [@27357` | Live, 5 brackets — **not in the spec** |
...
`const allComplete = returnsComplete === 20;@28204`, and its "started" test reads exactly the two
state containers §X specified: `const started = !!(S_story.quests[primaryId])@28198` or a defeated
...
| 9 | Toast *"📖 Froberger's Journal — Entry [N] found"* | `Journal — Entry ' + entry.entryNum + ' found.@30224` | **SHIPPED exact** |
| 10 | Entry 41 sets a terminal flag (implied by *"the spine"*) | `S_story.frobergerLastEntryRead = true;@30216` | Shipped, unspecified |
...
| 12 | Variant selected in `storyRender()` at SQ/CO | `id:'story-sweelinck-variant'@31375`, `nodes:['NUE','TLS']` — SQ→NUE, CO→TLS | **SHIPPED exact** |
...
| 15 | Victory screen shows *Shards Gathered* + *People Returned To* | `vic-shards` prints `7 / 7`; `vic-returns').textContent@28224` prints `N / 20` | **SHIPPED exact** |
...
| 20 | *"Kazrath's journal mentions Birka twice"* (§IV) | `mentions Birka. Twice.@26497` — correct | **SHIPPED exact** |
| 21 | *"mentions Birka forty times"* (Entry 26) | `mentions Birka forty times.@27211` — also correct | **Engine self-contradiction — F5** |
...
| 23 | Epic boss: deadly tier, DANGER:EPIC banner, NPC warning verbatim | Banner revealed at `_epBanner.style.display = '';@24709`, printing `d.warning` + the NPC name | **SHIPPED exact** |
...
`function _curseScore() {@28192` is byte-identical between the archive and HEAD. It never changed,
...
`_storyEbReturnBeat(ebCode) {@30359` — the function §EPIC-01 proved unreachable, because seven code
...
- **§V's thesis is dead with it.** *"The EB return beats are the real victory"* — none of the twenty
  beats can fire, so `vic-returns').textContent@28224` prints **0 / 20** for every player who has
...
- **`SWEELINCK_DIALOGUE_VARIANTS` — 3 of 4 dead.** The selector matches `cs >= v.minScore && cs <= v.maxScore`
  ...
  Birka override (`_birkaVar || SWEELINCK_DIALOGUE_VARIANTS.find@31379`), which fires on three
  friendships regardless of score and was already present at the archive.
- **`const COVENANT_STANDING_LABELS = [@27357` — 4 of 5 dead.** Its brackets are −6 / 0 / 7 / 14 / ∞,
...
+curseScore)"* (`⚡ Destroy the housing — STR Ceremonia Roll DC 15@31827`) and, on success, writes
`S_story.curseScore = (S_story.curseScore || 0) + 5;@31848` and prints *"[curseScore +5]"*.
...
`function storyCheckJournal(node) {@30210` matches `e.nodeCode === node.code`, and only a cell's
primary node can become `currentCode` (`const CELL_GRID = (() => {@9852`,
`S_story.currentCode = destCode;@28374`). Walking all 41 entries against cell primacy:
...
(`mentions Birka. Twice.@26497`) and *"mentions Birka forty times"* in Froberger's Entry 26
(`mentions Birka forty times.@27211`). **Both strings are present at the archive**, so this was born
...
entries (§III of the original) were removed: `const FROBERGER_JOURNAL = [@27185` is the single
