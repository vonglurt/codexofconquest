...
`` `src/js/wbapi-server.js:const NONCE_TTL@48` `` is `5 * 60 * 1000`. Single-use, bound to one
...
> replaced by brace-depth entry bounds (`` `src/js/wbapi-core.js:function findEntryBounds(sectionSrc, entryKey)@385` ``),
> a depth-0 comment- and string-aware field locator
> (`` `src/js/wbapi-core.js:function firstTopLevelMatch(body, re)@305` ``), an escape-aware quote
...
`src/api/wb.js`. Only the arguments rotted:
