...
# Lab Report — The CodexOfConquest.com Documentation System
...
| Monsters (`const MONSTER_POOL = {@5352`) | 370 | **370** | 398 | ✅ exact when written |
| `WORLD_DB` terrains (`const WORLD_DB = {@6280`) | 66 (46 base + 20 epic) | **66** | 111 | ✅ exact when written |
| Story nodes (`const NODE_MAP = {@8437`) | 76 | **76** | 416 | ✅ exact when written · 5.5× |
| State fields (`const _S_DEFAULTS = () => ({@23092`) | 107 | **107** | 318 | ✅ exact when written |
| Froberger entries (`const FROBERGER_JOURNAL = [@27318`) | 41 | **41** | **41** | ✅ **unchanged in 79 days** |
| Named Birka NPCs (`const NPC_DIALOGUES = {@10410`) | 6 | **6** | 204 profiles / 213 dialogues | ✅ exact when written |
...
start of each session: recount lab reports with `ls docs/lab-reports/lab-report-*.md | wc -l`"* — so the
...
FC05's shipped half works exactly as specified. `const S29_AUROS_THEORY =@27184` carries
...
live guard reads the favor ledger: `s29LineDelivered) {@32884` gates on `_npcFavor('auros') >= 2`.
...
The HTML's `FROBERGER_JOURNAL` and its declared home doc `src/sources/froberger-journal-all-entries.txt`
...
The file is additionally **invisible to every gate in the repo**. `src/scripts/legacy-codes.js` drives gate
#16 from an explicit `SWEEP` list of eleven `.md` paths, with `HISTORY_DIRS` covering `docs/lab-reports/`,
`docs/archive/`, `docs/spec/` and others; the string `.txt` does not appear in the script. So all **41** entry
...
`if (g.favorMin)@22087` — a different *shape*, not merely different names. §DOC-02d traced the naming
...
`fav_corelli: 0@23176` is a real `_S_DEFAULTS()` field, read at two sites and written at
`S_story.fav_corelli = Math.min@32027` as `min(3, corelli_purchase_count)` (§DOC-02g verified this
...
The activation-gate reader contains `st.visited || st.visitedNodes@22078`. `visitedNodes` occurs
...
| `froberger-journal-all-entries.txt` | `src/sources/` | relocated; gate-invisible (Finding 3) |
...
| **§AUDIT-03z (a)** | `src/sources/froberger-journal-all-entries.txt` is a live home doc invisible to every gate: 41 retired 26×16 codes in its headers, and entries **17** and **29** carry superseded text. Widen gate #16's `SWEEP` to non-`.md` live docs | No |
...
| `src/scripts/legacy-codes.js` | Gate #16; its `SWEEP` list is the subject of §AUDIT-03z (a) |
