...
3. Reachability was computed from `const CELL_GRID = (() => {@9852` over `NODE_COORDS`, with
   `S_story.currentCode` assigned at exactly two sites (`S_story.currentCode = destCode;@28374`,
   `S_story.checkpointNode || 'LHR'@26010`), then closed transitively over single-writer gate flags.
   `function _uqfActivateAtNode(node) {@30138` keys on `node.code`.
...
| §HUNT-01 | `quest_hunt_01`–`_04`; entry flag `S_story.huntHookReceived = true;@33217` | 4 quests live |
...
| §WISDOM-01 | `quest_wis_00`–`_07`; hub `_runNodeHook('wis-vs-hub', node)@36011` | 8 quests live |
| Token objects | `name:"Smalt's Trust"@12354` · `name:"Bram's Fish Scale"@13021` · `name:"Oat's Harbor Bead"@33806` · `name:'Dunfall Drift Spore'@13073` · `name:'Highland Letter of Clearance'@33785` | all 5 live |
...
| 2 | Table I row 14 — §WISDOM-01 W6, "Accept the shadow", WIS **DC 14** | `quest_wis_06` is `type:'side'`, `bits:[]`; the accept path is a button (`Accept the reflection (receive the shadow@33471`) that always succeeds | ❌ **NOT SHIPPED — born so** (absent at `e339aeb` too) |
...
| 14 | §X: "The cook has never apologized. Ord has not asked for one." — quoted twice as corpus text | Live string: `Ord the passenger is owed an apology@33610` *"…the cook has declined to give."* The first half is a paraphrase; **the second half has no source in the file** | ❌ PARAPHRASE + fabrication |
...
|---|---|---|---|
| `wisHookReceived` | `S_story.wisHookReceived = true;@33502` | `if (node.code === 'VS' && S_story.personalLegendComplete) {@33436` | **All 8 §WISDOM-01 quests dead** — `wis_01`/`_02`/`_03` sit on *live* nodes and can still never open |
| `visbyUnderground` | `S_story.visbyUnderground = true;@33421` | `node.code === 'VS'` | `activateNode:'VS', gate:{ flags:['visbyUnderground'] }@13479` — W6 dead a second way |
| `huntHookReceived` | `S_story.huntHookReceived = true;@33217` | `if (node.code === 'HFT' && !S_story.huntHookReceived) {@33206` | **All 4 §HUNT-01 quests dead** |
| `whodunitSolved` | `S_story.whodunitSolved = true;@12436` | inside `quest_spark_04` (`activateNode:'SEN'`) | `quest_spark_05` dead; **the Aldous confession branch `S_story.whodunitSolved && S_story.wrenpemburyInconsistencyNoticed && !S_story.aldousConfessed@33564` never fires** |
| `roenAtSea` | `S_story.roenAtSea = true;@33372` | `node.code === 'SEN'` | `activateNode:'PDL', gate:{ flags:['roenAtSea'] }@13251` and the whole §ALCHEMY relay below it dead |
...
`label:'Shadow — The Mirror Construct'@33488` (`key:'shadow'`, ac 12), whose defeat sets the same
...
ends: **`W6 here (WIS 14 or combat)@33494`**. Its first five entries — W1 WIS 13, W2 WIS 12, W3 INT
...
| **§AUDIT-03ad (NEW, 🟢 no design call)** | `W6 here (WIS 14 or combat)@33494` advertises a DC that has never existed. Fix is either a `skill_check` bit or a two-word string edit — **but the string's other five DCs are exact, so the detector wanted is "a DC named in prose with no matching `dc:` in the quest it names"** |
| **§AUDIT-03s extended** | `Ord the passenger is owed an apology@33610` — the §WHODUNIT-01 resolution line renders only at non-primary `SEN` |
