...
At `a105e0e^`, `src/js/wbapi-server.js` line **8899** opened the PUT field loop, and line **8902** split it
...
`editField` cannot cover the gap either: `src/js/wbapi-core.js:function patchStringField(sectionSrc, entryKey@345`
...
**`src/scripts/check-array-patch.js`** (`npm run check:arraypatch`), which loads the committed HTML
