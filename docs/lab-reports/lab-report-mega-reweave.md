...
**`PRIORITY_HIGHWAYS`** — *"edit the array in `src/api/wb.js`."* Correct (3 hits, CLI-side). But **only 2 of
...
report's own birth commit (`src/api/wb.js`, the values actually sent to the server on every invocation):
...
`PUT /api/roads`, which shells out to **`src/scripts/build-roads.js --apply`**. It is the §DOC-02c
...
**The old bounds did not all retire.** `src/tools/worldmap.js:224–227` still carries
...
| **§DX-02bh** | 🟢 no design call | `src/tools/worldmap.js`'s `MAP` bounds are a fossil of the retired 8–500 box-fit projection. `PDL` (Ponta Delgada, lon −25.7) is 1 of 155 `GEO` cities outside `minLon:-25` and is silently dropped from every region of `worldmap --regions`. |
...
(§AUDIT-03n / `src/scripts/legacy-codes.js` HISTORY class).*
