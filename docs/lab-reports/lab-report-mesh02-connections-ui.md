...
### A. Server surface (`src/js/wbapi-server.js`; at the root as `wbapi-server.js` until `5e48dd7`, 2026-07-09)
...
| `shareBlocklist` ignored by `aclAllows()` | 0 occurrences in `src/js/mesh.js` | ✅ exact |
...
| §3.1 header path `wbapi-server.js` | `src/js/wbapi-server.js` since `5e48dd7` (2026-07-09) | ⓘ **correct for its day**, stale for 40 days |
...
| No LAN broadcast discovery | ✅ held — `dgram` occurs **0** times in `src/js/wbapi-server.js` and `src/js/mesh.js` |
...
5. **D7 measured, not asserted.** `grep -cE "msub|mpBlacklist|MP\.|mesh" src/js/mover.js` → **0**.
