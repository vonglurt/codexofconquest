...
The repo says so in its own words. The fix arrived at §ARCH-01 Wave 8b, `11af1e5` (2026-07-03 20:57), which added `src/js/wbapi-server.js:const JSONF = ['gate','bits','completion','itemChain'@1713` and recorded in its commit subject: *"all silently dropped before — API-posted UQF quests arrived stripped dead."* **Exposure window: 6 d 4 h 18 m** from inc3.
