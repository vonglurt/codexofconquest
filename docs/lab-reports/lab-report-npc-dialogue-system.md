...
- **It gives repeat visits a reason.** `return { quote: pool[count % pool.length], meta: d.meta, fav };@23663`
...
- **It converts kindness into an ending.** `function _missionComplete() {@23696` counts twelve acts that
...
`const NPC_DIALOGUES = {@10410` (a `◆◆◆ WORLDBUILDER ◆◆◆` data section, so it is API-authorable).
...
### B. The selector — `function _getNPCDialogue(npcKey) {@23582`

Pool order at HEAD is `if (fav >= 2) pool = d.dearFriend;@23591`, then `questActive`, then `friendly`
...
### C. The mission ledger — `function _missionComplete() {@23696`

Twelve named bits, `return Object.values(bits).filter(Boolean).length >= 8;@23698`. **Byte-identical to
...
`if (missionDone && curse <= 0) {@28393` → Covenant Keeper · `else if (curse <= 0)` → the Capable Warden ·
`} else if (curse >= 15) {@28407` → the Loop Continues · `else` → the Imperfect Covenant. Plus a stricter
...
`disposition: '"The commissioners don\'t read field reports about the Slums.@21251` — Yael's grievance,
...
| 3 | Cycling by `visitCount % pool.length`, not random | `pool[count % pool.length]@23663` | ✅ **EXACT, 82 days** |
| 4 | 4 favor states numbered 0/1/2/3 | **three** numbers; Friendly = 1, Dear Friend = 2 | ⚠️ compressed (§V-B) |
| 5 | World Truth footer at "favorability 2 (Friendly)" | `if (fav >= 2 && dlg.meta && dlg.meta.worldTruth) {@23799` | ⚠️ literal shipped, **tier moved** (§V-B) |
...
| 11 | "Deacon", the BA fence with a code on the wall | the **code** shipped, owned by Pachelbel — `const DEACON_CODE_TEXT =@27682` | ⚠️ name demoted to an artifact (§V-C) |
| 12 | Nodes CI · IN · TV · BA · CY · SQ | `LHR` · `TLL` · `MHQ` · `LLA` · `HKG` · `NUE` | ⚠️ 0 of 6 resolve, all 6 correctly remapped |
| 13 | Crov and Auros share a node | `HKG:['crov','auros']@35347` | ✅ EXACT — the one geography claim that holds |
...
| 15 | Bit `weckmannPitTrainingDone` | **0 occurrences**; ships as `crovPitTrainingWins@23680` | ❌ renamed to the profile key |
| 16 | Bit `couperiSongReceived` | **shipped, typo and all** — 9 occurrences (§V-C) | ✅ EXACT |
| 17 | Bit `ebReturnsCompleted >= 20` | `allEbReturns@23688` reads `ebReturnDone`, threshold **5** | ⚠️ and it minted a dead field (§V-D) |
| 18 | Bit `journalEntriesRead >= 17` (a number) | `journalHalf@23689` — an **array**, `length >= 9` of 41 | ⚠️ type + threshold changed |
...
`if (fav >= 2 && dlg.meta && dlg.meta.worldTruth) {@23799` — **the literal is byte-faithful to the spec
...
- `crov` — key wins, flag renamed to `crovPitTrainingWins@23680`.
...
Nine occurrences at HEAD, including `S_story.couperiSongReceived = true;@35507` and a `_S_DEFAULTS()`
...
it"* shipped as `const DEACON_CODE_TEXT =@27682` — *"Deacon's Code (4 rules; readable at BA; Pachelbel
Dear Friend)"* — with `function _nodeHookBirkaDeaconCode(node, { npcRowDiv }) {@32342` rendering it at
...
return — and `allEbReturns@23688` reads **`ebReturnDone`** instead. `ebReturnsCompleted` has **12 fewer
...
`allEbReturns: Object.keys(NPC_DIALOGUES).length > 0@23688` — a conjunct over an immutable non-empty
literal, so it is **always true and tests nothing**; and `journalHalf: (S_story.journalEntriesRead || []).length >= 9,@23689`
is named *half* while asking for 9 of `FROBERGER_JOURNAL`'s 41 entries — **22 %**. Also
`returnedToCI: !!(S_story.visited && S_story.visited['LHR'] && S_story.level >= 5),@23693`: a dead node
...
`function _curseScore() {@28340` scores each of the twenty `const _EB_CODES = [@28171` as *returned*,
*started-not-returned* (×3) or *never started* (×1). `returned` reads `S_story.ebReturnDone[code]`,
written **only** by `function _storyEbReturnBeat(ebCode) {@30516`, called **only** from
`c.addEventListener('click', () => _storyEbReturnBeat(ebCode));@36086`, which renders only when
`const returnId   = 'quest_' + ebCode.toLowerCase() + '_return';@36073` is active — i.e.
`quest_prn_return`. `QUEST_DB` holds `quest_ef_return` … `quest_eg_return`, the **retired EA–ET codes**.
The same mismatch sits in `const primaryId = 'quest_' + code.toLowerCase() + '_primary';@28345`.
...
never renders, and the three score-banded `const SWEELINCK_DIALOGUE_VARIANTS = [@27363` below 15 are
...
`_missionComplete()` takes collateral damage from the same cause: `noHighCurse: _curseScore() < 10,@23692`
...
| **§EPIC-01** annotated | 🔴 | The id mismatch is at two computing sites, not one: `const primaryId = 'quest_' + code.toLowerCase() + '_primary';@28345` and `const returnId   = 'quest_' + ebCode.toLowerCase() + '_return';@36073`. Fixing only the primaries leaves `ebReturnDone` unwritable and the curse floor intact. |
| **§DX-02n** +2, and one **origin** | 🟢 | `ebReturnsCompleted`'s origin traced to §IV of this report. New: the always-true conjunct in `allEbReturns@23688`, and `journalHalf@23689` — a bit named *half* that asks for 9 of 41. |
| **§AUDIT-03s** family +1 | 🟢 | `returnedToCI@23693` — a retired node code preserved in an **identifier**, invisible to every gate by construction. |
...
`const npcQuests = { yael:['quest_slums_cleanup','quest_city_watch_patrol']@23666` is still dangling but
is already §DX-02o.
...
Live anchors: `const NPC_DIALOGUES = {@10410` · `function _getNPCDialogue(npcKey) {@23582` ·
`function _missionComplete() {@23696` · `function _curseScore() {@28340` ·
`const SWEELINCK_DIALOGUE_VARIANTS = [@27363` · `const DEACON_CODE_TEXT =@27682` ·
