...
|------|-------|-----|---------|--------|--------|
| WG0 | The Deeper Gate | 121 | `hag_swamp` | none | `` `WG0:{ num:121@8569` `` |
...
| HCA | The Deeper Clearing | 129 | `hag_swamp` | none | `` `HCA:{ num:129@8601` `` |
...
`waneStoneDCReduced@23234`, `electricEelOrganHeld`, `atlanteanProcessKnown`,
`iodineBuffActive` — plus one the amendment never named, `iodineBuffBonus`.

### C. Helpers

`` `function _innKindness(n)@23551` `` and `` `function _addCroneMark()@23561` `` are
...
`` `function _burnIodineSalt(questId)@23566` `` — the §A.5 "burn a dose" sketch,
...
|---------|-----------|--------|
| HG1 first-visit gift | `NODE_PANELS` entry, `once:'glut_gift_held'` | `` `id:'story-hg1-gift'@31686` `` |
| HCA mark conversion | `NODE_PANELS` entry, `once:'croneMarksBanked'` | `` `id:'story-hca-marks'@31695` `` |
| WG0 gate trial | `NODE_HOOKS` block | `` `_wg0Div.id = 'story-wg0-trial'@33305` `` |
| HCA Leviathan | `_nodeHookHcaLeviathan@33322` | — |
| INN free sleep | inline cost override | `` `node.code === 'INN' && S_story.freeBookingUnlocked@36394` `` |
...
| 5 | III.D, IV | Progression gate `activateCond: () => (quests['prev']\|\|'') !== ''` — "attempted, pass or fail both qualify" | Shipped as `gate:{ questsAttempted:[…] }`; the compiler is **`(QS[id] \|\| '') !== ''`**, `` `g.questsAttempted@22083` `` — the same predicate, verbatim | SHIPPED (renamed) |
...
| 16 | A.4 | Sea Spawn kill drops `quest_wane_02`'s DC from 13 to 10 | `waneStoneDCReduced` is **written once and read nowhere** (`set:['waneStoneDCReduced']@12009` → `waneStoneDCReduced: false@23234`, no third site). `quest_wane_02` is `retryable:false`, so there is no retry to discount either way | **NOT SHIPPED** → §DX-02n |
| 17 | A.5 | Iodine Salt +3 / Charged +5 to a skill check, spent before the roll | Live: `` `const iodineBonus = st.iodineBuffActive@22277` `` adds it inside `_rollSkill` and clears it | SHIPPED |
| 18 | A.5 | Marsh Seaweed 🪸 — a second gathered reagent | The item drops (`` `name:'Marsh Seaweed'@7073` ``, 5 monsters) and its `desc` says *"Burns to trace iodine salt at the inn hearth."* **It occurs exactly once in the file. No quest, craft or handler consumes it** | **SHIPPED HALF** → §DX-02n / §AUDIT-03v cluster |
...
| 21 | A.7 | Leviathan defeat is the precondition for the conversion | Exact: `` `id:'story-hca-marks'@31695` `` requires `defeatedBattles['HCA_BOSS']` | SHIPPED |
...
`` `const CELL_GRID = (() => {@9865` `` builds each cell as an **array** in `NODE_MAP`
declaration order, and `` `CELL_GRID[key]?.[0]@9874` `` — the primary — is what the
browser reads. `S_story.currentCode` is assigned at exactly two sites in the file
(`` `S_story.currentCode = destCode@28525` ``, where `destCode = res.destCodes[0]`,
and `` `S_story.currentCode = S_story.checkpointNode@26142` ``), both yielding the
...
- `_uqfActivateAtNode@30293` keys on `node.code`. **HW1 carries 8 quests, HG1 8 and
...
3. **§A.4's thesis — "a failed skill check does not strand the player, it redirects
   ...
   distinction — `` `S_story.quests[questId] = 'failed'@7012` `` — but the activation
...
| **§DX-02r** (NEW) | The activation-gate grammar can express *attempted* and *done* but not *failed*, though `'failed'` is a real written status — so three quests authored as failure consolation activate for everyone. Wants a `questsFailed` leaf beside `` `g.questsAttempted.every@22083` ``. |
