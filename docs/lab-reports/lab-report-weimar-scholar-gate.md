...
`` `key:'scholars_guard'@5403` `` — all eight fields exact as specified. Drop
`` `icon:'🔏', sell:20@5848` `` → *Scholar Kings' Seal*, exact. Terrain
`` `P.bone_naga, P.scholars_guard@6292` `` — present in `scholars_qtr` alongside the homunculi,
...
`` `function _tomeBonuses()@23439` `` is byte-identical to the report's listing modulo the
whitespace of two `if` bodies. All three integration points survive:

| System | Live anchor | Shipped expression |
|---|---|---|
| Initiative | `` `const _tomeInit = _tomeBonuses().initiative@7451` `` | `d20 + dexMod + tome initiative` |
| Death saves | `` `_tomeBonuses().deathSave + _kingsSealBonus@7515` `` | exactly as specified, including the Corelli seal addend |
| Attack | `` `_tomeBonuses().atk + _lakeMagicBonuses().atk@25080` `` | as specified, plus a §DROP-03 addend added later |
...
`` `wmDoc1Read: false@23162` `` through `` `wmFirstResearcherKnown: false@23164` ``, plus
`archiveLetterObtained` in the same section. Not one was renamed by §ARCH-01 or §VM-01.

### D. Archive interface

`` `const WM_ARCHIVE_DOCS = [@27928` `` — three documents, keys and titles as specified, both
...
`` `function _storyWmArchiveModal(wrap)@27950` `` — toggle-style, read/unread colour coding
...
The launcher is now a registry entry: `` `id:'nue-lower-archive', nodes:['NUE']@34417` `` →
`` `function _nodeHookNueLowerArchive(node)@33198` ``, still gated on `wmLowerArchiveUnlocked`.
...
`` `bonus:{initiative:2}@11113` ``, `` `bonus:{atkWhileQuestActive:1}@11123` ``, all
...
`` `yealFav >= 1@32504` `` → `` `S_story.archiveLetterObtained = true;@32512` ``, at the Blue
Shutters Archive, on the starting node. **A design decision that survived 80 days and a world
migration without a single edit.**

### G. Geography

The node is `` `NUE:{ num:35, code:'NUE'@8718` `` — same `num`, same `name:'scholars_qtr'`, same
`` `label:"Scholar's Quarter — Weimar"@8718` ``, same `act:6` as the report's `SQ`. **The node
survived; only the key was renamed.**

**Reachability: clean.** `` `NUE:{r:20,c:191}@9628` `` is the **sole occupant of its cell**, so it
is `list[0]`, it can become `currentCode`, and all five `activateNode:'NUE'` quests activate on
arrival — `` `activateNode:'NUE', // §VM-01-G3@11094` ``. Unlike §CROWN-01, **this arc is not a
...
| 8 | §IV: "no notification prompt" for the unredaction | correct; and the `` `Read (unredacted)' : '📄 Read'@27991` `` label is itself unreachable (Finding 2c) | **correct, and worse than stated** |
...
game's real clock is `S_story.day` (`` `S_story.day + '/49'@36280` ``) and `S_story.gameDay`
(22 sites) — **and this report names `gameDay`, correctly, twice.**

So `today` is `0` on every render, forever. The first click runs
`` `S_story.wmSessionsDays.push(today);@34996` `` and thereafter `sessions.includes(0)` is
permanently true. The button locks at 1/3 reading *"📖 The circle meets again tomorrow."* — a
promise made by a clock that does not tick. `` `if (n >= 3) S_story.wmBenediktCircleComplete = true;@35004` ``
never fires.
...
`` `hint:'Return to the lower archive and read the unredacted personnel file.'@11125` ``. Reading
...
(`` `const savedNgRun@24083` `` and its neighbours) and resets the rest from `_S_DEFAULTS()`.
