...
|---|---|---|
| `BIRKA_NPC_PROFILES`, 6 entries, fields `{key, name, occupation, node, neutral, friendly, dearFriend}` | **exact** — all six shipped, all fields present, `special` used exactly once (Yael) | `const BIRKA_NPC_PROFILES@22741` |
| `S_story.npcFavorability` — `npcKey` → 0 / 1 / 2 | **exact**, declared in `_S_DEFAULTS()` | `npcFavorability: {}@23082` |
| `_setNpcFavor(npcKey, level)` + its two log lines | **exact**, both strings verbatim: *"looks at you differently now"* / *"says your name when you walk in"* | `function _setNpcFavor@23505` |
| `_npcFavor(npcKey)` selects the card variant | **exact** — 42 call sites | — |
| `_renderNpcCard(npcKey)` | **exact** (signature grew a `container` argument) | `function _renderNpcCard@23718` |
| `S_story.roughWhiskeyUsed` — one-time drunk-fight latch | **exact** | `roughWhiskeyUsed: false@23083` |
| `S_story.yaelEscortUsed` | **exact**, and load-bearing in four more places than specified | `yaelEscortUsed: false@23084` |
| Rough Whiskey — 5gp at the bar, `consumable_misc`, sell 2gp | **exact, all three** | `Rough Whiskey',icon:'🥃',type:'consumable_misc',sell:2@24444` |
| Six of seven quests: slums cleanup · ledger · lute · shipment · pit training · void below | **all live as UQF-1.0** | `quest_slums_cleanup@21245` … `quest_void_below@21345` |
| Gold rewards 80 / 40 / 60 | **exact, all three** | `gold:80@21246`, `gold:40@21291`, `gold:60@21307` |
...
| Pachelbel hands the lute over on request, no fight — with his line verbatim | **exact**, including *"it didn't look like something that should be left under a chair in here"* | `key === 'pachelbel' && S_story.quests@23806` |
| Sweelinck's four curse-score variants **plus** a fifth Birka variant at ≥3 friends | **exact structure** — 5 entries, the 5th flagged `birka:true` | `const SWEELINCK_DIALOGUE_VARIANTS@27363` |
| Player starts with 150gp | **exact** | `gold: 150@23034` |
| Auros → Dear Friend on Void Below; the arc's only fav-2 grant | **exact** (`{kind:'favor', npc:'auros', set:2}`) | `quest_void_below@21345` |
...
| 2 | **`quest_drunk_fight`** — the 7th quest, "auto-trigger, no activation" | **NOT SHIPPED — and left dangling** | No `QUEST_DB` entry; `git log -S` = 1 commit, the reference itself. It is listed in `_hasActiveQuestFor`'s `npcQuests` under `crov:['quest_pit_training','quest_drunk_fight']@23668`, so the lookup is permanently `undefined` and the entry is inert. **The drunk fight shipped as an engine event, not a quest.** See §V-3. |
| 3 | **Drunk Pit Fight mechanics** — disadvantage on all attacks · +3 damage on a hit · one-time · item consumed | **SHIPPED, exactly** | `if (S_story._drunkFight) adv = 'dis'@25094`; `const drunkBonus = S_story._drunkFight ? 3 : 0@25121`; latch + inventory filter at `roughWhiskeyUsed = true@24740`. |
...
| 6 | **First drunk win grants the Pit Legend Token early**, regardless of Quest 5 | **NOT SHIPPED** | The victory branch does exactly one thing beyond the message: `if (_npcFavor('crov') < 1) _setNpcFavor('crov',1)@25467`. **No token.** The favor half of the claim shipped; the item half did not. |
...
| 10 | **Brynn's Ledger reward: "free lodging for 3 nights"** | **NOT SHIPPED** | `onComplete` emits *"🛏 Brynn offers 3 free nights"* and stops. No counter, no flag, no state. `storySleep` charges `node.sleepCost` unless `node.code === 'INN' && S_story.freeBookingUnlocked@36394` — an unrelated flag on a different node. **TLL charges `sleepCost:5` before and after the quest.** See §V-2. |
...
| 12 | **Rough Whiskey NPC reactions** — two authored (Quill refuses; Weckmann *"After the fight."*) | **CHANGED — expanded 2 → 18** | `const ROUGH_WHISKEY_REACTIONS@27498` covers **all six** NPCs × three tiers, read through `_checkRoughWhiskeyReaction@28301` and gated by a *second*, separate flag `roughWhiskeyActive` cleared on sleep and on battle victory. **Neither authored line survives verbatim** — Quill's became *"the lute strings are sensitive and I can smell everything from up here."* |
| 13 | **Three favorability states** — neutral / friendly / dear friend; **"one quest is enough"** | **CHANGED — four tiers, and two ways up** | The dialogue registry is keyed `impartial · questActive · friendly · dearFriend`; **`questActive`** is a fourth tier the report does not specify, selected by `_hasActiveQuestFor@23665`. §NPC-01-D then added a second route to Friendly with no quest at all: `TALK_TO_FRIENDLY = 3@23535` deliberate talks on distinct game-days. Fav 2 remains quest/personal-act earned, so the report's *"one quest is enough"* is now *"one quest **or** three conversations."* |
| 14 | **`S_story.lubeckFriends`** — a derived count state field | **CHANGED** | Shipped as a **function**, `function _lubeckFriends()@23493`, not a field. **The name is a fossil**: the city is Birka, and `Lübeck` survives only as an unrelated live node, `LBC` — the Hansa Gate, Act 2. A reader who greps `lubeck` lands in the wrong city. |
| 15 | **Escort mechanic** — a *"special node-description overlay on the corridors from CI"*, firing on the **next move** to an adjacent node, with the blue-shutters / Varga's-pigeons narration verbatim | **CHANGED — three ways** | (a) There are **no corridors** — §WALK/§NAV-01 deleted that layer entirely (§DOC-02c delta 3), so it renders as a **modal fired on click**, `function _yaelEscortAction@23833`; (b) the narration is a **different, shorter text** — Skalder's corner, Nivers at the fountain, the unread riot report — none of the specified paragraphs shipped; (c) `yaelEscortUsed` and the *"🛡 Walk the beat"* re-view both shipped exactly, and the flag grew **four more readers** the report never anticipated: Yael's dear-friend second act, a `_missionComplete` bit, a journal beat, and an Act-5 records line. |
| 16 | **Void Below combat: "Void Horror or equivalent, drawn from the WORLD_DB void pool"** | **CHANGED** | *"Void Horror"* has **0 occurrences**. The fight is a **synthetic battle code**: `code:'CY_VOID'@34409` — two `void_walker`, classified in `check-noderegs.js`'s `SYNTHETIC_BATTLE_CODES` as *"Layer 41 — Void Below descend at HKG"*. |
...
previously rendered nowhere"* (`const birkaNpcs@35347`, §PLAY-01-G). These codes came from the
...
1. **§AUDIT-03u — the world has 416 nodes; two player-facing strings still say forty-two.**
   ...
   Real Map beside it — **all forty-two nodes** — and pushes it toward you"* (`STN:{ num:9@8649`),
   and Yael's Level-1 tutorial monologue opens *"Check your MAP. The known world has **forty-two
   nodes** and you can get lost in it, badly, fast"* (`yael: { meta:@10411`). Forty-two was the
...
2. **§AUDIT-03v — Brynn promises a free room twice and charges 5gp both times.** `TLL`'s node text
   has her say *"'Free,' she says. 'The room's free too, if you need it.'"* (`TLL:{ num:2@8444`),
   ...
   *different* node (`node.code === 'INN' && S_story.freeBookingUnlocked@36394`). So the game states
...
3. **§DX-02o — one quest-id string literal in the engine resolves to nothing, and no gate can see
   it.** `quest_drunk_fight@23668` has no `QUEST_DB` entry; `S_story.quests['quest_drunk_fight']` is
