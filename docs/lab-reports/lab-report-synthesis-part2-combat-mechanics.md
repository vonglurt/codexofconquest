...
4. **Read the source reports at the birth commit** (`git show 6be4180:docs/lab-reports/…`) — 6 of the 7
...
- **`_onStoryVictory()` — 0 commits in the file's entire history.** The victory host is and always
  was `function _storyBattleVictory() {@25281`. This is **inherited, not invented**: §DOC-02v found
...
| Claim | Verdict |
|---|---|
| `XP_LEVELS` — 20 levels, 0 → 195,000 | ✅ **exact, and byte-identical at HEAD** (`const XP_LEVELS = [@24419`) |
...
| *"specific values may have been rebalanced"* | ❌ **hedge, and wrong** — all 12 entries **byte-identical** from the earliest surviving build to HEAD (`const CONDITION_GOLD = {@24619`) |
...
| *"`_lu_applyGiftsAndFinish()` awards HP, gold, shield, tattoo"* | ⚠️ **half right** — it applies gold, the shield gift and the `levelUpLog` row (`function _lu_applyGiftsAndFinish(lvl, hp) {@38470`); **HP is applied by the modal's roll handlers** and the tattoo is written at a different site |
...
| *"the 1.5 AP economy is the structural spine"* | ✅ enforced at every consumable (`function _storyUseFlashbang(invIdx) {@24935` guards on `usedMainAttack`/`usedBonusAction`) |
...
| Attributed to `_onStoryVictory()` at 23,244 | ❌ function **never existed**; the formula is at the head of `function _storyBattleVictory() {@25281`, 99 lines above the cited line |
| Checkpoint respawn preserves tattoos | ✅ exact, and byte-identical to its 2026-05-25 spec (`function storyRespawnFromCheckpoint()@23923`) |
| **Necklace: *"first arrival at each node awards a Knowledge Bead"*** | ❌ **INVERTED MECHANISM.** Beads are awarded on **rest**, not arrival — two call sites, both rest paths (`function _maybeAddKnowledgeBead(nodeCode) {@25809`, called from `storyShortRest` and `storyConfirmSleep`). **The source report says "rests" five times.** |
| *"Short rests (3/day, 50gp, partial heal)"* | ⚠️ 3/day ✅; **no gold cost has ever existed** (`shortRestCost` = 0 occurrences, then and now); heal is 25 % of max HP **at an inn** and **50 % away from one** (`const heal  = isInn ? base : base * 2;@25840`) |
| Long rest = full heal + day advance, cost varies by node | ✅ `sleepCost` 3–5gp per inn node |
| *"`void_mercy_count` prevents the Void Tide from killing a player who is actively playing"* | ❌ **materially overstated** — it is **one** reprieve, granted only at pressure 9 **and only with ≥5 of 7 shards** (`S_story.void_mercy_count = (S_story.shards >= 5) ? 1 : 0;@26985`), spent at the next tide with the line *"💤 You sleep fitfully. The Void holds its breath with you."* A player at 4 shards gets nothing. |
...
*Source: `lab-report-loot-drop-weapon-economy.md` (2026-05-21) — a proposal, superseded by Register 4. Now in `docs/archive/`.*
...
| `_magicTierAllowed()` gates on `S_story.level` | ✅ exact and unchanged: `return (S_story.level || 1) >= magic * 5;` (`function _magicTierAllowed(magic) {@24510`) |
| Offhand dagger and shield mutually exclusive | ✅ **enforced in both directions and twice over** — equipping either returns the other to inventory *("Dagger moved to inventory" / "Shield moved to inventory")*, and the offhand attack refuses outright while a shield is worn (`if (S_story.equippedShield) { _sboLog('🗡 Unequip shield to use offhand.'); return; }@25106`) |
...
| `_rollMonsterWeaponDrop` → 1 base weapon, quality −4…0, never positive magic | ✅ exact at HEAD, comment included: `// FC06: monster drops capped at base tier` (`function _rollMonsterWeaponDrop(monsterDmgDie) {@24582`) |
| `LAKE_MAGIC_DB` the exclusive positive-magic source | ⚠️ true of the data; **no live grant path at HEAD** — see §VI |
| `LOOT_TABLE` *"dead code … marked in a comment"* | ❌ **the comment says the opposite.** `const LOOT_TABLE = [@24442` is annotated *"(d100 result → item; used by `_rollD100Loot()`)"* — naming a caller that reads `_D100_TABLE` instead. Zero readers, a comment asserting one. → **§DROP-01-FU (a)**, open |
...
| Section header *"⚫ Your Tattoos"* | ❌ **0 occurrences, then or now.** The section is `makeSection('⚔ Character Tattoos');@31246` |
| Time-of-day: `hour` +1 per battle, +6 per sleep, 0–23 | ✅ both exact — the +1 fires at battle **start** (`function _storyRollInit() {@24625`), not at victory |
...
| `kenickieMarketUsed = true` on first purchase | ✅ exact (`S_story.kenickieMarketUsed = true;@33038`) |
| No quest entry, no map marker | ✅ still true |
| *"run stats first ('This run'), career stats second ('All time')"* at game-over | ❌ **three ways wrong.** The game-over modal shows **one** column — `<div class="goc-title">This Run` — with **nine** rows and **no career figures at all**, then and now (`function _populateGameoverChronicle() {@23853`). The two-column ledger lives on the **character sheet** (`function storyRenderCharSheet() {@37591`) and its headers are **"This Life" / "All Lives"**, the second suppressed until the first death. |
...
the Boyscout Token for a first short rest at a node (`// Boyscout Token — first short rest at this location@25850`)
...
never drained, and `function _grantExplorationXp@30078` opens by clearing it, so the next step onto
...
death save is the same shape: `let d20 = Math.ceil(Math.random() * 20) + _luckMod();@25897` decides
...
`// §D02 xpAward on side-type quest completion@30202` in the quest-completion loop, shipped
