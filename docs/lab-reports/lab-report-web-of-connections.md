...
*CodexOfConquest.com / Codex of Conquest — Layer 45 design lock, re-verified against the live engine*
...
| VI | Training log | ✅ | Full log, Bruna's entries, the two-year gap, player entries keyed to `pitTrainingWins`, plus a Layer 64 championship entry. **F5 closed 2026-09-07 (§DX-02fg)** — the stage direction is four authored entries; **its header closed 2026-09-08** — the title names Weckmann, not the `crov` key (§DX-02jq), and the pit by its label rather than its retired node code (§DX-02jr) |
...
Anchors at HEAD: `function _checkFrobergerTrace(npcKey)@27789` · `function _getYaelLocation()@27801` · `function _buildWeckmannLog()@27808` · `const FROBERGER_TRACES = {@27826` · `const NPC_CROSS_REFS = {@27835` · `const NIVERS_DIALOGUE = "Evening.";@27867` · `const YAEL_PATROL_NODES = [@27869` · `const WECKMANN_TRAINING_LOG =@27877` · `function _npcFavor(key)@23492` · `function _setNpcFavor(key, level, say)@23527` · `function _checkDearFriendUpgrade(key, say)@23538` · `function _nodeHookBirkaNiversPasses(node,@32443` · `function _nodeHookBirkaYaelPatrolLine(node,@32733` · `function _nodeHookBirkaRoom6(node,@32993`.
...
Every write to `npcFavorability` in the file resolves to four sites: `_setNpcFavor`'s own assignment, two hardcoded auto-upgrade lines, and one unrelated NPC. The two auto-upgrade sites — inside `function _setNpcFavor(key, level, say)@23527` and `function _checkDearFriendUpgrade(key, say)@23538` — are identical and both write the literal **2**:
...
|---|---|---|---|---|
| `yael` | `set:1`, `add:1` (cap 3) | `_setNpcFavor('yael', 3)@21452` (Ceremonia, CHA DC 15) | **3** | patrol entry 4 ✅ |
| `brynn` | `set:1`, `add:1` (cap 3) | — | **3**, order-dependent | Froberger trace ⚠️ · Room 6 ⚠️ |
| `quill` | `set:1` | — | **2** | — |
| `pachelbel` | `set:1` | — | **2** | — |
| `crov` | `set:1` + `add:2` (§DX-02fb) | `_setNpcFavor('crov',1)@25467` | **3** | Froberger trace ✅ · Layer 44 `weckmann_class` ✅ |
| `auros` | `set:2` | — | **2** | — |
...
The lock writes `[years of entries — fighters' names, brief notes, outcomes]` as an authoring instruction, in the same bracket convention it uses for `[gap of two years]`. The first is a note to whoever fills the log in; the second is prose. `const WECKMANN_TRAINING_LOG =@27877` shipped **both** verbatim, and `_buildWeckmannLog()` substitutes only `{PLAYER_ENTRIES}` and `{CHAMP_ENTRY}`. Measured: the rendered log contains the literal string `[years of entries — fighters' names, brief notes, outcomes]`.
...
> **✅ SHIPPED 2026-09-07 (§DX-02fg) `fdae2b8`.** Four entries covering Year 15 to Year 22 — the *"running legal fights only"* era between the gap and the player's own first line — with three fighters named from the same 17th-century organist register the log already uses for Cabanilles and Bruna: **Reincken, Kerll, Roberday**, none of which occurs anywhere in `play.html` or `docs/`. Driven through the real `_buildWeckmannLog()` in Node before and after, at 0 wins and at 5 wins + champion: **bracketed spans 2 → 1**, the survivor being `[gap of two years]`, the prose one. **Written against the corpus rather than freely, because Weckmann's own dialogue constrains it in four places a fast reading would have broken:** *"I had forty-six fighters. Three of them lasted a decade"* (`friendly` and `dearFriend`) — Cabanilles left and Bruna was hurt, so the log had **none of the three on the page**, and Reincken is now named as the third; *"You're the best student I've had since him"* (favor tier 3, of Bruna) — so no invented fighter may outrank Bruna, which is why Reincken is *"never the best in the room on a given night"*; *"The promoters tried to run a fixed fight here last year. I closed the pit for a week rather than host it"* — **already canon and previously unwritten**, now the Year 22 entry, which places the present at Year 23; and the Year 12 *"when a fighter says they're fine, check the footwork"*, of which the Year 17 entry is the lesson applied and says so. **This paragraph's own quotation was wrong and is corrected above:** the literal is `[years of entries — …]`, which the *Measured* sentence had right and the opening sentence rendered as *"twelve years"*; the backlog row copied the wrong one. **Two findings on the path, filed not done, both in the header line this fix did not touch** — `CROV'S TRAINING LOG — CY Fighting Pit`: **§DX-02jq**, the NPC's database key printed where every other surface says *Pit Master Weckmann* (`Crov` capitalised occurs in `play.html` **twice**: this header and the flag `act8FarewellCrov`), and **§DX-02jr**, `CY` — not a node, against 416 — still spoken to the player in `hint`, `vignetteText`, `failText`, `text`, `msg` and `readText`.

> **✅ SHIPPED 2026-09-08 (§DX-02jq) `585b9c7`.** The header line this section's fix deliberately did not touch — `CROV'S TRAINING LOG — CY Fighting Pit` — printed the `NPC_DIALOGUES` key as the notebook's title, where `meta.name`, the `NPC_REGISTRY` row, all five `PIT_PERK_UNLOCKS` attributions and perk 5's own title say **Pit Master Weckmann**. Through the same `_buildWeckmannLog()` harness at 0 wins and at 5 wins + champion: **`[Cc]rov` in the rendered log 1 → 0**, bracketed spans **1** in both states, unsubstituted tokens **0** in both. The em-dash rule beneath the title is part of the same template literal and was re-cut **37 → 41** to match the longer name, both counted as code points. **The backlog row's Verify clause could not be satisfied and would have read as a regression** — `grep -c "\bCrov\b"` returns **0**, and returned 0 before the change too, because `act8FarewellCrov` has no word boundary before `Crov`; the measurement that holds is `grep -o Crov | wc -l`, **2 → 1**, and the survivor is a state flag no player reads. **`CY Fighting Pit` is still standing on purpose:** that is **§DX-02jr**, whose call — leaked node code or in-world district name — gives opposite edits, and splitting one line across two rows' authority would make both rows' measurements unreadable.

> **✅ SHIPPED 2026-09-08 (§DX-02jr) `8d8d8fb`.** The `CY Fighting Pit` half of the header, which the entry above left standing on purpose, closed the same day. `CY` is the pre-airport-code key for `HKG` — `docs/maps/node-index.md`'s LEGACY CODE MAP row `| CY | 6 | HKG | Neon Undercity |`, corroborated by node number and label — so the title is now **`WECKMANN'S TRAINING LOG — Undercity Fighting Pit`** and the em-dash rule beneath it was re-cut **41 → 48**, the second re-cut in two rows. The replacement is the node's own label rather than `HKG`, because a player reads prose and `HKG` is equally a code. §DX-02jr swept 21 player-facing sites in all; the log's header was one of them.
