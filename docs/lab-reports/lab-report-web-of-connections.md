...
Anchors at HEAD: `function _checkFrobergerTrace(npcKey)@27650` · `function _getYaelLocation()@27662` · `function _buildWeckmannLog()@27669` · `const FROBERGER_TRACES = {@27687` · `const NPC_CROSS_REFS = {@27696` · `const NIVERS_DIALOGUE = "Evening.";@27728` · `const YAEL_PATROL_NODES = [@27730` · `const WECKMANN_TRAINING_LOG =@27738` · `function _npcFavor(key)@23461` · `function _setNpcFavor(key, level)@23463` · `function _checkDearFriendUpgrade(key)@23490` · `function _nodeHookBirkaNiversPasses(node,@32210` · `function _nodeHookBirkaYaelPatrolLine(node,@32501` · `function _nodeHookBirkaRoom6(node,@32761`.
...
| `quill` | `set:1` | — | **2** | — |
| `pachelbel` | `set:1` | — | **2** | — |
| `crov` | `set:1` + `add:2` (§DX-02fb) | `_setNpcFavor('crov',1)@25402` | **3** | Froberger trace ✅ · Layer 44 `weckmann_class` ✅ |
| `auros` | `set:2` | — | **2** | — |
...
The lock writes `[twelve years of entries — fighters' names, brief notes, outcomes]` as an authoring instruction, in the same bracket convention it uses for `[gap of two years]`. The first is a note to whoever fills the log in; the second is prose. `const WECKMANN_TRAINING_LOG =@27738` shipped **both** verbatim, and `_buildWeckmannLog()` substitutes only `{PLAYER_ENTRIES}` and `{CHAMP_ENTRY}`. Measured: the rendered log contains the literal string `[years of entries — fighters' names, brief notes, outcomes]`.
