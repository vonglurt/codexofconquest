...
| Artefact | Status |
|---|---|
| `QUEST:CORE:START ◆◆◆@21988` … `QUEST:CORE:END ◆◆◆@22362` | live, 370 lines |
| `function createQuestRuntime(host) {@22203` | live, the factory |
| `function _questRunToCompletion(gen) {@22059` | live, the pure twin of `function _uqfRunToCompletion(gen) {@6851` |
| `const SCHEMA_VERSION@21989` · `const BIT_CONTRACTS = {@21993` · `function validateQuest(q) {@22027` · `function adaptLegacyQuest(id, q) {@22049` | the four pure members, moved unchanged |
| `const QuestRuntime = createQuestRuntime({@22369` · `getState: () => S_story,@22370` · `rng:       () => _seededNext(),@22374` | host wiring, outside the sentinels |
| `src/js/quest.js:module.exports = { createQuestRuntime@408` | the export line, byte-exact as specified |
...
3. **`resolveSkillCheck` uses a kernel-internal pure `_questRunToCompletion`, not the host driver** — **held.** `function _questRunToCompletion(gen) {@22059` is live inside the sentinels; the four outer call sites still use the host driver, exactly as scoped.
