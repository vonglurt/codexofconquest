...
| Cell grid | "90×360" | **`const GEO_PROJ = { ROWS: 90, COLS: 360 }@9915`** ✅ | unchanged |
...
- **Company changes the arithmetic, gently.** §MESH-01f grants **+1 to hit per co-present ally,
  capped at +2** (`function _partyHitBonus(allies)@28810`), **+10 % XP and gold per ally, capped at
  +20 %** (`function _partyLootMult(allies)@28811`), and **halves the wilderness encounter rate**
  for a party travelling together (`function _partyEncounterRate(base)@28822`). The doom clock is the
...
- **Somebody can hold the road.** §MESH-01h sentry bots suppress encounters in the cell they garrison
  ...
  junction cannot inflate anyone's to-hit (`function _mpAllyCount()@28803`).
...
(`src/js/wbapi-server.js:const MANIFEST_PARTS@674`: `NODE_MAP`, `NODE_COORDS`, `SEA_RUNS`, `SEA_LANES`,
...
(`src/js/wbapi-server.js:function getManifest@676`):
...
*(`src/js/wbapi-server.js:function pidOf@119` — byte-identical at HEAD, comment included.)*
...
*(`src/js/wbapi-server.js:function broadcastCell@129` — byte-identical at HEAD.)*
...
*(Byte-identical at HEAD — `MP.nearby.some(p => p.r === r && p.c === c)@37571`.)*
...
- ✅ **"The movement kernel never reads MP state."** HOLDS. `src/js/mover.js` has zero `MP` references at
...
- ❌ **"There is no way for the mesh to alter a die roll."** **RETIRED `a07b281` (2026-07-05).**
  ...
  not carry mesh state."* (`S.partyAllies   = _mpAllyCount()@24713`.)
- ⚠ **"MP state … never in the save file."** **CORRECTED.** The session id still lives only in
  ...
  occurred. But two mesh fields now persist: `S_story.playerKey` (`function _mpPlayerKey()@29135`,
...
Every ~2 s (`src/js/mesh.js:const MESH_GOSSIP_MS@116`) each server pushes its payload to ≤3 random peers
and merges what comes back. The payload is self-describing — identity triple, then state, then peer
candidates (`src/js/mesh.js:function meshPayload@216`):
...
merge (`src/js/mesh.js:function meshIngest@228`). **A gossip POST is answered with a full gossip
...
redundantly deliver it (`src/js/mesh.js:function meshMergeEvents@176`):
...
- **The freshness window** (`src/js/mesh.js:const MESH_FANOUT_MAX_AGE@118`, 10 s). A rejoining server
...
(`src/js/mesh.js:function meshMergeSnapshot@207`):
...
(`src/js/mesh.js:const MESH_ORIGIN_TTL@117`, 90 s of silence) — an origin that stops heartbeating takes
...
(`src/js/mesh.js:function remotePlayersAt@156`). Because the client consumes presence through those
...
Servers announce every ~30 s (`src/js/mesh.js:const MESH_ANNOUNCE_MS@439`) with the identity triple,
...
tables (`src/js/mesh.js:function trackerMergeRecords@396`). This is *structurally the same function* as
...
- **Idempotent exchange ⇒ no flood loops by construction.** Merging the same table twice is a no-op,
  ...
  nothing to loop*.
- **Bounded tables, updates-preferred** (`src/js/mesh.js:const TRACKER_MAX_RECORDS@388`, 500). A spammer
...
`src/js/mesh.js:function loadStaticPeers@317`. Rung 3 closes the loop automatically: every 30 s the
...
moved to `src/config/peers.txt` in the 2026-07-09 layout change; the format is unchanged.)*
...
(`src/js/mesh.js:function fetchBootstrapUrls@466`). The recommended host is a versioned, revocable, free
...
(`src/js/mesh.js:function meshAdvertise@123`) — while a real cross-machine mesh needs *both*
...
(`src/js/mesh.js:function meshReachabilityWarnings@135`, verbatim at HEAD including both warning
...
(`function mpParseMagnet(str)@29451`): resolve `tr`, filter the group by `wh`, ping each candidate
...
(`src/js/mesh.js:function pushTraffic@112`), served by one endpoint and rendered by the worldbuilder's 🌐
...
- **ACLs** (`src/config/mesh-acl.json`, hot-reloaded): allow/block by serverId, IP and worldHash, with an
...
> (`src/js/wbapi-server.js:function playerKeyRegister@290`) is the durable identity §6.4 specified. *A
...
|---|---|---|---|
| `src/tests/mud-harness.mjs` | "75 checks" | **75** ✅ (`56def8a`: *"harness 75"*) | **269** (267 ✓ / 2 ✗) |
...
| # | Claim | Verdict |
|---|---|---|
| 1 | 57 named symbols across four files | **All resolve.** Mesh half relocated to `src/js/mesh.js` |
...
| *"all excerpts verbatim from `wbapi-server.js`"* | App. | **RELOCATED** `4367a38`, +9 h. Still verbatim, now `src/js/mesh.js` |
...
- **§DX-02cn** 🟢 — `S_story.playerKey` and `S_story.pvpOff` are undeclared in `_S_DEFAULTS()`
  (`const _S_DEFAULTS = () => ({@23092`). Because `storyNewGame` does `Object.assign(S_story,
...
original are, since `4367a38`, in `src/js/mesh.js` — moved without edit. Function names are stable public
