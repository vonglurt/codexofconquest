...
`const CONDITION_GOLD = {@24677` · `const CONDITION_ITEMS = [@22437` · `function storyPreBattle(node) {@36580`

**Flashbang — exact.**
`const COMBAT_ITEMS = [@24521` · `function storyBuyFlashbang() {@24430` ·
`function _renderSboSpells() {@24959` · `function _storyUseFlashbang(invIdx) {@24984` ·
`function _playerHasBonusOptions() {@25027` · `{ weight:6, _type:"flashbang" },@24582`

**Progression — shipped under a different architecture.**
`const XP_LEVELS = [@24482` (20 entries) · `const FIGHTER_FEATURES = {@25571` (19 entries, Lv2–20) ·
`const _ASI_LEVELS = new Set([4, 6, 8, 12, 14, 16, 19]);@25593` ·
`function _showLevelUpModal(lvl) {@25743` · `function _checkLevelUp() {@25799` ·
`function _extraAttackCount() {@25045` · `const critMin = _lv >= 20 ? 17@25077`

**Integration points named by the report.**
`function _storyBattleVictory() {@25339` · `_levelUpQueue = [];   // reset before each battle resolution@25355` ·
`const lvlAtk@25080` · `function _calcPlayerAc() {@24670` ·
`const nextThresh = XP_LEVELS[curLv];@25536` · `const lvEl = document.getElementById('s-level');@36312`
...
| 2 | Condition item source = *"Pre-existing inventory"* (§V.A) | Not inventory-gated: `// All condition items available for gold purchase (not inventory gated)@36584` | **NOT SHIPPED — superseded, and it strengthens §IV** |
...
(`const _ASI_LEVELS = new Set([4, 6, 8, 12, 14, 16, 19]);@25593`). `function _showLevelUpModal(lvl) {@25743`
gates each section behind the previous one; `function _checkLevelUp() {@25799` now only *queues*.
...
scalar: `function _extraAttackCount() {@25045` turns the main action into 2/3/4 rolls at L5/11/20 and
`const critMin = _lv >= 20 ? 17@25077` widens the crit window at L3/15/20 — changes to the *structure*
...
— the `(S_story.acBonus || 0)` *(deleted, §DX-02y)* term inside `function _calcPlayerAc() {@24670` — and **zero
...
`const lvlAtk@25080`. Two individually-correct changes compose into three disagreeing surfaces:

- **The battle engine** reads its ability modifier from the dice-roller's own control, which is
  `<option value="dex" selected>@3715`. The story→simulator sync at
  `document.getElementById('char-level').value = _slv;` *(extracted 2026-08-26 into `_syncStoryCharInto@25663`, §ATKAB-01)* writes the level, AC, max HP and all six
...
`const CONDITION_GOLD = {@24677` is **byte-identical at `32c10c5` and at HEAD** — all 12 entries,
...
(`// All condition items available for gold purchase (not inventory gated)@36584`), so **gold is the
...
overlay (`function storyPreBattle(node) {@36580`) and the in-page battle accordion price from the same
table.
...
`ancient_dragon:  { key:'ancient_dragon'@5526` pays **12,012** — 4× the ceiling. Trivial is inverted:
...
`function _playerHasBonusOptions() {@25027` tests the shield, offhand, scrolls, flashbangs and
potions in turn — and then closes with `return true;  // wimper always available@25034`. Only the
...
- `function _storyUseFlashbang(invIdx) {@24984` — the guard order is exactly as specified: *must have
...
- **Expanded beyond spec:** `{ weight:6, _type:"flashbang" },@24582` makes it a 6 % loot drop, so the
