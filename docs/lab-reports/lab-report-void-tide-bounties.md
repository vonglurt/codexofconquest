...
42) fired on sleep by `function storyCheckVoidTide()@36340`, reached from
`function storyConfirmSleep()@36226`. Beat **#21** narrates a Warrant bounty the engine never
modelled:
...
|---|---|---|---|
| `VOID_TIDE_FEATURED` | const map, tideDay → `{quest, standing, rumor}` | `const VOID_TIDE_FEATURED@37086` | the featured roster, per-quest standing bonus, dread hook |
| `VOID_FEATURED_IDS` | derived `Set` | `const VOID_FEATURED_IDS@37091` | ids excluded from the normal rotation pool |
...
| `_bountyPostable(q,node)` | pure predicate | `function _bountyPostable(q, node)@37131` | shared legality gate; deliberately excludes the FU7 reward ceiling |
| `_voidFeatured(node)` | pure selector | `function _voidFeatured(node)@37145` | the active featured bounty, or `null` |
| `_voidBonus(id)` | pure lookup | `function _voidBonus(id)@37093` | escalated standing for a Void bounty, else 1 |
...
`function storyCheckQuests(node)@30167`, and `_bountyPostable`. What falls out for free:
...
two honest ways: the quest's own `reward` XP bit, read by the existing `function _boardReward@37035`
path (never fake gold, per FU1), and the escalated standing applied at
`function _creditWarrant(id)@37099`. Standing is host bookkeeping, not an economy change.
...
*Unknown* (`rewardCap: 250@37057`, slate 4) and the day-42 pin still renders **⭐320 xp** as a fifth
...
- `function _boardBounties(node, limit)@37165`: (a) the loop's legality checks were factored into
  `_bountyPostable`; (b) one skip added — `if (VOID_FEATURED_IDS.has(q.id)) continue;@37173`; (c)
  after distance labelling, `const vf = _voidFeatured(node);@37199` and `shown.unshift(vf);@37203`.
  The pin is an **extra** row, unshifted past the slate slice, always first. Pure and read-only.
- Render, at `lbl: b.void ?@35689` inside `function storyRender(node, prefix)@34549`: the card
...
| Data-driven, no per-node `if` — the pin rides the node-flag path via `function _boardHost(node)@37020` | ✅ held |
...
site — `S_story.day = Math.min(49, S_story.day + 1)@36252`, inside `storyConfirmSleep` — a retry
...
`active`, and `onFail` is empty. The only movement is §XP-01 effort XP — `const EFFORT_XP_PCT@24427`
= 0.25 of the pass reward, paid **once per quest** (80 of the 320; a second failure pays nothing).
The measured delta was 90, of which the remaining 10 is `const EXPLORE_XP@24436` for first arrival at
...
- **The engine comment this increment shipped carries a line number that was wrong the day it was
  written.** `beats at 21821@37080` points at `VOID_TIDE_EVENTS`, which sits at **21822** in the very
...
since the initial commit — `dayEl.className@36087` — so FU8's second and third windows open on
