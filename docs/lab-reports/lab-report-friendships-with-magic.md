...
`function _weaponScore(w) {@24578` · `function _isLastWeapon(item) {@24599` ·
`const _wpDrop = _rollMonsterWeaponDrop@25429` · `const _monDmgDie = S.opp@25428` ·
`function _updateHuntBtn() {@38056` · `function storyToggleHunt() {@38067` ·
`function _curseScore() {@28192` · `const _EB_CODES = ['PRN'@28031` ·
`function _lubeckFriends()@23462` · `const FROBERGER_JOURNAL = [@27185` ·
`const _BASE_WEAPONS = [@24472` · `const WEAPON_ITEMS = [0, 1, 2, 3, 4]@24495` ·
...
| `_stalkedMonsterPick` | **RETIRED** | 3 at the archive, 5 commits. Survives at HEAD only as the tombstone `_stalkedMonsterPick) removed with the Hunt feature@38251`. |
...
|---|---|---|---|
| 1 | `_updateExitLinks()` computes the first step toward the waypoint before rendering exits | Does exactly this, via `wpDir = _roadGridDir(@37506` | ⚠️ contract kept, mechanism replaced |
...
| 6 | Exits are derived from the BFS path over `NODE_MAP` | Exits are **one cell only**, from the geo grid; the comment `// Waypoint BFS direction (§CELL-09)@37502` sits directly above a `_roadGridDir` call | ⚠️ stale comment, see F3 |
...
| 13 | Negotiation rolls 1d20 + `floor((cha − 10) / 2)` vs DC 17 | `const roll   = Math.floor(Math.random() * 20) + 1;@30293`, `total >= 17` | ✅ byte-exact |
...
| 18 | 1d4, minimum 1, HP cannot drop below 1 | `S_story.hp     = Math.max(1, (S_story.hp || 1) - dmg);@30312` | ✅ |
...
| 41 | §III thesis: *"the stat block is self-contained — `_rollMonsterWeaponDrop()` reads `S.opp.dmgDie` without a lookup"* | `const _monDmgDie = S.opp@25428` — the only `S.opp.dmgDie` read in the file | ✅ holds |
...
`function _curseScore() {@28192` is **byte-identical between the archive and HEAD** — 82 days, zero
...
`S_story.ebReturnDone[ebCode] = true;@30364` inside `function _storyEbReturnBeat(ebCode) {@30359`,
...
- `else if (curse >= 15)` — `Sweelinck sets the journal on the table@28259`, *"The Void is sealed. The
  curse is not in the Void."* — **fires 100 % of the time.**
- The final `else` (Mixed) is dead: the window `0 < curse < 15` cannot be entered.
- Sweelinck's Last Question collapses the same way — always `Were you alone by choice?@28277`.
...
**F1b — a separate, born-broken off-by-one.** `const _isTrue = missionDone && curse <= -6@28230`
gates the *"Covenant Keeper (True)"* standing. The best attainable score in a fully working engine is
`0 + 0 − 5 = −5` — the `(allComplete ? 5 : 0)@28205` bonus is the only negative term and it is worth
...
- `WEAPON_ITEMS.filter(w => w.die <= monsterDmgDie && w.magicBonus === 0@24588` — magic tiers excluded.
- `const deg = Math.min(0, d6 - 5)@24593` — a −4…0 degradation roll, prefixing *Wrecked / Rusted /
  Chipped / Worn*.
- Justified in place by `FC06: monster drops capped at base tier@24587`, which continues *"fishing is
  the only source of +bonus weapons"* — the giving half §DOC-02n measured as never built.

**The call site still carries the original rationale.** `no level/magic gate (Finders Keepers)@25427`
sits one line above `const _wpDrop = _rollMonsterWeaponDrop@25429`. Two comments in one call chain
...
`const _BASE_WEAPONS = [@24472` holds **14** entries and `const WEAPON_ITEMS = [0, 1, 2, 3, 4]@24495`
...
which `S.weapon.flatMod = mw.magicBonus@24670` feeds straight into attack and damage. Three of the
four surfaces that render a bonus print it **only when positive** — `_wpCopy.magicBonus > 0 ? ' +'@25443`
(victory drop line), `const tag = mw.magicBonus > 0 ? dieStr@30919` (inventory tag),
`it.magicBonus > 0 ? ' +' + it.magicBonus + ' atk/dmg'@31096` (item detail). Only the Character Sheet
...
`_updateHuntBtn / storyToggleHunt removed@38095` states that §TIMELESS-01 removed both functions.
`function _updateHuntBtn() {@38056` and `function storyToggleHunt() {@38067` are declared **39 and 28
...
`// Waypoint BFS direction (§CELL-09)@37502` sits directly above a `_roadGridDir` call, and
`no level/magic gate (Finders Keepers)@25427` (F2).
...
`const FROBERGER_JOURNAL = [@27185` holds **41 at HEAD and 41 at the archive** — §DOC-02i measured it
...
Also traceable to this specification: `const roll   = Math.floor(Math.random() * 20) + 1;@30293` and
...
| **§ENDING-01 (b)** *(new)* | `missionDone && curse <= -6@28230` against a score floor of −5: the *"Covenant Keeper (True)"* standing is unsatisfiable and always has been. One-character fix, but which threshold is intended is the author's call. | Small |
| **§FISH-02** *(extended)* | Denominator corrected: **56 of 70** `WEAPON_ITEMS` unreachable, not 48 of 60. Adds **F2b**: negative `magicBonus` is applied to attack/damage but hidden on 3 of 4 render surfaces. Adds the live-comment contradiction (`Finders Keepers@25427` vs `FC06@24587`). | Existing |
| **§AUDIT-03aa** *(extended)* | Second instance of the class, and stronger: `_updateHuntBtn / storyToggleHunt removed@38095` is refuted by declarations 39 and 28 lines above it. Adds the stale `// Waypoint BFS direction (§CELL-09)@37502`. Argues the wanted detector is *"a comment asserting removal of a symbol that resolves"* — mechanically checkable. | No |
