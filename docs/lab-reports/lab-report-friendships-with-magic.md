...
`function _weaponScore(w) {@24637` · `function _isLastWeapon(item) {@24658` ·
`const _wpDrop = _rollMonsterWeaponDrop@25495` · `const _monDmgDie = S.opp@25494` ·
`function _updateHuntBtn() {@38270` · `function storyToggleHunt() {@38281` ·
`function _curseScore() {@28340` · `const _EB_CODES = ['PRN'@28171` ·
`function _lubeckFriends()@23493` · `const FROBERGER_JOURNAL = [@27318` ·
`const _BASE_WEAPONS = [@24527` · `const WEAPON_ITEMS = [0, 1, 2, 3, 4]@24550` ·
`#eb-npc-cha-roll-line.cha-pass@3384` · `He's Fine — Continue →@4912` ·
...
| `_stalkedMonsterPick` | **RETIRED** | 3 at the archive, 5 commits. Survives at HEAD only as the tombstone `_stalkedMonsterPick) removed with the Hunt feature@38465`. |
...
|---|---|---|---|
| 1 | `_updateExitLinks()` computes the first step toward the waypoint before rendering exits | Does exactly this, via `wpDir = _roadGridDir(@37727` | ⚠️ contract kept, mechanism replaced |
...
| 6 | Exits are derived from the BFS path over `NODE_MAP` | Exits are **one cell only**, from the geo grid; the comment `// Waypoint BFS direction (§CELL-09)@37723` sits directly above a `_roadGridDir` call | ⚠️ stale comment, see F3 |
...
| 13 | Negotiation rolls 1d20 + `floor((cha − 10) / 2)` vs DC 17 | `const roll   = Math.floor(Math.random() * 20) + 1;@30450`, `total >= 17` | ✅ byte-exact |
...
| 18 | 1d4, minimum 1, HP cannot drop below 1 | `S_story.hp     = Math.max(1, (S_story.hp || 1) - dmg);@30469` | ✅ |
...
| 41 | §III thesis: *"the stat block is self-contained — `_rollMonsterWeaponDrop()` reads `S.opp.dmgDie` without a lookup"* | `const _monDmgDie = S.opp@25494` — the only `S.opp.dmgDie` read in the file | ✅ holds |
...
`function _curseScore() {@28340` is **byte-identical between the archive and HEAD** — 82 days, zero
...
`S_story.ebReturnDone[ebCode] = true;@30521` inside `function _storyEbReturnBeat(ebCode) {@30516`,
...
- `else if (curse >= 15)` — `Sweelinck sets the journal on the table@28408`, *"The Void is sealed. The
  curse is not in the Void."* — **fires 100 % of the time.**
- The final `else` (Mixed) is dead: the window `0 < curse < 15` cannot be entered.
- Sweelinck's Last Question collapses the same way — always `Were you alone by choice?@28426`.
- **2 of the 12 mission bits are permanently false**: `allEbReturns: Object.keys(NPC_DIALOGUES)@23688`
  (needs ≥ 5 returns) and `noHighCurse: _curseScore() < 10@23692` (floor 20). `_missionComplete()`
...
`0 + 0 − 5 = −5` — the `(allComplete ? 5 : 0)@28353` bonus is the only negative term and it is worth
...
- `WEAPON_ITEMS.filter(w => w.die <= monsterDmgDie && w.magicBonus === 0@24647` — magic tiers excluded.
- `const deg = Math.min(0, d6 - 5)@24652` — a −4…0 degradation roll, prefixing *Wrecked / Rusted /
  Chipped / Worn*.
- Justified in place by `FC06: monster drops capped at base tier@24646`, which continues *"fishing is
  the only source of +bonus weapons"* — the giving half §DOC-02n measured as never built.

**The call site still carries the original rationale.** `no level/magic gate (Finders Keepers)@25493`
sits one line above `const _wpDrop = _rollMonsterWeaponDrop@25495`. Two comments in one call chain
...
`const _BASE_WEAPONS = [@24527` holds **14** entries and `const WEAPON_ITEMS = [0, 1, 2, 3, 4]@24550`
...
which `w.flatMod = mw.magicBonus@25638` feeds straight into attack and damage. Three of the
four surfaces that render a bonus print it **only when positive** — `_wpCopy.magicBonus > 0 ? ' +'@25509`
(victory drop line), `const tag = mw.magicBonus > 0 ? dieStr@31105` (inventory tag),
`it.magicBonus > 0 ? ' +' + it.magicBonus + ' atk/dmg'@31282` (item detail). Only the Character Sheet
...
`_updateHuntBtn / storyToggleHunt removed@38309` states that §TIMELESS-01 removed both functions.
`function _updateHuntBtn() {@38270` and `function storyToggleHunt() {@38281` are declared **39 and 28
...
`// Waypoint BFS direction (§CELL-09)@37723` sits directly above a `_roadGridDir` call, and
`no level/magic gate (Finders Keepers)@25493` (F2).
...
`const FROBERGER_JOURNAL = [@27318` holds **41 at HEAD and 41 at the archive** — §DOC-02i measured it
...
Also traceable to this specification: `const roll   = Math.floor(Math.random() * 20) + 1;@30450` and
...
| **§FISH-02** *(extended)* | Denominator corrected: **56 of 70** `WEAPON_ITEMS` unreachable, not 48 of 60. Adds **F2b**: negative `magicBonus` is applied to attack/damage but hidden on 3 of 4 render surfaces. Adds the live-comment contradiction (`Finders Keepers@25493` vs `FC06@24646`). | Existing |
| **§AUDIT-03aa** *(extended)* | Second instance of the class, and stronger: `_updateHuntBtn / storyToggleHunt removed@38309` is refuted by declarations 39 and 28 lines above it. Adds the stale `// Waypoint BFS direction (§CELL-09)@37723`. Argues the wanted detector is *"a comment asserting removal of a symbol that resolves"* — mechanically checkable. | No |
