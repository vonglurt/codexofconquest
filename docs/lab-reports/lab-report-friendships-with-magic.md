...
`function _weaponScore(w) {@24591` · `function _isLastWeapon(item) {@24612` ·
`const _wpDrop = _rollMonsterWeaponDrop@25451` · `const _monDmgDie = S.opp@25450` ·
`function _updateHuntBtn() {@38138` · `function storyToggleHunt() {@38149` ·
`function _curseScore() {@28261` · `const _EB_CODES = ['PRN'@28099` ·
`function _lubeckFriends()@23469` · `const FROBERGER_JOURNAL = [@27253` ·
`const _BASE_WEAPONS = [@24483` · `const WEAPON_ITEMS = [0, 1, 2, 3, 4]@24506` ·
`#eb-npc-cha-roll-line.cha-pass@3384` · `He's Fine — Continue →@4911` ·
`.exit-waypoint { background@2839` · `_updateExitLinks` · `_updateWaypointBtn` ·
...
| `_stalkedMonsterPick` | **RETIRED** | 3 at the archive, 5 commits. Survives at HEAD only as the tombstone `_stalkedMonsterPick) removed with the Hunt feature@38333`. |
...
|---|---|---|---|
| 1 | `_updateExitLinks()` computes the first step toward the waypoint before rendering exits | Does exactly this, via `wpDir = _roadGridDir(@37585` | ⚠️ contract kept, mechanism replaced |
...
| 6 | Exits are derived from the BFS path over `NODE_MAP` | Exits are **one cell only**, from the geo grid; the comment `// Waypoint BFS direction (§CELL-09)@37581` sits directly above a `_roadGridDir` call | ⚠️ stale comment, see F3 |
...
| 13 | Negotiation rolls 1d20 + `floor((cha − 10) / 2)` vs DC 17 | `const roll   = Math.floor(Math.random() * 20) + 1;@30367`, `total >= 17` | ✅ byte-exact |
| 14 | CHA 8 ⇒ −1 modifier, needs a natural 18 | 18 − 1 = 17 ≥ 17 | ✅ arithmetic exact |
| 15 | Pass panel `#44ee88` / `#051a0d` / `#22cc66` | `#eb-npc-cha-roll-line.cha-pass@3384`, all three values | ✅ byte-exact |
...
| 18 | 1d4, minimum 1, HP cannot drop below 1 | `S_story.hp     = Math.max(1, (S_story.hp || 1) - dmg);@30386` | ✅ |
...
| 41 | §III thesis: *"the stat block is self-contained — `_rollMonsterWeaponDrop()` reads `S.opp.dmgDie` without a lookup"* | `const _monDmgDie = S.opp@25450` — the only `S.opp.dmgDie` read in the file | ✅ holds |
...
`function _curseScore() {@28261` is **byte-identical between the archive and HEAD** — 82 days, zero
...
`S_story.ebReturnDone[ebCode] = true;@30438` inside `function _storyEbReturnBeat(ebCode) {@30433`,
...
- `else if (curse >= 15)` — `Sweelinck sets the journal on the table@28329`, *"The Void is sealed. The
  curse is not in the Void."* — **fires 100 % of the time.**
- The final `else` (Mixed) is dead: the window `0 < curse < 15` cannot be entered.
- Sweelinck's Last Question collapses the same way — always `Were you alone by choice?@28347`.
- **2 of the 12 mission bits are permanently false**: `allEbReturns: Object.keys(NPC_DIALOGUES)@23662`
  (needs ≥ 5 returns) and `noHighCurse: _curseScore() < 10@23666` (floor 20). `_missionComplete()`
...
`0 + 0 − 5 = −5` — the `(allComplete ? 5 : 0)@28274` bonus is the only negative term and it is worth
...
- `WEAPON_ITEMS.filter(w => w.die <= monsterDmgDie && w.magicBonus === 0@24601` — magic tiers excluded.
- `const deg = Math.min(0, d6 - 5)@24606` — a −4…0 degradation roll, prefixing *Wrecked / Rusted /
  Chipped / Worn*.
- Justified in place by `FC06: monster drops capped at base tier@24600`, which continues *"fishing is
  the only source of +bonus weapons"* — the giving half §DOC-02n measured as never built.

**The call site still carries the original rationale.** `no level/magic gate (Finders Keepers)@25449`
sits one line above `const _wpDrop = _rollMonsterWeaponDrop@25451`. Two comments in one call chain
...
`const _BASE_WEAPONS = [@24483` holds **14** entries and `const WEAPON_ITEMS = [0, 1, 2, 3, 4]@24506`
...
which `w.flatMod = mw.magicBonus@25604` feeds straight into attack and damage. Three of the
four surfaces that render a bonus print it **only when positive** — `_wpCopy.magicBonus > 0 ? ' +'@25465`
(victory drop line), `const tag = mw.magicBonus > 0 ? dieStr@30991` (inventory tag),
`it.magicBonus > 0 ? ' +' + it.magicBonus + ' atk/dmg'@31168` (item detail). Only the Character Sheet
...
`_updateHuntBtn / storyToggleHunt removed@38177` states that §TIMELESS-01 removed both functions.
`function _updateHuntBtn() {@38138` and `function storyToggleHunt() {@38149` are declared **39 and 28
...
`// Waypoint BFS direction (§CELL-09)@37581` sits directly above a `_roadGridDir` call, and
`no level/magic gate (Finders Keepers)@25449` (F2).
...
`const FROBERGER_JOURNAL = [@27253` holds **41 at HEAD and 41 at the archive** — §DOC-02i measured it
...
Also traceable to this specification: `const roll   = Math.floor(Math.random() * 20) + 1;@30367` and
...
| **§FISH-02** *(extended)* | Denominator corrected: **56 of 70** `WEAPON_ITEMS` unreachable, not 48 of 60. Adds **F2b**: negative `magicBonus` is applied to attack/damage but hidden on 3 of 4 render surfaces. Adds the live-comment contradiction (`Finders Keepers@25449` vs `FC06@24600`). | Existing |
| **§AUDIT-03aa** *(extended)* | Second instance of the class, and stronger: `_updateHuntBtn / storyToggleHunt removed@38177` is refuted by declarations 39 and 28 lines above it. Adds the stale `// Waypoint BFS direction (§CELL-09)@37581`. Argues the wanted detector is *"a comment asserting removal of a symbol that resolves"* — mechanically checkable. | No |
