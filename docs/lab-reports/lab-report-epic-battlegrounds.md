...
|---|---|---|
| Boss statlines | `const EPIC_BOSS_POOL = {@26390` | 20 entries, all `tier:'deadly'` |
| Quest-giver dialogue | `const EB_NPC_DIALOGUE = {@26432` | 20 entries × 10 fields |
| Reward items | `const EB_STORY_ITEMS = {@26416` | 11 declared, 10 referenced |
| Epic node records | `PRN:{ num:52@8749` | 20 records, `num:` 52–71 contiguous |
| Epic terrain pools | `label:'Epic Battleground — Forest'@6344` | 20 `epic_*` terrains, 3 monsters each |
| Danger banner | `sbo-epic-danger">⚠ DANGER: EPIC@5057` | markup + `pulse-red` animation |
| Pre-battle trigger | `function storyEpicPreBattle(node) {@36599` | synthesises the battle from `bossKey` |
| Return/payment beat | `function _storyEbReturnBeat(ebCode) {@30516` | gold + item + beat |
| Node-code roster | `const _EB_CODES = [@28171` | live codes `PRN`…`TBS` |
| Primary quests | `quest_ef_primary: { id:'quest_ef_primary'@10730` | 20, `type:'epic'`, UQF-1.0 |
| Return quests | `quest_ef_return: { id:'quest_ef_return'@10891` | 20, `type:'epic'`, UQF-1.0 |

Node records carry `isEpicBattleground:true, bossKey:'thornwood_king'@8749` rather than a populated
...
- `const primaryId = 'quest_' + ebCode.toLowerCase() + '_primary';@30504` (contract accept)
- `'quest_' + node.code.toLowerCase() + '_primary';@36100` (battle chip)
- `const returnId = 'quest_' + pb.nodeCode.toLowerCase() + '_return';@36795` (victory)
...
1. **All forty authored epic quests are orphaned.** `_questsByNode` excludes them by design —
   `if (!q || q.type === 'epic' || !q.activateNode) continue;@37217` — so arrival never activates them,
...
2. **The entire payment layer is unreachable.** `function _storyEbReturnBeat(ebCode) {@30516` has exactly
...
`const EB_STORY_ITEMS = {@26416` declares eleven items. Ten are referenced as a `specialItem`; each
...
`S_story.inventory.push({ name: si.name, desc: si.desc, icon: si.icon || '✨' });@30534`, additionally
...
And `kazrath_journal:@26427` has **one occurrence in the whole file and zero consumers** — the
...
`const accepted = _ebNpcNegotiated ? d.paymentCeiling : d.paymentFloor;@30501`. The CHA check
(`S_story.abilityScores || {}).cha) || 8;@30448`, DC 17) gates only the *damage*, not the gold:
...
- `Math.max(1, Math.floor(Math.random() * 4) + 1)@30468` and the d20 roll three lines above are
...
(`docs/lab-reports/` is HISTORY under `src/scripts/legacy-codes.js`).*
