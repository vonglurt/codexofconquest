...
> *"Level 21 is undefined. That is not a bug. That is the door."* — still verbatim at `` `That is not a bug. That is the door.@31632` ``, 79 days on. The line survived. The numbers around it did not.
...
`const XP_LEVELS = [@24482` has twenty entries. `const FIGHTER_FEATURES = {@25571` is keyed 2 → 20. There is no L21 threshold and no `FIGHTER_FEATURES[21]` — verified absent at HEAD. A player at L20 has a live doom clock (Day 49) and a dead reward loop.

The obvious patch — a "You Win" banner — was rejected because the game already has a victory screen: `function storyCheckVictory@28356`, fired at the same node. A second congratulation would compete with the real ending.
...
|---|---|---|
| Trigger node | `` `TLS:{ num:42, code:'TLS'@8739` `` — *Cosmic Realm — The Convergence*, `act:8` | ✅ same node, renamed from `CO` |
| Panel registration | `` `Layer 49: §XIV Quest -1@31615` `` — a `NODE_PANELS` entry, `nodes:['TLS']` | ✅ migrated by §VM-01-G1 |
| Trigger condition | `` `>= 20 && !st.questMinusOne@31617` `` | ✅ contract preserved |
| Styling | `` `el.className = 'sweelinck-variant';@31837` `` + `css:'border-left-color:#556b2f'` | ✅ verbatim intent |
| Run-once flag | `` `questMinusOne: false@23156` `` | ✅ specified name |
| NG+ carry flag | `` `priorQuestMinusOne: false@23159` `` | ✅ specified name |
| NG+ capture | `` `const savedPriorQuestMinus1@24086` `` | ✅ byte-identical to reference build |
| NG+ restore | `` `S_story.priorQuestMinusOne = savedPriorQuestMinus1@24097` `` | ✅ byte-identical |
| Completion path | `` `S_story.questMinusOne = true; storyAutoSave();@31645` `` — console only | ✅ by design |
| NG+ pairing | `` `quest_ng_02: { id:'quest_ng_02'@11063` `` | ✅ now declarative UQF-1.0 |
| Victory screen | `` `if (!S_story.defeatedBattles['TLS']) return;@28358` `` | ✅ the guard §VII.1 wants, three thousand lines away |
...
Two mechanical details in the disclosure prose were checked and are **exact**: *"The crit window is 17–20"* matches `` `const critMin = _lv >= 20 ? 17@25077` ``, and *"Indomitable has been yours for eleven levels"* matches `FIGHTER_FEATURES[9] = Indomitable I` (20 − 9 = 11). The author counted.
...
| 8 | *"plan.md §XIV describes the World Creator Wizard"* | `plan.md` **does not exist** | 🔴 Deleted by `5e48dd7` (split into `CONTRIBUTING.md` + `BACKLOG.md`). The pointer is in a **player-facing string** at `` `plan.md §XIV describes the World Creator@31642` ``. |
...
**Reachability (instrument 19): 100 %.** `` `TLS:{r:26,c:181}@9693` `` is the **sole** occupant of its cell, so it is `list[0]` and can become `currentCode` — this arc is untouched by §AUDIT-03x. `_renderNodePanels(node, S_story)` is called unconditionally in `storyRender`, and the panel's `when` has no dead dependency. The disclosure renders exactly as specified.
...
|---|---|---|
| 1 | Guard the disclosure on `defeatedBattles['CO']` so it cannot pre-empt the story climax | 🔴 **OPEN, and cheaper than stated.** `TLS` is the final-battle node; `storyCheckVictory` already tests `` `if (!S_story.defeatedBattles['TLS']) return;@28358` ``. Because movement is free (invariant #1), an L20 player can walk to `TLS` and read the developer disclosure before ever fighting the Void Warlord. The fix is one clause copied from a function in the same file. |
...
| File | Anchor | Content |
|---|---|---|
| `play.html` | `` `questMinusOne: false@23156` `` | run-once flag in `_S_DEFAULTS()` |
| `play.html` | `` `priorQuestMinusOne: false@23159` `` | NG+ carry flag |
| `play.html` | `` `const savedPriorQuestMinus1@24086` `` | NG+ capture |
| `play.html` | `` `Layer 49: §XIV Quest -1@31615` `` | the disclosure panel |
| `play.html` | `` `quest_ng_02: { id:'quest_ng_02'@11063` `` | the Entry 42 pairing |
