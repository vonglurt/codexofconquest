...
| `_ceremoRetryBlocked(questId)` | **exact**, body identical modulo one inlined local | `function _ceremoRetryBlocked@6817` |
| `S_story.skillCheckAttempts = { questId: { lastDay, failures } }` | **exact shape**, declared in `_S_DEFAULTS()`; `failures` is never read (delta 12) | `skillCheckAttempts: {}@23182` |
| `retryable` + `retryGateDays` (default 1) | **exact**, both read by the live gate; the default is the only way to say *one day* and, since §DX-02ee, `0` is no longer written anywhere — it was inert (`0 \|\| 1`) and all 42 uses were deleted | `q.retryGateDays@6822` |
| `activateCond` patch to the activation loop | **exact and still live**, at 2 call sites; 66 quests carry one | `if (q.activateCond && !q.activateCond()) return;@30310` |
| Card label `'ROLL'` for skill-check quests | **exact** — and now the *default* of a five-way typed table | `verb:'Roll Ceremonia'@35791` |
| `btn-talk` on the roll button | **exact** | `cls:'btn-talk'@35790` |
| All 5 `_S_DEFAULTS()` fields: `skillCheckAttempts` · `ceremoniaYaelAct` · `ceremonia_yael_04_failed` · `ceremonia_yael_complete` · `cryptSurveyed` | **all five, under their specified names** | `skillCheckAttempts: {}@23182` … `cryptSurveyed: false@23187` |
| All 9 quests: courier release · crypt survey · watch patrol · pit debut · yael 01–05 | **all live, all UQF-1.0** | `quest_courier_release@21356` … `quest_ceremonia_yael_05@21438` |
| **All 15 Yael prose strings** — 5 vignettes, 5 pass, 5 fail | **verbatim, every one** | `@21368`–`@21424` |
| `vignetteTextAlt` + its selection rule (`ceremonia_yael_04_failed`) | **exact**, and the report's *"selected at render time, not a QUEST_DB field"* note is wrong twice over — it **is** a field, and it is selected at render time | `q.vignetteTextAlt && S_story.ceremonia_yael_04_failed@35779` |
...
| Act V's `onPass`: `ceremoniaYaelAct=5` · `_setNpcFavor('yael',3)` · push **Yael's Watch Token** (🪙, `type:'token'`, `sell:0`) with its full `desc` | **exact, all four, including the description string verbatim** | `Yael's Watch Token@21453` |
| Crypt Survey prose — all three strings incl. *"a cleaner hand than Froberger's"* | **verbatim** | `@21344`–`@21346` |
| Patrol route as a three-node ordered visit tracked in `storyCheckQuests` | **shipped**, with the nodes remapped (delta 16) | `§DESIGN-03: Track patrol route@30327` |
| `pitTrainingWins >= 1` completes Pit Debut; its reward message | **exact**, message verbatim: *"🥊 First Blood! +100gp +250 XP — The Pit knows your face now."* | `quest_pit_debut@21378` |
...
| 3 | **`d20 = Math.ceil(Math.random() * 20)`** | **CHANGED — and the spec as written is now a banned shape** | `Math.ceil(E.rng() * 20)@22276` — the seeded mulberry32 stream. Invariant #6 (*seeded RNG for game state*) makes the report's line a defect today. It was correct when written; §VM-01-B moved it. |
| 4 | **`total = d20 + mod + prof`** | **CHANGED — two addends added** | `total: d20 + mod + prof + iodineBonus + lmAll`. §CROWN-01 added **Iodine Salt** (+3, or +5 charged), burned by a dedicated button *inside* the roll card and consumed by the roll itself; §DROP-03's lake-magic passives added `allAbility` (`_lakeMagicBonuses@23452`). The three specified terms are untouched. |
...
| 9 | **§7 step 1 — XP goes through `S_story.xp` then `_checkLevelUp()`** | **SHIPPED** | The `reward` handler does exactly this: `if (E.checkLevelUp) E.checkLevelUp(); }@22299`, wired at `checkLevelUp: () =>@22376`. |
