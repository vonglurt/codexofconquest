...
- **Routing origin** — `function _playerPos@37731`, the player's cell, never the last named node.
...
**Runtime constants unchanged.** `function _roadGridPathCore@37820` still weights the route at `ROAD_CELLS.has(k) || SEA_LANES.has(k)) ? 1 : 2@37850`; `function _travelTick@38016` still steps at `_travelTimer = setTimeout(_travelTick, 120)@38043` and still halts on all four interrupt classes. `function _enterEmptyCell@28421` renders the room, and the old sentence *"The path continues. No named location marks this ground."* occurs **zero** times in the file.
...
| 4 | GLOBE panel click → jump map tab / world panel to that region (read-only aid) | ⚠ **Superseded.** §MAP-NAV wired the globe canvas to `function _navClickCell@37973`, which **travels** there — auto-walk via `_navTravelTo`, never a warp, so invariant #3 is intact. The read-only *centering* variant was ruled out of scope pending a pannable-view design and is recorded deferred in BACKLOG. |
