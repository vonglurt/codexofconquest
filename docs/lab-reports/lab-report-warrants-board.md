...
**Original date:** 2026-07-21 · **Measured against:** `play.html` @ `43bd09c` (`ENGINE_VER = coc-3.104.0`, 37,271 lines) — *all three figures re-checked exact*
...
does not. Survives at HEAD as `q.type === 'epic' || !q.activateNode@37217`.
...
|---|---|---|---|---|
| C1 | remote quest activation as a player verb | the dead `unlock` opcode — **first live consumer** | `function _acceptBounty(id)@37430` | ✔ **20 `unlock` authors** |
| C2 | `NODE_MAP[code].board` opt-in host flag | `node.isFishingLake` (32965) | `function _boardHost(node)@37241` | ⚠ **0 authors, ever** |
...
| C4 | postable-bounty predicate | `_questNodes()` scan + `canActivate` | `function _boardBounties(node, limit)@37386` | ✔ split out as `function _bountyPostable(q, node)@37352` |
| C5 | deterministic board rotation | the Town Crier's `gameDay % n` (26151) | `function _boardSeed(str)@37243` | ✔ unchanged |
| C6 | the board surface | `_mkSection`/`_mkCard` (32919/32932) | `story-board-section@35912` | ✔ unchanged |
...
Shipped **byte-identical** as `const BOUNTY_TYPES@37239`, and byte-identical at HEAD.
...
|---|---|---|---|
| `BOUNTY_TYPES` | 8-type `Set` | `const BOUNTY_TYPES@37239` | **byte-identical** |
| `_boardHost` | `return node && (…)` | `function _boardHost(node)@37241` | `!!node` coercion added |
| `_boardSeed` | `_boardSeed(code, day)`, builds `code+'\|'+day` inside | `function _boardSeed(str)@37243` | ⚠ **signature narrowed to one arg**; callers build the string; output identical |
| `_rewardStr(q)` | 8-line body | `function _boardReward(q)@37256` | ⚠ **renamed**; body byte-identical |
...
| `_acceptBounty` | 8-line body incl. `execBits` + toast + re-render | `function _acceptBounty(id)@37430` | `String()` coercion on the label; else identical |
...
> verbatim at HEAD in `function _bountyPostable(q, node)@37352`. ***A predicate that must mirror
...
already ships (`src/js/wbapi-core.js`) for any figure a decision rests on.***
