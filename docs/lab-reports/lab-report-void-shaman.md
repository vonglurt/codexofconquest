...
Declared contiguously at `vshamanFound: false@23141`. `vshamanDefeated` and
...
| Exact name matching on `i.name === "The Constructor's Log"` (the report's own §IV worry) | **Still true, still fragile** — and now *two* grant sites push the item (`action:'grant',name:'The Constructor@11138` and `27868`). A rename in either breaks the persuasion path silently. Low risk while both are literal. |
