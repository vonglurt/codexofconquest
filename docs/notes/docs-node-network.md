...
> ## ⚠️ Read this before trusting a node code or a coordinate in this file
>
> This document was written against the **retired 26×16 grid**, and §WALK replaced it with a 90×360 equirectangular geo grid. Three of its sections describe a world that no longer exists and are kept as **history — annotated, not rewritten** (§DX-02c / §AUDIT-03m precedent):
>
> | Section | Status | Why |
> |---|---|---|
> | **§2 Node Types / Act grouping** | ⚠️ HISTORICAL | Counts and code lists are pre-§WALK. `junction:true` is **0** live, not 10; `isFishingLake` is **1** (`BOO`), not 2. |
> | **§7 Dead-End Nodes** | ⚠️ HISTORICAL | *"`W → SL`"*-style single-neighbour claims were derived from 26×16 adjacency. The coordinates all changed; these adjacencies are unverified against the live grid. |
> | **§10 Grid Coordinate Table** | ⚠️ HISTORICAL | **Every `r,c` in it is from the retired grid.** Live coordinates span r 2–73, c 154–249. Do not copy a coordinate out of it. |
>
> **The live node reference is `docs/maps/node-index.md`** — generated by `npm run nodes`, gate-fenced by `check:nodeindex` (§AUDIT-03l). Never read a node code off a hand-maintained table.
>
> The rest of the file (§1, §3–§6, §8–§13) describes the **current** engine and was re-checked on 2026-08-04: 0 of 416 nodes carries an `N`/`S`/`E`/`W`/`SW`/`spire`/`portal` field, 20 Epic Battlegrounds, 38 `sleep:true` rest nodes.

---

## 1. Grid System

All nodes have a grid coordinate `{r, c}` stored in `NODE_COORDS`. The grid is **90 rows × 360 columns** — `const GEO_PROJ = { ROWS: 90, COLS: 360 }`, the §WALK equirectangular 1° projection. (The pre-§WALK text here claimed *"at least 16 rows × 200 columns"*; that was the retired grid.) Live node coordinates occupy r 2–73, c 154–249.

```
r = 0 (north) → 89 (south)     — band-clamped, no wrap
c = 0 (west)  → 359 (east)     — wraps at the antimeridian
```
...
## 2. Node Types ⚠️ HISTORICAL (pre-§WALK counts and codes — see the banner above)

> **Not a live table.** Kept for the shape of the record. Live as of 2026-08-04: **0** `junction:true`, **20** `isEpicBattleground`, **1** `isFishingLake` (`BOO` — `SSJ`/Yugurt Cabin is a rest node, not a fishing node), **38** `sleep:true`. The act histogram below is also stale: the live spread is 138/19/76/53/14/44/8/1 across Acts I–VIII with 63 nodes carrying no `act` at all.

| Type | Count | Code pattern | `junction` | `isEpicBattleground` | `isFishingLake` |
|------|-------|-------------|------------|----------------------|-----------------|
| Story nodes (main arc) | ~78 | `LHR`, `BMA`, `TLL` … `TLS` | — | — | — |  *(historical: `CI`=`LHR` · `SL`=`BMA` · `IN`=`TLL` · `CO`=`TLS`)*
| Named junction nodes | 10 | `J1`–`J7` (historical — none live), `J13`, `WRO`, `RD` (historical — no live node) | — | — | — |
| Epic Battleground | 20 | E* (`PRN`, `INV` … `TBS`) | — | `true` | — |  *(historical: `EF`=`PRN` · `EH`=`INV` · `EG`=`TBS`)*
| Fishing lake | 2 | `BOO` (historical `YL`), `SSJ` (historical `YC`) | — | — | `true` |
| DeFi Land (special Act I) | 3 | `ZRH`, `DUS`, `MUC` | — | — | — |  *(historical: `DF`=`ZRH` · `HM`=`DUS` · `GL`=`MUC`)*
| Mountain pass (hunt) | 1 | `GVA` (historical `MT`) | — | — | — |
| Ally Cat Arc | 1 | `CDG` (historical `CQ`) | — | — | — |
...
| Act | Nodes | Theme |
|-----|-------|-------|
| I | `LHR` `BMA` `CDG` `TLL` `MHQ` `LLA` `KRN` `HKG` `ZRH` `DUS` `MUC` | Birka city |  *(historical: `CI`=`LHR` · `SL`=`BMA` · `CQ`=`CDG` · `IN`=`TLL` · `TV`=`MHQ` · `BA`=`LLA` · `CR`=`KRN` · `CY`=`HKG` · `DF`=`ZRH` · `HM`=`DUS` · `GL`=`MUC`)*
| II | `LCY` `LGW` `STN` `SEN` `GOT` | Saltwick / Tilbury |  *(historical: `DK`=`LCY` · `MQ`=`LGW` · `SF`=`STN` · `MS`=`SEN` · `AL`=`GOT`)*
| III | `MAN` `FRO` `KIR` `MSY` `SDQ` `BGI` `GVA` `BOO` `SSJ` | The Wilds |  *(historical: `MI`=`MAN` · `FO`=`FRO` · `HL`=`KIR` · `SW`=`MSY` · `HS`=`SDQ` · `BE`=`BGI` · `MT`=`GVA` · `YL`=`BOO` · `YC`=`SSJ`)*
| IV | `HOR` `PDL` `RAI` `GIB` `TRF` `SID` | The Deep / Ocean |  *(historical: `OC`=`HOR` · `IS`=`PDL` · `AT`=`RAI` · `SC`=`GIB` · `FL`=`TRF` · `DS`=`SID`)*
| V | `SFT` `VBY` `TRD` `NAS` `FEZ` `VIE` `CLJ` | Ashcrag / Visby |  *(historical: `SE`=`SFT` · `BK`=`VBY` · `GC`=`TRD` · `PC`=`NAS` · `MC`=`FEZ` · `CA`=`VIE` · `VC`=`CLJ`)*
| VI | `AHB` `DOH` `BEL` `ERF` `NUE` `EGE` | Desert & Weimar |  *(historical: `DE`=`AHB` · `DC`=`DOH` · `JU`=`BEL` · `BQ`=`ERF` · `SQ`=`NUE` · `OU`=`EGE`)*
| VII | `HER` `BHD` `BKK` `CTU` `LYR` | Mythic |  *(historical: `GA`=`HER` · `KT`=`BHD` · `OP`=`BKK` · `HC`=`CTU` · `AR`=`LYR`)*
| VIII | `TLS` (historical `CO`) | Finale |
...
> **⚠️ Corrected 2026-08-04 (§AUDIT-03m-FU).** The sentence below said *"Remaining named nodes with J-prefix (`WRO`–`J7` (historical — no live node), `RD` (historical — no live node)) … serve as ordinary named story locations."* **They do not exist.** `WRO`–`J7` (historical — no live node) and `RD` (historical — no live node) are absent from `NODE_MAP`; the only survivors of the J-purge are **`J13`** (The Western Sea Road, now at r19,c174) and **`WRO`** (Midlands Road Fork, r18,c197), and both carry `junction:false`. The coordinate list below is retired-grid history. **Zero nodes carry `junction:true` today** (§DX-01a removed the last two, J14/J15).

Remaining named nodes with J-prefix (`WRO`–`J7` (historical — no live node), `RD` (historical — no live node)) have no `junction` flag and serve as ordinary named story locations at their grid coordinates.
...
## 7. Dead-End Nodes ⚠️ HISTORICAL (26×16 adjacency — see the banner above)

> **Not a live table.** These single-neighbour claims were derived from the retired 26×16 grid; every coordinate in that grid changed at §WALK, so none of the `W → …` relations below has been re-verified against the live `CELL_GRID`. The mechanism described is still real — exits *are* derived from `CELL_GRID` adjacency at runtime — but the specific pairs are history. To ask which cells actually border a node now, read `NODE_COORDS` or call `GET /api/node/:code` and look at `derived_exits`.
...
|------|-------|---------------------|
| `CDG` (historical `CQ`) | The Cat Quarter | W → `BMA` (historical `SL`) |
| `DUS` (historical `HM`) | Frequency Row | W → `ZRH` (historical `DF`) |
| `MUC` (historical `GL`) | Old Guard's Corner | E → `ZRH` (historical `DF`) |
| `STN` (historical `SF`) | The Map Shop | W → `LGW` (historical `MQ`) |
| `SEN` (historical `MS`) | Aboard the Tilbury Star | W → `LCY` (historical `DK`) |
| `SSJ` (historical `YC`) | Yugurt Cabin | N → `BOO` (historical `YL`) |
| `EGE` (historical `OU`) | The Observatory Outhouse | W → `ERF` (historical `BQ`) |
| `HER` (historical `GA`) | Greek Agora | N → `BHD` (historical `KT`) |
| `TLS` (historical `CO`) | Cosmic Realm | S → `LHR` (historical `CI`) (game finale) |
...
|---------|-------------|-------------|
| `storyPortal()` + `node.portal` | Instant `EGE`→`HER` teleport | Walk the grid (`EGE` and `HER` are navigable by cell path) |  *(historical: `OU`=`EGE` · `GA`=`HER`)*
...
The diagonal (south-west) and `spire` direction fields were stripped in §CELL-01 — re-measured 2026-08-04, **0 of 416 nodes carries any of `N`/`S`/`E`/`W`/`SW`/`spire`/`portal`**, so the removal held.

> **⚠️ Corrected 2026-08-04 (§AUDIT-03m-FU).** This section used to end *"DS → ED and HC → EK are co-located in CELL_GRID at adjacent cells."* That was true of the 26×16 grid and is **not** true now: `SID` (ex-`DS`) sits at r53,c157 while `OTP` (ex-`ED`) is at r25,c206; `CTU` (ex-`HC`) is at r30,c247 while `MAD` (ex-`EK`) is at r29,c176. Neither pair is adjacent, or close. §WALK re-placed every node geographically — no pre-§WALK adjacency claim survives it.
...
## 10. Grid Coordinate Table ⚠️ HISTORICAL — DO NOT COPY A COORDINATE FROM HERE

> **Every `r,c` below is from the retired 26×16 grid, and every code is a pre-§WALK name.** Live coordinates span r 2–73, c 154–249 in the 90×360 geo grid — e.g. this table puts `CI` at `5,16`; the node it became (`LHR`) sits at `17,190`. The table is preserved because it records what the world looked like when the arcs were authored, which is why so many docs still speak in these codes. **The live reference is `docs/maps/node-index.md` (`npm run nodes`).**
>
> *(The block is fenced, so `check:legacycodes` deliberately does not annotate it — a fenced block is code, not prose. That exemption is why this banner has to say it in words.)*

### Retired 26×16 coordinates (core 78 story/junction nodes)
...
The game's Map sheet carries the multiplayer UI as sub-tabs (🗺 Local · 🌍 World · 🛰 Full · 🌐 Multiplayer · 🔭 Discover · 🛡 Lists) — player-facing walkthrough: `mechanics.md §Multiplayer` "Connection center"; design + locked decisions (D2/D3/D4/D6/D7): `docs/lab-reports/lab-report-mesh02-connections-ui.md`. Server surfaces added for it: **`GET/PUT /api/mesh/acl`** (validated merge-write ACL editor over `MESH_ACL_FILE`, comment keys preserved, hot-reload) · **`GET /api/mesh/blocklist`** (403 until the `shareBlocklist` opt-in; publishes the three block* lists only — peers preview and merge manually, never automatically) · **`POST /api/mesh/connect`** (runtime peer/tracker dial, same shapes as `--peer`/`TRACKER_URL`, dials in the same request, outbound ACL-gated) · **`GET /api/session/chat[?limit=&r=&c=]`** (global chat-history ring for the 💬 panel) · footprints on `buildLook` (§MESH-02j — per-cell `[{pid,name,agoMs}]`, ≤8/cell, 30-min TTL, display-only). CLI parity (§MESH-02g): `./bin/api mesh acl|blocklist|connect` — see `wbapi-help.md §Mesh API`. Everything here is connection/display layer: the mover reads none of it.

**Test gates:** `npm run test:mud` — 270 checks, sections [A]–[R] (incl. [R] §MESH-02a ACL editor + blocklist share flip) (incl. [P] rate limiting, the `./bin/api mesh` CLI wrappers, and [Q] §MESH-01-FU 11–13 ACL template / tracker cache+bootstrap / chat backlog) + [H] §MESH-01h sentry cases (deploy→presence, deterministic encounter suppression, recall→leave, prune-immunity), including the Inc (e) partition-heal harness (3 servers + tracker: convergence, exactly-once across partitions via ACL-file split/heal, stale-replica availability, snapshot re-convergence, incompat-refusal + world-group segregation). Playwright: `multiplayer-presence.test.js` 7/7, `mesh-sentry.test.js` 7/7 (client half), `mesh-copresence-buff.test.js` 6/6, `mesh-hireling-guide.test.js` 6/6, `worldbuilder-mesh.test.js` + mesh tab 4/4, `mesh-connections-ui.test.js` 8/8 (§MESH-02f — hermetic connection-center UI, `:1367` route-blocked).
...
**Authoring (edit.html):** §NAV-01g drag-&-lock cities (marker drag / lat-lon → `PUT /api/coords`; 🔒 lock persists into `src/config/roads-pins.json`'s `locked`, geo-seed never moves locked cities) · §NAV-01h road-net editor (ROAD_RUNS chain-link overlay; vertex drag → pin; ✚ intersection / ┬ T-junction palette; 🔗 link toggle; 🗑 delete; **♻ Reweave Net** = `PUT /api/roads` → `build-roads.js --apply` → `check:roads`, red check rolls the game file back). CLI: `./bin/api roads | reweave`.
