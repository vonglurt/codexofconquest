...
The server reads `play.html` into memory. Every write mutates in-memory objects and then persists to disk automatically — a temp file beside `play.html`, renamed into place (atomic, so a reader never sees a half-written game file). `POST /api/save` is a *different* thing: it writes a **dated snapshot** next to the game file and copies it over, which is what `monitor-snapshots.py` / `archive-snapshots.sh` turn into the `milepoints/patches` chain. You do not need it to persist a write. *(§DX-02k, 2026-08-03 — before that fix every write went through the snapshot path and left the ~5.4 MB file behind, once per PUT.)*
