...
|---|---|---:|---:|---|
| **1** | [Playable Truth](BACKLOG-1-playable-truth.md) | 11 | 6 | Shipped content the player can never reach or complete: unreachable act-gated beats, inert counters, gate flags with no writer, quest deadlocks, endings out of range. |
...
| **5** | [Platform & Tooling](BACKLOG-5-platform-tooling.md) | 27 | 21 | Everything outside the game loop: the WBAPI server and its endpoints, the worldbuilder and editors, the HTML parse/export layer, multiplayer mesh, the NPC-speak service. |
| **6** | [Verification & Documentation](BACKLOG-6-verification-docs.md) | 49 | 17 | What proves the rest is true: CI gates and parity fences, Playwright and MUD suites, the anchor resolver, engine comments, maintained-doc rot, and the §DOC-02 lab-report verification program. |
