...
**Finesse (§WEAP-FIN):** exactly **two** base types declare `finesse: true` — **Scimitar** and **Rapier** — which is **10 of the 70** items once the five magic tiers expand. A finesse weapon keys its attack and damage off whichever of STR/DEX is higher; every other weapon keys off STR. **`function _storyWeaponIsFinesse@25597` is the single source**, and it resolves the equipped weapon's `tier` against `WEAPON_ITEMS` rather than trusting the stored flag, so saves written before the field existed are correct on load. `function _storyAtkAbility@25604` returns `{mod, label}` and the character sheet prints the label, so the sheet cannot name STR while the engine rolls DEX (§AUDIT-03ae). **Bow and Crossbow are NOT finesse** — a ranged attack keys off DEX outright rather than `max(STR, DEX)`, and that is the open row **§WEAP-RANGED**, not this one.
