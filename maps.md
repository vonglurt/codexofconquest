...
| LJ3 | 117 | junction | IV | off-map §SIREN-01 | The Serpent Passage — Sea Serpent (solo). ⚠ NOT AS-BUILT: no LJ0–LJ3 junction exists in NODE_MAP (only the courts shipped); the navigator trigger lives at LSO (§VM-01-G1-FIX) |
...
> **§SIREN-01 — Littoral Courts (Nodes 111–120):** Ten-node sequential ocean arc in the extended south-west grid. Entry via DS east probe (DS at r:25,c:10; LJ0 at r:25,c:14 — 4 cells east, within probe limit). Chain runs south at c:14 from LJ0 (r:25) to LCA (r:41) in 2-row steps; LSO branches east at (r:37,c:18). Three sea-battle junctions (LJ1/LJ2/LJ3) alternate with four port-courts (LC1/LC2/LC3/LC4) — ⚠ the junctions are NOT as-built (no LJ0–LJ3 in NODE_MAP; only the courts + LCA + LSO shipped, and as-built coords differ from this section's plan). The navigator-trigger panel renders at LSO while the fog is unresolved (§VM-01-G1-FIX; it was keyed to the unbuilt LJ3 and had never rendered). LCA is the arc terminal with a three-variant betrayal-count arc-close. LSO is the Overseer dead-end (WIS DC 15, Succubus/Incubus telepathic bond mechanic). Betrayal flags: `betrayalThought` (LC1 fail), `betrayalWord` (LC2 fail), `betrayalDeed` (LC3 fail). All nodes Act IV.
