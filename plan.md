...
> **Updated 2026-06-26 · branch `main` · last commit `b726a44` (§WALK-4 Inc 3 uncommitted).** This is the canonical handoff — the most current snapshot of where the work stands and everything still outstanding. (The top-of-file §RESUME just points here.)
...
**§WALK-5** — MUD multi-client harness; instanced per-session encounters on `session/move`. §WALK-4 is now **complete** (Inc 1 invariant proof + Inc 2 CI gate + Inc 3 rebuilt nav tests; surfaced + fixed 3 latent bugs incl. a P0 empty-cell crash). §WALK-5 wires server `terrainAt`/`encounterRate` (per §WALK-2-FU), keeps co-presence social but combat instanced per session, and asserts no cross-session encounter bleed. **Needs a `lab-report-*.md` design pass first** (lab report §7 has the v1 decision table). Also still open: **§WALK-1.5-FU(a–d)** (geo cleanups), **§WALK-2-FU** (cleanup-cruft.sh hook bug; ferry hook).
...
- **§WALK series (sequential):** §WALK-3 ✅ + §WALK-4 ✅ complete → **§WALK-5** (MUD multi-client harness; instanced per-session encounters on `session/move`) — **needs a `lab-report-*.md` design pass first** (lab report §7 has the v1 instanced-vs-shared decision table).
...
>
> 1. **§WALK-5** (MUD multi-client harness) — **write `lab-reports/lab-report-walk5-mud-harness.md` FIRST** (lab report §7 has the v1 instanced-vs-shared decision), then implement: instanced per-session encounters on `session/move`, social co-presence (chat/`who`/shared `look`) but combat not shared; wire server `terrainAt`/`encounterRate` (per §WALK-2-FU); assert no cross-session encounter bleed. Resolve the inert ferry hook (author `FERRY_EDGES` vs delete).
