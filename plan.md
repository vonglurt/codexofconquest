...
>
> **§MESH-02 (Map-Tab Connection Center) is IN FLIGHT** — design locked in `lab-reports/lab-report-mesh02-connections-ui.md`; increments **(a) mesh ACL/blocklist endpoints · (h) multi-user chat history · (i) runtime mesh connect ✅ SHIPPED 2026-07-06/07** (`14bd572`, `2ca5179` — full ship records in the Multiplayer section rows). Gates at (h)/(i): mud-harness **251/251** · mesh Playwright **36/36** · two-server live verify. Same session: §DATA-01-REVERTED ✅ resolved (superseded by UQF; journal-renderer textContent residual shipped `1479e29`) · cellMove crash on pre-§CELL-04 saves fixed (`d1216f6`, loads layer `_S_DEFAULTS()` under saved data, autosave+navigation 39/39) · wbapi-toggle port check LISTEN-only · mud-harness main server pid-scopes its peers cache.
>
> **Next on "continue": §MESH-02 increment (d)** — Discover pane (🖥 local port scan of localhost:1360–1380 per D6, tracker/magnet resolve into the pane, `mpListSources` + `mpParseServerList` with D4 whitelist-gated auto-load; (b) shell + (c) Connect pane ✅ shipped 2026-07-06 — `mpResolveMagnet(ui)`/`_mpRenderServerRows(rows,contId)` already container-parameterized, reuse them), then (e) Lists (client lists + ACL editor + peer-blocklist preview/merge) → (f) tests → (g) docs/CLI. After §MESH-02 closes: pick a planned arc — §GR / §DESIGN-03 / §DUNGEON-01 / §MATH-01 — each needs its lab report locked before any HTML edit (Game Content below). *(The §MESH-01-REVIEW refactor trio — mesh.js extraction, `_mpRepaintMaps` debounce, `_mpBase()` origin fix — ✅ all SHIPPED 2026-07-06; only minor conditional notes remain in the Multiplayer section.)*
>
