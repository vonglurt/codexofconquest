...
> **⟶ ACTIVE: UQF Wave 3** (side quests → declarative completion — see §ARCH-01; a fresh session should read `lab-reports/lab-report-uqf-migration-playbook.md` first). §NAV-01 is **fully closed** (Inc a–h + docs close-out + `lab-reports/lab-report-nav01-navigable-world.md`, 2026-07-03) — only the small follow-ups list under §NAV-01 remains as backlog. The §MESH-01 gameplay ladder (f–j, under Multiplayer) queues after UQF W3 unless redirected. **New decision item found during the docs sync: §CELL-13-REVERTED** (jump-travel back in code — see Data / Architecture).
>
...
- [ ] **§CELL-13-REVERTED — jump-travel removal partially reverted** (found 2026-07-03 during the §NAV-01 docs sync) — §CELL-13 (2026-06-15) recorded `storyPortal()`, `storyUseTransmort()`, `storySetHearthHome()`/`hearthHome` as removed ("all jump-travel removed"), but **all are live in the current code** — grep-verified; `storyPortal` even carries a §MESH-01a `mpBeacon()` call, and mechanics.md still documents Hearth Home + Transmort as player features (those sections match code today). Same snapshot-rollback era as §DATA-01 (2026-06-16). The N/S/E/W edge-field strip, corridor removal, and `GATE_LOCKS` removal DID persist. **Decision needed:** re-remove jump-travel (§CELL-13 intent — pure cell-walk world; note §MESH position beacons assume `cellMove`-shaped moves, though the beacon is passability-validated so teleports still render) vs accept + document (delete §CELL-13 claims from docs-node-network.md §8, keep mechanics.md sections). Docs currently carry ⚠️ flags on both sides (maps.md FL1, docs-node-network.md §8).
