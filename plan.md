...
> **⟶ ACTIVE: UQF Wave 3** (side quests → declarative completion — see §ARCH-01; a fresh session should read `lab-reports/lab-report-uqf-migration-playbook.md` first). §NAV-01 is **fully closed** (Inc a–h + docs close-out + `lab-reports/lab-report-nav01-navigable-world.md`, 2026-07-03) — only the small follow-ups list under §NAV-01 remains as backlog. The §MESH-01 gameplay ladder (f–j, under Multiplayer) queues after UQF W3 unless redirected. §CELL-13-REVERTED (jump-travel back in code, found during the docs sync) was **resolved same day** — user directed re-removal; see the checked item under Data / Architecture.
>
