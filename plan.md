...
> **Updated 2026-06-28 · branch `main` · last commit `df36c93`.** Active work = **§ARCH-01 Phase 3 UQF migration**, Wave 1, **one arc per "continue"** (each increment: commit + `say` the subject + sync plan.md / index.md / the migration playbook). **Through Wave 1o:** ~85 quests migrated; `quest-runtime-uqf` **90 tests green**; `check:walk` + `navigation` 14/14 green. **Next op:** migrate the `bilge` arc (§WHODUNIT-01), then `alch`, `scar`, + ~10 singletons (incl. `sea_overseer`) to finish Wave 1. Per-wave detail in the §ARCH-01 entry below + `lab-reports/lab-report-uqf-migration-playbook.md`. **All closed / completed work has been archived → `plan-archive.md`.**
