...
>
> **Next on "continue": §MESH-01-FU 10 — `./api.sh mesh` commands** (`mesh status` / `mesh peers` / `mesh tracker <url>` CLI wrappers — API-first parity with the Mesh tab), or FU 11–13 · §MESH-01-REVIEW refactors · §DATA-01-REVERTED decision · or a planned arc (**§GR / §DESIGN-03 / §DUNGEON-01 / §MATH-01** — each needs its lab report locked before HTML edits; see Game Content below). *(FU 7+8 ✅ → archived; FU 9 world-diff depth ✅ 2026-07-06 — see the FU list; mud-harness **235/235** incl. [P] rate limiting.)*
>
