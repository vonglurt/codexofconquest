...
> **Updated 2026-06-29 · branch `main` · last commit `d84c87b`.** Active work = **§ARCH-01 Phase 3 UQF migration**, now entering **Wave 2**, **one arc/cluster per "continue"** per the Directive. Each increment: migrate the next arc → commit → `say` the subject → sync plan.md / index.md / the migration playbook → `npx playwright test quest-runtime-uqf` + `npm run check:walk` + `npx playwright test navigation`.
>
> **Verification (current):** `quest-runtime-uqf` **122 passed** (Wave 1v ×3-repeat clean); `check:walk` green; `navigation` 14/14. ~115 quests migrated through **Wave 1v** (Highland trade `df_02`/`sk_02` + folk wisdom `lxvii67`/`guide_04`) — **Wave 1 COMPLETE** (all skill-check arcs with closures + all Wave-1 singletons). Docs + memory synced; playbook Wave-1 closeout current.
>
> **Next (on "continue"):** begin **Wave 2** — the **~2149 simple `checkStat` skill_checks** (no onPass/onFail closures, no checkPassFlag). These are pure mechanical `activateCond→gate` + `checkStat→bits[skill_check]` + `xpAward→reward{xp}` transforms → **script-assisted bulk migration** (a generator pass + a parity test table), not hand-edit-per-quest. First op: pick the migration unit (by node cluster or quest-id prefix) and stand up the bulk transform + its validation harness. → then Waves 3–7 (see the §ARCH-01 entry below). Per-wave detail in that entry + `lab-reports/lab-report-uqf-migration-playbook.md`. **All closed / completed work is archived → `plan-archive.md`.**
