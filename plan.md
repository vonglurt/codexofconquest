...
>
> **Next on "continue": §MESH-01-FU 8 — ingress rate limiting** (per-IP token bucket on the unauthenticated mesh POSTs — gossip/announce/sync — before JSON parse). After that: FU 9–13 · §MESH-01-REVIEW refactors · §DATA-01-REVERTED decision · or a planned arc (**§GR / §DESIGN-03 / §DUNGEON-01 / §MATH-01** — each needs its lab report locked before HTML edits; see Game Content below).
>
