...
>
> **Verification (current):** `quest-runtime-uqf` **96 passed**; `check:walk` green; `navigation` 14/14. ~96 quests migrated through **Wave 1q** (`alch` — Roen's Personal Legend). Docs + memory synced; playbook Wave-1 progress current.
>
> **Next (on "continue"):** migrate the **`scar`** arc, then the **~10 Wave-1 singletons** (incl. `sea_overseer` — a `checkAbility` singleton with the `checkFailFlag` pattern) to close out **Wave 1** → then Waves 2–7 (see the §ARCH-01 entry below). Per-wave detail in that entry + `lab-reports/lab-report-uqf-migration-playbook.md`. **All closed / completed work is archived → `plan-archive.md`.**
