...
>
> **Next on "continue": an ASK task — every remaining backlog item needs a user decision.** Options: **§1367** (8 clarification questions gate it, `project_1367_setting`) · **§GR-D** Froberger Entry 42 (needs NG+ tracking, unsupported) · the flagged latent quest bugs awaiting approval (`project_open_gaps` items 3/4/6: skill_check 'done'-vs-'complete' dead handlers, onComplete+xpAward double-counts, guide_02/03/06 dead gates) · the monster-drop nerf · `[INVESTIGATE]`/reading-circle UI gaps · §NAV-01-FU small items · §MBIT-02 remaining (journal token timeline only — `_takeMissionBit` safety ✅ + worldbuilder schema audit ✅ done) · the two pending Design Decisions (arc-ID field, §MBIT-02-E). New arcs need their `lab-report-*.md` locked before any HTML edit.
>
