...
> **Updated 2026-06-29 · branch `main` · last commit `ae4f6de`.** Active work = **§ARCH-01 Phase 3 UQF migration**, **Wave 2 (script-assisted bulk)** in progress, **one family/cluster per "continue"** per the Directive. Each increment: migrate the next family via `scripts/uqf-bulk-migrate.js` → commit → `say` the subject → sync plan.md / index.md / the migration playbook → `npx playwright test quest-runtime-uqf` + `npm run check:walk` + `npx playwright test navigation`.
>
...
>
> **Next (on "continue"):** migrate the **next Wave-2 family/cluster** (~2085 well-formed legacy skill_checks remain). **Key scoping finding:** across ALL top families, **0 have a "complex" activateCond** — every gate is either absent (→`gate:{}`) or the trivial `()=>!!S_story.<flag>` (→`gate:{flags:[…]}`); no xp/gold/bitLabel/retry/fail-flag anywhere. So the migrator handles every remaining family with **no `_legacyFn` fallbacks**. Good next units by size (all `checkPassFlag`, mixed none/trivial gates): `ath` (93), `lis` (65), `zth` (59), `flr` (55), `hft_`/`rkv_` (50), `ist` (48), `rix_` (47), `ost_` (46), `arn_` (43), `clj`/`nwi`/`vby_` (42). Protocol per family: capture golden → `--prefix <fam>` → verify byte-for-byte → +self-contained tests. **NB:** 30 degenerate book-stubs (`blq_*`: `reward:NaN`, `activateCond:()=>!!S_story.null`, no checkDC) are **out of scope** — never well-formed/activatable. → then Waves 3–7 (see §ARCH-01 entry). **Archive → `plan-archive.md`.**
