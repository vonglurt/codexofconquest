...
**Current state (2026-06-26, last commit `5a54657`):** Thread **§TIMELESS-01** (timeless one-cell movement + Hunt-feature removal) is **complete — Inc A + B + C + D all done** (movement no longer advances the clock; all Hunt/Stalk JS + UI/DOM/CSS removed; quest comment + map-click wording + scoped doc sync done; verified 0 console errors in headless Chromium). **Immediate next: §WALK-3 Inc 3** (delete dead `reweave-all` body + orphaned `reweave` CLI). Also newly queued: **§TIMELESS-01-FU** (deep Hunt-residue doc sweep across 7 spec docs — see §TIMELESS-01). Prior thread §WALK-1/1.5/2 + §WALK-3 Inc 1/2 done. Working tree clean except untracked `1367-sources/pla.md` + incidental `milepoints/api-cli.log` churn (hook-managed; don't stage). wbapi-server restarted (in-memory matches disk). Full status + copy-paste prompt at the bottom of this file.
...
>
> 1. **§WALK-3 Inc 3** — delete the dead `reweave-all` body in `wbapi-server.js` (~3,200 lines below the early-return at :5763, incl. P1 rip-and-connect / P4 fix-all-broken / P5 fix-bidirectional phases) **and** the orphaned `reweave` CLI orchestrator in `api/wb.js` (already un-advertised; it called the now-410 rip-and-connect). Restart the server; confirm the endpoint table + `./api.sh` still work (`./api.sh reachability` = 409/409, mover parity green).
> 2. **§TIMELESS-01-FU** (deep Hunt-residue doc sweep) — retire/rewrite the stale Hunt/Stalk/`HUNTING_GROUNDS`/`huntMode` references still in 7 deeper docs (`monsters.md` F5 milepoints + fn-table rows 733–736, `maps.md` 614/627, `spec-engine.md` 655/792/890/892, `spec-world.md` 556, `mechanics.md` 653, `docs-node-network.md` 277, `cell-resume-prompts.md` 573/581/1154). Fold a §WALK line-number re-audit into the same pass.
