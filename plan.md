...
> **Updated 2026-06-29 · branch `main` · last commit `a1b67db`.** Active work = **§ARCH-01 Phase 3 UQF migration**, **Wave 1**, **one arc per "continue"** per the Directive. Each increment: migrate the next arc → commit → `say` the subject → sync plan.md / index.md / the migration playbook → `npx playwright test quest-runtime-uqf` + `npm run check:walk` + `npx playwright test navigation`.
>
> **Verification (current):** `quest-runtime-uqf` **110 passed** (Wave 1t ×3-repeat clean); `check:walk` green; `navigation` 14/14. ~107 quests migrated through **Wave 1t** (biblical singletons `stoning_lystra` + `basket_damascus`). Docs + memory synced; playbook Wave-1 progress current.
>
> **Next (on "continue"):** the **remaining ~8 Wave-1 singletons** to close out **Wave 1**. The sweep found 14 un-migrated onPass/checkFailFlag skill_checks; SIREN (5) + biblical (2) now done. Remaining clusters: **Atlantean iodine chain** `iodine_01`/`shore_02`/`forge_01`/`sunken_01` (item/gold/knowledge onPass closures → `_legacy_fn`); **Highland trade** `df_02`/`sk_02` (`checkStat`, rich onPass+onFail msg); **folk wisdom** `lxvii67` (HKG jester, `faith_folk++`) + `guide_04` (SSJ, `emmerStage4a`). → then Waves 2–7 (see the §ARCH-01 entry below). Per-wave detail in that entry + `lab-reports/lab-report-uqf-migration-playbook.md`. **All closed / completed work is archived → `plan-archive.md`.**
