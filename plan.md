...
>
> **Next on "continue": §MESH-01-FU 11–13** (11 `mesh-acl.json.example` template · 12 tracker persistence + federation bootstrap · 13 chat backlog on join — all small, batchable in one pass; closing them closes §MESH-01-FU entirely), then §MESH-01-REVIEW refactors · §DATA-01-REVERTED decision · or a planned arc (**§GR / §DESIGN-03 / §DUNGEON-01 / §MATH-01** — each needs its lab report locked before HTML edits; see Game Content below). *(FU 9 world-diff ✅ + FU 10 `./api.sh mesh` CLI ✅ 2026-07-06 — see the FU list; mud-harness **239/239** incl. 4 new CLI-parity checks.)*
>
...
- [ ] **§MESH-01-FU — remaining follow-ups** (small; numbering preserved from the archived checkpoint list; 7 publish-bootstrap.sh + 8 ingress rate limiting ✅ 2026-07-06 → archived):
