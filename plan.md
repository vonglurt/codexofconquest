...
**Current state (2026-06-26, last commit `d602b7f`):** Threads **§TIMELESS-01** (Inc A–D + FU all done — incl. the Hunt-residue doc sweep across all 7 deeper spec docs) and **§WALK-3** (Inc 1–3 done; deleted the 3,240-line dead `reweave-all` body + the orphaned `reweave` CLI, verified reweave-all/rip-and-connect→410, reachability 409/409, mover parity + behaviour green) are both **complete**. **Immediate next: §WALK-4** (CI-gated invariant suite + Playwright rebuild). Working tree clean except untracked `1367-sources/pla.md` + incidental `milepoints/api-cli.log` churn (hook-managed; don't stage). wbapi-server restarted (in-memory matches disk). Full status + copy-paste prompt at the bottom of this file.
...
- [ ] **§WALK-4** — CI-gated invariant suite: reachability proof (I1/I2/I3) + walk parity (structural inline-identity + behavioural kernel trace). Spec: lab report §6.
  ...
  - Inc 2 ⬜ **CI wiring** — `.github/workflows/` job running the invariant proof + `check-mover-parity` + `check-mover-behaviour` (the structural + behavioural walk-parity halves, which already exist) on push/PR touching `mover.js`/`roll2hit-v3.html`/`wbapi-server.js`; add an `npm run` alias.
  - Inc 3 ⬜ **rebuild stale Playwright nav tests** — `tests/integration/navigation.test.js` (+`worldbuilder-walk.test.js`) hardcode pre-§WALK-1.5 coords/adjacencies (BOO 47,223→2,194; LHR+BK co-locate at 10,197). Reseed against current geo; assert one-cell moves + sea-block (§WALK-1.5-FU(e)).
...
**§WALK-4 Inc 2** — CI wiring. Inc 1 ✅ (the `scripts/check-invariants.js` I1/I2/I3 proof is green and already caught + fixed two latent bugs). Next: add a `.github/workflows/` job that runs `check-invariants` + `check-mover-parity` + `check-mover-behaviour` on push/PR touching `mover.js`/`roll2hit-v3.html`/`wbapi-server.js`, plus `npm run` aliases. Then **Inc 3** — rebuild the stale Playwright nav tests (§WALK-1.5-FU(e)).
...
>
> 1. **§WALK-4** — CI-gated invariant suite (I1/I2/I3 reachability proof + walk-parity) and rebuild the stale Playwright nav tests. **§WALK-5** — MUD multi-client harness; instanced per-session encounters on `session/move` (wire server `terrainAt`/`encounterRate` per §WALK-2-FU; resolve the inert ferry hook).
