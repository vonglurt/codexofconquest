...
- [ ] **§WALK-5** — MUD multi-client harness; instanced per-session encounters on `session/move`; assert no cross-session encounter bleed. **Lab report ✅ DESIGN LOCKED** — `lab-reports/lab-report-walk5-mud-harness.md` (2026-06-26). Reconciled the parent §7 sketch with current code: (1) **no `huntMode`** (§TIMELESS-01 removed it — roll is just `roll < baseRate`); (2) blocked move stays **409** (the parent's `200 {ok:false}` predates the live handler — additive on the 200 path only); (3) encounter stored at **`s.encounter`** (new field), NOT `s.state.encounter` (`s.state` is the `'active'` lifecycle string serialized by `who`). 4-increment plan: **Inc 1** wire `terrainAt`/`encounterRate`/`SEA_LANES` into `getMoverWorld()` + terrain-parity guard in `check:walk`; **Inc 2** per-session seeded RNG + instanced roll on `session/move` success path; **Inc 3** `tests/mud-harness.*` K-client driver (asserts co-presence delivery + instancing + social state); **Inc 4** TTL-prune assert + doc-sync. Ferry **deferred not deleted** → **§WALK-5-FU** (author `FERRY_EDGES` for server+SP together, or drop the kernel branch). Monster pick uses **flat tier weights** (server has no notoriety state) — logged SP/MP divergence.
...
> **Updated 2026-06-26 · branch `main` · last commit `d44f8b9` (§WALK-4 complete; §WALK-5 lab report committed next).** This is the canonical handoff — the most current snapshot of where the work stands and everything still outstanding. (The top-of-file §RESUME just points here.)
...
**§WALK navigation-core redesign** (lab report: `lab-reports/lab-report-terrain-field-mover-redesign.md`): **§WALK-1** (junctions deleted), **§WALK-1.5** (geo re-projection + SEA_MASK + sea-lane land-bridges), **§WALK-2** (shared `mover.js`), **§WALK-3** ✅ (reachability=land-flood 409/409, mutators→410, dead `reweave-all` body + `reweave` CLI deleted), **§WALK-4** ✅ — Inc 1 invariant proof (`scripts/check-invariants.js`, I1/I2/I3), Inc 2 CI gate (`.github/workflows/walk-invariants.yml` + `npm run check:walk`), Inc 3 rebuilt Playwright nav tests. **§WALK-4 caught + fixed 3 latent bugs:** orphaned `junction` WORLD_DB terrain, VBY `'bar (Visby)'`→`'bar'` terrain key, and a **P0 `_enterEmptyCell` crash** (wrote to non-existent `#story-content` → every empty-cell step threw; restored the §UNIFY-01 shared-shell render). **§WALK-5 lab report ✅ DESIGN LOCKED** (`lab-reports/lab-report-walk5-mud-harness.md`) — implementation pending (Inc 1: wire `terrainAt`/`encounterRate`/`SEA_LANES` into `getMoverWorld()`).

**Test integrity (all green):** `npm run check:walk` (invariants + structural + behavioural parity); Playwright integration suite — `navigation` 13/13, `worldbuilder-walk` 18/18, `autosave` 4/4 (2 stale-geo tests reseeded LHR→BMA), `debug`+`fishing` 10/10. Working tree clean except untracked `1367-sources/pla.md` + incidental `milepoints/api-cli.log` churn (pre-commit hook manages `milepoints/`; never stage it). Verify nav-tooling with `./api.sh reachability` (409/409) + `node scripts/check-mover-{parity,behaviour}.js`; full Playwright via `npx playwright test`.

### Immediate next step

**§WALK-5 — MUD multi-client harness.** Lab report ✅ **DESIGN LOCKED** (`lab-reports/lab-report-walk5-mud-harness.md`, 2026-06-26). Implement per the 4-increment plan in the report §7: **Inc 1** — wire `terrainAt`/`encounterRate`/`SEA_LANES` into `getMoverWorld()` (mirrors client `_inferTerrain`/`TERRAIN_ENCOUNTER_RATE`; parse `SEA_LANES` from `_rawSrc` like `getImpassable()` does `SEA_RUNS`) + add a server↔client terrain-parity guard to `npm run check:walk`; **Inc 2** — per-session seeded RNG (`s.seed`/`s.rngState`) + the instanced encounter roll on the `session/move` **success** path, stored at **`s.encounter`** (NOT `s.state` — that's the `'active'` lifecycle string); **Inc 3** — `tests/mud-harness.*` K-client driver asserting co-presence delivery + instancing (each client's trace is pure fn of its seed+path) + social state; **Inc 4** — TTL-prune assert + doc-sync. Key reconciliations baked into the report: **no `huntMode`** (roll is `roll < baseRate`), blocked move stays **409** (additive on 200 path only), ferry **deferred → §WALK-5-FU**, monster pick uses **flat tier weights** (server has no notoriety state). Also still open: **§WALK-1.5-FU(a–d)** (geo cleanups), **§WALK-2-FU** (cleanup-cruft.sh hook bug).
...
- **§WALK series (sequential):** §WALK-3 ✅ + §WALK-4 ✅ complete → **§WALK-5** (MUD multi-client harness; instanced per-session encounters on `session/move`) — **lab report ✅ DESIGN LOCKED** (`lab-reports/lab-report-walk5-mud-harness.md`); implement Inc 1→4 per the report §7.
- **§WALK-1.5-FU** (geo follow-ups): (a) 4 anachronistic realms HKG/BKK/CTU/SJO anchored to Samarkand — off-grid/portal vs Earth-anchor; (b) ~19 German/scholarly interiors piled on Weimar (the 21-node pile at cell 19,191) — redistribute; (c) revisit 0.25° resolution for dense regions; (d) browser smoke-test of the sea-gated overworld; (e) ✅ **done** — Playwright nav tests rebuilt in §WALK-4 Inc 3.
...
>
> 1. **§WALK-5** (MUD multi-client harness) — lab report ✅ **DESIGN LOCKED** (`lab-reports/lab-report-walk5-mud-harness.md`); implement its 4-increment plan (§7), one per "continue": **Inc 1** wire `terrainAt`/`encounterRate`/`SEA_LANES` into `getMoverWorld()` + terrain-parity guard in `check:walk`; **Inc 2** per-session seeded RNG + instanced encounter roll on `session/move` success path (store at `s.encounter`, NOT `s.state`); **Inc 3** `tests/mud-harness.*` K-client driver (co-presence delivery + instancing + social state); **Inc 4** TTL-prune assert + doc-sync. Reconciliations already settled in the report: no `huntMode` (roll `< baseRate`), 409 block stays, ferry deferred → §WALK-5-FU, flat tier weights (no notoriety state server-side).
> 2. **Follow-ups & backlog** (see the sections above for full detail): **§WALK-1.5-FU(a–d)** (realm anchoring, Weimar 21-node pile redistribution, 0.25° resolution, overworld smoke-test — (e) Playwright rebuild is ✅ done), **§WALK-2-FU** (cleanup-cruft.sh `PRUNE_LIST[@]` hook bug), **Tooling** (§WORLDBUILDER-01, §EDITOR-02/03, §WALK-G, §WBAPI-01 ph3–5, §EDITOR-01-D, §CELL-14 run `--execute`), **Game-content arcs** (each needs a lab report first: §GR, §DESIGN-03, §DUNGEON-01, §MATH-01, §1367, §FUTURE-01, §GR-D), **Mechanics** (§ARCH-01 UQF phases 1–5, §MBIT-02, monster drop nerf, `fishmongerRowRestored`, UI gaps), **Design decisions** (Arc ID as UQF field; §MBIT-02-E ontology).
