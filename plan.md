...
> **Updated 2026-06-29 · branch `main` · last commit `cdc788f`.** Active work = **§ARCH-01 Phase 3 UQF migration**, **Wave 2 (script-assisted bulk)** in progress, **one family/cluster per "continue"** per the Directive. Each increment: migrate the next family via `scripts/uqf-bulk-migrate.js` → commit → `say` the subject → sync plan.md / index.md / the migration playbook → `npx playwright test quest-runtime-uqf` + `npm run check:walk` + `npx playwright test navigation`.
>
> **Verification (current):** `quest-runtime-uqf` **126 passed** (Wave 2 ×3-repeat clean); `check:walk` green; `navigation` 14/14. **Wave 1 COMPLETE** (~115 quests). **Wave 2 started:** first family **`hav_*`** (30 acts) bulk-migrated + parity-verified vs a pre-migration golden capture. Docs synced.
>
...
>
> **Next (on "continue"):** migrate the **next Wave-2 family/cluster**. **Landscape:** 2350 well-formed legacy skill_checks remain (30 → done in `hav_*`; ~2320 to go) — **0 carry onPass/onFail closures** (Wave 1 cleared them), dominant shape = `checkPassFlag`/no-bitLabel. Pick the next unit by node cluster (WM 285, CON 154, NUE 114, VEN/ROM ~58) or id-prefix family; capture golden, apply the script, verify, +tests. **NB:** 30 degenerate book-stubs (`blq_*` etc.: `reward:NaN`, `activateCond:()=>!!S_story.null`, no checkDC) are **out of scope** — never well-formed, never activatable. → then Waves 3–7 (see §ARCH-01 entry). **Archive → `plan-archive.md`.**
