...
- [ ] **§WALK-3** — Recast reweave as read-only `GET /api/graph/reachability`; retire `fill-gap`/`rip-and-connect`/`fix-all-broken`/`fix-bidirectional`/`reweave-all` (410); delete dead reweave body (`wbapi-server.js:5891+`).
  ...
  - Inc 2 ⬜ **410 the mutators** — `POST /api/graph/fill-gap` (creates junction stubs §WALK-1 abolished) + `POST /api/graph/rip-and-connect` (relocates strays) → return 410 pointing at `GET /api/graph/reachability`; drop their `./api.sh` commands. (`reweave-all` already 410s at :5937.)
