...
**Current state (2026-06-26, last commit `5a54657`):** Active thread **§TIMELESS-01** (timeless one-cell movement + Hunt-feature removal) — **Inc A + Inc B + Inc C done & committed** (movement no longer advances the clock; all Hunt/Stalk JS + UI/DOM/CSS removed; verified 0 console errors in headless Chromium). **Immediate next: §TIMELESS-01 Inc D** (comment `slStalksWon`, reword map-click hint, doc sync). Prior thread §WALK-1/1.5/2 + §WALK-3 Inc 1/2 done; **§WALK-3 Inc 3 queued** behind Inc D. Working tree clean except untracked `1367-sources/pla.md` + incidental `milepoints/api-cli.log` churn (hook-managed; don't stage). wbapi-server restarted (in-memory matches disk). Full status + copy-paste prompt at the bottom of this file.
...
- [ ] **§WALK-3** — Recast reweave as read-only `GET /api/graph/reachability`; retire `fill-gap`/`rip-and-connect`/`fix-all-broken`/`fix-bidirectional`/`reweave-all` (410); delete dead reweave body (`wbapi-server.js:5891+`).
  ...
  - Inc 3 ⬜ **delete dead reweave body** — `wbapi-server.js` ~5938→~9183 (~3,200 lines below the `reweave-all` early-return, incl. P1 rip-and-connect / P4 fix-all-broken / P5 fix-bidirectional phases). Verify endpoint table + `./api.sh` unaffected after deletion.
...
- [ ] **Inc D — Quest + wording + docs** — confirm `quest_slums_cleanup` completes via 3 BMA battle wins (comment that `slStalksWon` = BMA wins; keep field name, no save migration); reword the map-click "not reachable in one step" hint if it reads as distance; sync `mechanics-combat.md §Hunting Mode` (retire), `index.md` (Hunt reverse-lookup rows + badge + state-field count), `plan.md`. Two-way sync rule.
...
> **Updated 2026-06-26 · branch `main` · last commit `5a54657`.** This is the canonical handoff — the most current snapshot of where the work stands and everything still outstanding. (The top-of-file §RESUME just points here.)

### Where we are

**§TIMELESS-01 (timeless one-cell movement + Hunt-feature removal)** is the active thread (user directive 2026-06-26; spec: `lab-reports/lab-report-timeless-movement-hunt-removal.md`). Done & committed: **Inc A** (movement no longer advances the clock — removed the two `hoursElapsed`/`hoursSinceSlept` writes in `cellMove`), **Inc B** (removed all Hunt/Stalk JavaScript — encounter branch → plain `baseRate`+`_weightedMonsterPick`; deleted hunt fns, `HUNTING_GROUNDS`, `huntMode` state, `pb.stalk` flags/guards, hunt cards), **Inc C** (removed the 🎯 button → inert spacer, the stalk modal HTML + hunt-specific CSS; kept shared `.btn-hunt`/`.hunt-accordion`). Verified in headless Chromium: 0 console errors on load; hunt button/modal/fns gone; 5 cell-moves keep `hoursElapsed` at 0. **Inc D remains** (quest comment + wording + doc sync).

Prior thread — **§WALK navigation-core redesign** (lab report: `lab-reports/lab-report-terrain-field-mover-redesign.md`): **§WALK-1** (junctions deleted), **§WALK-1.5** (geo re-projection + SEA_MASK + sea-lane land-bridges), **§WALK-2** (shared `mover.js`), **§WALK-3 Inc 1** (reachability = land flood, 409/409) ✅ and **Inc 2** (410'd `fill-gap`+`rip-and-connect`) ✅ — all done & committed. **§WALK-3 Inc 3** is queued behind §TIMELESS-01.
...
**§TIMELESS-01 Inc D (quest + wording + docs)** — finish the timeless/hunt thread:
1. In `roll2hit-v3.html`, add a clarifying comment at the `slStalksWon` increment (`pb.nodeCode === 'BMA'`) that it counts BMA battle wins (keep the field name — no save migration); confirm `quest_slums_cleanup` still completes via 3 BMA wins (it never depended on Hunt Mode).
2. Reword the map-click hint *"not reachable in one step. Use D-pad to navigate toward it."* if it reads as a "gap distance" concept.
3. Two-way doc sync: retire `mechanics-combat.md §Hunting Mode`; update `index.md` (Hunt-Mode reverse-lookup rows, doc-health badge, state-field count now that `huntMode` is gone); flip the §TIMELESS-01 Inc D checkbox.

**Then §WALK-3 Inc 3** (queued) — delete the dead `reweave-all` body (`wbapi-server.js`, ~3,200 lines below the `reweave-all` early-return at :5763; also removes the `fix-all-broken`/`fix-bidirectional` phase code, which are not separate endpoints) **and** the now-orphaned `reweave` CLI orchestrator in `api/wb.js` (it called the 410'd rip-and-connect; already un-advertised in Inc 2). Restart the server; confirm the endpoint table + `./api.sh` are unaffected.

### All outstanding work (index into the sections above)

- **§TIMELESS-01 (active):** Inc A ✅ + Inc B ✅ + Inc C ✅ done; **Inc D (quest comment + map-click wording + doc sync) remains** — the only outstanding piece. Spec: `lab-reports/lab-report-timeless-movement-hunt-removal.md`. Touches `roll2hit-v3.html` directly (engine code, not WBAPI data).
- **§WALK series (sequential):** §WALK-3 Inc 3 (delete dead reweave body + `reweave` CLI) → **§WALK-4** (CI-gated invariant suite: I1/I2/I3 reachability proof + walk-parity; also rebuilds the stale Playwright nav tests) → **§WALK-5** (MUD multi-client harness; instanced per-session encounters on `session/move`).
...
>
> 1. **§TIMELESS-01 Inc D** (finishes the active thread) — in `roll2hit-v3.html`: (a) comment the `slStalksWon` increment (`pb.nodeCode === 'BMA'`) to note it counts BMA battle wins, keep the field name (no save migration), and confirm `quest_slums_cleanup` still completes via 3 BMA wins; (b) reword the map-click *"not reachable in one step"* hint if it reads as a gap-distance concept; (c) doc sync — retire `mechanics-combat.md §Hunting Mode`, update `index.md` (Hunt reverse-lookup rows + doc-health badge + state-field count now `huntMode` is gone), flip the Inc D checkbox. Smoke-test in headless Chromium (0 console errors, hunt UI absent, moves keep `hoursElapsed` flat).
> 2. **§WALK-3 Inc 3** — delete the dead `reweave-all` body in `wbapi-server.js` (~3,200 lines below the early-return at :5763, incl. P1 rip-and-connect / P4 fix-all-broken / P5 fix-bidirectional phases) **and** the orphaned `reweave` CLI orchestrator in `api/wb.js` (already un-advertised; it called the now-410 rip-and-connect). Restart the server; confirm the endpoint table + `./api.sh` still work (`./api.sh reachability` = 409/409, mover parity green).
> 3. **§WALK-4** — CI-gated invariant suite (I1/I2/I3 reachability proof + walk-parity) and rebuild the stale Playwright nav tests. **§WALK-5** — MUD multi-client harness; instanced per-session encounters on `session/move` (wire server `terrainAt`/`encounterRate` per §WALK-2-FU; resolve the inert ferry hook).
