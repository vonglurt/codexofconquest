...
**§WALK-4 Inc 3** — rebuild the stale Playwright nav tests. Inc 1 ✅ (invariant proof, caught + fixed 2 latent bugs) + Inc 2 ✅ (CI gate `.github/workflows/walk-invariants.yml` + `npm run check:walk` aliases). Next: `tests/integration/navigation.test.js` (+`worldbuilder-walk.test.js`) hardcode pre-§WALK-1.5 coords/adjacencies (BOO 47,223→2,194; LHR+BK co-locate at 10,197) and are red on main — reseed against current geo, assert one-cell moves + sea-block (§WALK-1.5-FU(e)).
