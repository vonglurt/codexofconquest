...
>
> **Next on "continue": §MESH-01-FU 9 — world-diff depth** (deep-parse via wbapi-core `extractObj` for exact per-entry field diffs + `--json` mode), or FU 10–13 · §MESH-01-REVIEW refactors · §DATA-01-REVERTED decision · or a planned arc (**§GR / §DESIGN-03 / §DUNGEON-01 / §MATH-01** — each needs its lab report locked before HTML edits; see Game Content below). *(FU 7+8 ✅ 2026-07-06 → archived; mud-harness now **235/235** incl. [P] rate limiting.)*
>
