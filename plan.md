...
**Current state (2026-06-26):** §WALK-1/1.5/2 **done & committed**; §WALK-3 **in progress** (Inc 1 + Inc 2 ✅). **New active thread: §TIMELESS-01** (timeless one-cell movement + Hunt-feature removal) — spec + lab report locked, **implementation Inc A next**. Working tree clean except untracked `1367-sources/pla.md` + incidental `milepoints/api-cli.log` churn (hook-managed; don't stage). wbapi-server restarted (in-memory matches disk). **Immediate next: §TIMELESS-01 Inc A** (movement timeless) — or resume §WALK-3 Inc 3 (delete dead `reweave-all` body) if continuing the nav-tooling thread. Full status + copy-paste prompt at the bottom of this file.
...
- [ ] **Inc A — Movement timeless** — delete the two `hoursElapsed`/`hoursSinceSlept` += 1 lines in `cellMove` (roll2hit-v3.html ~25762–63). Verify: moving leaves `⏱ Hours` unchanged; battle/sleep still advance it. (Short-rest costs at 6449/23413 are NOT movement — keep.)
- [ ] **Inc B — Hunt logic + state** — `cellMove` encounter branch → plain path (always `baseRate` + `_weightedMonsterPick` + "Wild …"); remove hunt-only fns (`storyToggleHunt`, `_updateHuntBtn`, `storyQuestHunt`, `storyQuickWait`, `_stalkedMonsterPick`, `_getQuestTargetKeys`, `HUNTING_GROUNDS`), `huntMode` default, `pb.stalk` flags + `!pb.stalk` guards, the `btn-hunt-toggle` listener, escape-key + storyRender stalk-modal hides. Grep-clean target.
- [ ] **Inc C — Hunt UI/DOM/CSS** — remove 🎯 `#btn-hunt-toggle` (4290), stalk modal HTML (4388–4399), hunt CSS (`.btn-hunt` 1054/3030; `#story-stalk-modal`/`#stalk-card`/`#btn-stalk-*` 2174/3203), and the HUNT cards in `storyRender` (29792–29887, 29900–29907). Story node renders with no HUNT card; d-pad has no 🎯; no console errors.
...
**§TIMELESS-01 Inc A (movement timeless)** is the newly-prioritized active thread (user directive 2026-06-26; spec locked in `lab-reports/lab-report-timeless-movement-hunt-removal.md`): delete the two time-advance lines in `cellMove` (roll2hit-v3.html ~25762–63), then Inc B (hunt logic/state) → Inc C (hunt UI/DOM/CSS) → Inc D (quest comment + wording + docs). The nav-tooling thread's next step (queued behind it) is:

**§WALK-3 Inc 3** — delete the dead `reweave-all` body (`wbapi-server.js`, ~3,200 lines below the `reweave-all` early-return at :5763; this also removes the `fix-all-broken`/`fix-bidirectional` phase code, which are not separate endpoints) **and** the now-orphaned `reweave` CLI orchestrator in `api/wb.js` (it called the 410'd rip-and-connect; already un-advertised from help in Inc 2). Verify the endpoint table + `./api.sh` are unaffected after deletion.

### All outstanding work (index into the sections above)

- **§TIMELESS-01 (active, new):** Inc A movement-timeless → Inc B hunt logic/state → Inc C hunt UI/DOM/CSS → Inc D quest+wording+docs. Spec: `lab-reports/lab-report-timeless-movement-hunt-removal.md`. Touches `roll2hit-v3.html` directly (engine code, not WBAPI data).
...
> Read plan.md §RESUME (newest, bottom of file). Continue the §WALK series: do **§WALK-3 Inc 3** — delete the dead `reweave-all` body in `wbapi-server.js` (~3,200 lines below the early-return at :5763, incl. the P1 rip-and-connect / P4 fix-all-broken / P5 fix-bidirectional phases) **and** the orphaned `reweave` CLI orchestrator in `api/wb.js` (already un-advertised; it called the now-410 rip-and-connect). Restart the server and confirm the endpoint table + `./api.sh` still work (`./api.sh reachability` = 409/409, mover parity green). Work one increment per "continue" per the Directive; commit + `say` each subject; keep plan.md and index.md in sync.
