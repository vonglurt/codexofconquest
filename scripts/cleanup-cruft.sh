...
# in each category are kept; older ones are deleted.  Logs above their size
# cap are truncated in place (keeping the most recent tail).
#
# Usage:
...
# Delete every file in $@ except the most recent $1 (by filename, which is
# timestamped — newest sorts last under `sort`).  Honors --dry-run.
prune_keep_newest() {
  local keep=$1; shift
  local label=$1; shift
  local total=$#
  ...
  note "$label: $total file(s), pruning $cull oldest, keeping $keep newest"
  local i=0
  for f in "$@"; do
    i=$((i + 1))
    if [ "$i" -le "$cull" ]; then
...
cap_log_size() {
  ...
  note "$f: $(echo "scale=1; $bytes/1048576" | bc 2>/dev/null || echo "$bytes B") > ${LOG_CAP_MB}MB — truncating to last ${LOG_CAP_MB}MB"
