...
70 entries: 14 base weapon types × 5 magic tiers (base, +1, +2, +3, +4) — `[0,1,2,3,4].flatMap(...)` over `_BASE_WEAPONS` (`const _BASE_WEAPONS@24420`).
...
**Magic-tier level gate:** `_magicTierAllowed(magic)` (`function _magicTierAllowed@24458`) = player `level ≥ magic × 5` — so **+1 → Lv5, +2 → Lv10, +3 → Lv15, +4 → Lv20** (note: no `baseLv` term in the gate). Each item also carries a per-entry `minLevel = min(20, max(magic × 5, baseLv + magic × 4))` used for display/sort.
