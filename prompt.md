...
- `quest-runtime-uqf.test.js` — **now fully green** (was "green except one deferred red"). The long-standing *"~17–18-failure env baseline / live-server clobbers the mock world"* claim was **disproved 2026-07-22 (§DX-01b)**: the failing set was **byte-identical server-up vs server-down** — it was 17 *deterministic stale tests*, not env flakes (11 never updated for §XP-01 effort-XP, 4 for the §CEREMO-ACC card refactor, 1 brittle census, 1 real content finding). 16 were fixed then; **the git-stash-diff ceremony is retired** (a real regression now shows up directly). The last red — the `§ARCH-01 Wave 4` *"no legacy residue"* census over the Void-tide combat quests — was **retired 2026-07-23 (§BOARD-01-VOID-GATE)**: the 3 quests' day-gating moved off an `activateCond` closure into a `gate:{dayMin,dayMax}` grammar leaf, so the suite carries **no known red**.
