...
**Known pre-existing baseline reds: `check:walk` is fully green (11/11 since 2026-07-28, §DX-01a) — any red there is a real regression you introduced. The Playwright suite carries TWO:** (1) `tests/integration/worldbuilder-crud-arrays.test.js` fails **4/6** (CRUD array-field + itemChain widgets; it never loads `roll2hit-v3.html`), re-proved pre-existing by a stash-diff at HEAD on 2026-07-29 — see §DX-02d in BACKLOG.md; (2) `tests/integration/kg-zones.test.js` — *"the corridor is real road"* fails on 4 of its 6 sample cells (`10,205`/`10,209`/`11,210`/`14,216` are no longer in `ROAD_CELLS`), a §DX-01a road-regen leftover proved pre-existing at HEAD on 2026-07-29 — see §DX-02e.
