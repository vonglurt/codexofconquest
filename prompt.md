...
```bash
npm run check:walk        # 15 world-invariant CI gates (invariants/dupkeys/parity/behaviour/terrain/roads/rooms/quest/gate/rng/questgraph/nodeindex/noderegs/npcregs/anchors)
...
**Known pre-existing baseline reds: `check:walk` is fully green (15/15; 11/11 since 2026-07-28 §DX-01a, gate #12 added 2026-07-29 §AUDIT-03l, gate #13 added 2026-07-31 §AUDIT-03j, gate #14 added 2026-07-31 §AUDIT-03n, gate #15 added 2026-07-31 §DX-01e) — any red there is a real regression you introduced. The Playwright suite carries FIVE, in TWO files** — a full run is **769 passed / 5 failed / 0 flaky**, last re-proved **2026-08-04** (§AUDIT-03k; `multiplayer-presence.test.js:171` is a known flake that retries green — it is not a sixth red). This list is the whole red set; **anything beyond these five is yours**:
> - `worldbuilder-crud-arrays.test.js` — **4/6** (CRUD array-field + itemChain widgets; never loads `roll2hit-v3.html`) — §DX-02d
> - `dx02l-save-snapshots-cli.test.js:190` — *"no author-facing doc reaches `/api/save` with raw curl"*. **The docs are fine; the assertion is line-level.** CONTRIBUTING Hazard #7 — written by §DX-02l itself — now narrates *"verifying §DX-02k needed raw `curl`"* on the same line as `POST /api/save`, so a historical account of the defect reads to the regex as an instruction to commit it. Pre-existing, proved at HEAD on a clean tree during §AUDIT-03k (2026-08-04) — §DX-02l-FU
>
