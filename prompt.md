...
- **A verb is authored in `NODE_VERBS` (§VM-01-G4b).** The third small registry beside `NODE_PANELS`
  (flavour a node *shows*) and `NODE_HOOKS` (an interface a node *owns*): `{ id, nodes, when?, label?,
  bits?, ambient? }` — **`label` + `bits`** is a button that runs the chain on click; **`bits` with no
  `label`** means the chain *is* the surface, so it runs at render and its first bit **must** be a
  suspending `choice` (anything else is refused out loud — it would apply its effects on every draw);
  **`ambient`** alone is a flavour line for a state with nothing to do. Dispatch is **in place**
  (`_renderNodeVerbs` is called where the migrated block sat), so DOM order is preserved by
  construction. First consumer: Kern & Sable at DUS — the first `choice` ever executed in this game.
