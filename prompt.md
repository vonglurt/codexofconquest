...
- `quest-runtime-uqf.test.js` — **green except one deferred red** (`§ARCH-01 Wave 4` *"no legacy residue"*, the Void-tide combat quests). The long-standing *"~17–18-failure env baseline / live-server clobbers the mock world"* claim was **disproved 2026-07-22 (§DX-01b)**: the failing set was **byte-identical server-up vs server-down** — it was 17 *deterministic stale tests*, not env flakes (11 never updated for §XP-01 effort-XP, 4 for the §CEREMO-ACC card refactor, 1 brittle census, 1 real content finding). 16 were fixed; **the git-stash-diff ceremony is retired** (a real regression now shows up directly). The 1 remaining red is a genuine finding, not noise — deferred to **§BOARD-01-VOID-GATE** (3 Void-tide combat quests day-gate via an `activateCond` closure the grammar can't yet express).
